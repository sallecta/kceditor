...
( function() {
	...
	var DTD = kceditor.dtd,
		...
		// processElement flag - meaning explained in kceditor.FILTER_SKIP_TREE doc.
		FILTER_SKIP_TREE = 2,
		copy = kceditor.tools.copy,
	 ...
	 *
	 * See {@link kceditor.filter#addElementCallback} for more details.
	 *
	 * @since 4.4.0
	 * @readonly
	 * @property {Number} [=2]
	 * @member kceditor
	 */
	kceditor.FILTER_SKIP_TREE = FILTER_SKIP_TREE;
	 ...
	 *
	 * A filter instance is always available under the {@link kceditor.editor#filter}
	 ...
	 *
	 *	* By the user, with the {@link kceditor.config#allowedContent} setting.
	 ...
	 * setting the {@link kceditor.config#extraAllowedContent}
	 * configuration option.
	 *
	 * **Note**: Filter rules will be extended with the following elements
	 * depending on the {@link kceditor.config#enterMode} and
	 * {@link kceditor.config#shiftEnterMode} settings:
	 *
	 *	* `'p'` &ndash; for {@link kceditor#ENTER_P},
	 *	* `'div'` &ndash; for {@link kceditor#ENTER_DIV},
	 *	* `'br'` &ndash; for {@link kceditor#ENTER_BR}.
	 *
	 ...
	 * {@link kceditor.filter.allowedContentRules} instead of {@link kceditor.editor}
	 * to the constructor:
	 *
	 * ```javascript
	 * var filter = new kceditor.filter( 'b' );
	 *
	 * filter.check( 'b' ); // -> true
	 ...
	 * with the {@link kceditor.editor#method-destroy} method to prevent memory leaks.
	 *
	 * ```javascript
	 * // In both cases the filter will be removed during the {@link kceditor.editor#method-destroy} function execution.
	 * var filter1 = new kceditor.filter( editor );
	 * var filter2 = new kceditor.filter( editor, 'b' );
	 * ```
	 *
	 * @since 4.1.0
	 * @class
	 * @constructor Creates a filter class instance.
	 * @param {kceditor.editor/kceditor.filter.allowedContentRules} editorOrRules
	 * @param {kceditor.filter.allowedContentRules} [rules] This parameter is available since 4.11.0.
	 */
	kceditor.filter = function( editorOrRules, rules ) {
		/**
		 * Whether custom {@link kceditor.config#allowedContent} was set.
		 *
		 ...
		 * loaded from {@link kceditor.config#allowedContent} and
		 * {@link kceditor.config#extraAllowedContent}).
		 *
		 ...
		 * loaded from {@link kceditor.config#disallowedContent}).
		 *
		 ...
		 *
		 * To disable the filter, set {@link kceditor.config#allowedContent} to `true`
		 ...
		 *
		 * @readonly
		 * @property {kceditor.editor} [=null]
		 */
		this.editor = null;

		/**
		 * Filter's unique id. It can be used to find filter instance in
		 * {@link kceditor.filter#instances kceditor.filter.instance} object.
		 *
		 * @since 4.3.0
		 * @readonly
		 * @property {Number} id
		 */
		this.id = kceditor.tools.getNextNumber();
		...
		kceditor.filter.instances[ this.id ] = this;

		var editor = this.editor = editorOrRules instanceof kceditor.editor ? editorOrRules : null;
	 ...
	 * {@link #id} properties.
	 *
	 *		var filter = new kceditor.filter( 'p' );
	 *		filter === kceditor.filter.instances[ filter.id ];
	 *
	 * @since 4.3.0
	 * @static
	 * @property instances
	 */
	kceditor.filter.instances = {};

	kceditor.filter.prototype = {
		/**
		 ...
		 *		var header1Style = new kceditor.style( { element: 'h1' } ),
		 *			header2Style = new kceditor.style( { element: 'h2' } );
		 *		editor.filter.allow( [ header1Style, header2Style ], 'HeadersCombo' );
		 *
		 * @param {kceditor.filter.allowedContentRules} newRules Rule(s) to be added.
		 ...
		 * if {@link kceditor.config#allowedContent} is defined to avoid overriding it.
		...
		allow: function( newRules, featureName, overrideCustom ) {
			...
			else if ( newRules instanceof kceditor.style ) {
			...
			} else if ( kceditor.tools.isArray( newRules ) ) {
		...
		/**
		 * Applies this filter to passed {@link kceditor.htmlParser.fragment} or {@link kceditor.htmlParser.element}.
		 * The result of filtering is a DOM tree without disallowed content.
		 *
		 *			// Create standalone filter passing 'p' and 'b' elements.
		 *		var filter = new kceditor.filter( 'p b' ),
		 *			// Parse HTML string to pseudo DOM structure.
		 *			fragment = kceditor.htmlParser.fragment.fromHtml( '<p><b>foo</b> <i>bar</i></p>' ),
		 ...
		 *
		 * @param {kceditor.htmlParser.fragment/kceditor.htmlParser.element} fragment Node to be filtered.
		 * @param {Boolean} [toHtml] Set to `true` if the filter is used together with {@link kceditor.htmlDataProcessor#toHtml}.
		 ...
		 * Defaults to {@link kceditor.editor#activeEnterMode} for a editor's filter or to {@link kceditor#ENTER_P} for standalone filter.
		...
		applyTo: function( fragment, toHtml, transformOnly, enterMode ) {
			...
			fragment.forEach( function( el ) {
				if ( el.type == kceditor.NODE_ELEMENT ) {
					...
					// (https://dev.kceditor.com/ticket/10260) Don't touch elements like spans with data-cke-* attribute since they're
					...
					//       (toDataFormat).
					if ( toHtml && el.name == 'span' && ~kceditor.tools.object.keys( el.attributes ).join( '|' ).indexOf( 'data-cke-' ) )
						return;
				...
				else if ( el.type == kceditor.NODE_COMMENT && el.value.match( /^\{cke_protected\}(?!\{C\})/ ) ) {
			...
			var node, element, check,
				toBeChecked = [],
				enterTag = enterModeTags[ enterMode || ( this.editor ? this.editor.enterMode : kceditor.ENTER_P ) ],
				parentDtd;
			...
			while ( ( check = toBeChecked.pop() ) ) {
				...
				// Handle custom elements as inline elements (https://dev.kceditor.com/ticket/12683).
				...
				switch ( check.check ) {
					...
					case 'el-up':
						...
						if ( element.parent.type != kceditor.NODE_DOCUMENT_FRAGMENT && !parentDtd[ element.name ] )
							removeElement( element, enterTag, toBeChecked );
						break;
		...
		/**
		 * Checks whether a {@link kceditor.feature} can be enabled. Unlike {@link #addFeature},
		 * this method always checks the feature, even when the default configuration
		 * for {@link kceditor.config#allowedContent} is used.
		 *
		 *		// TODO example
		 *
		 * @param {kceditor.feature} feature The feature to be tested.
		 ...
		 * {@link kceditor.config#allowedContent} to `true`.
		 */
		disable: function() {
			this.disabled = true;
		},
		 ...
		 *
		 * @since 4.4.0
		 * @param {kceditor.filter.disallowedContentRules} newRules Rule(s) to be added.
		 */
		...
		/**
		 * Adds an array of {@link kceditor.feature} content forms. All forms
		 ...
		 *
		 * This method is used by the editor to register {@link kceditor.feature#contentForms}
		 * when adding a feature with {@link #addFeature} or {@link kceditor.editor#addFeature}.
		 *
		...
		addContentForms: function( forms ) {
			...
			for ( i = 0; i < forms.length && !preferredForm; ++i ) {
				...
				if ( ( typeof form == 'string' || form instanceof kceditor.style ) && this.check( form ) )
					preferredForm = form;
			}
		 ...
		 *
		 * By returning {@link kceditor#FILTER_SKIP_TREE} it is possible to
		 * skip filtering of the current element and all its ancestors.
		 *
		 *		editor.filter.addElementCallback( function( el ) {
		 *			if ( el.hasClass( 'protected' ) )
		 *				return kceditor.FILTER_SKIP_TREE;
		 *		} );
		 *
		 ...
		 *  use the pure {@link kceditor.htmlParser.element} interface.
		 *
		 * @since 4.4.0
		 ...
		 * for the current KCEditor instance, based on the HTML code the feature might
		 * generate and the minimal HTML code the feature needs to be able to generate.
		 *
		 *		// TODO example
		 *
		 * @param {kceditor.feature} feature
		 * @returns {Boolean} Whether this feature can be enabled.
		 */
		 ...
		 *
		 *	* `check` (optional) &ndash; if set and {@link kceditor.filter} does
		 *		not accept this {@link kceditor.filter.contentRule}, this transformation rule
		 ...
		 *		or `left` (if it is a {@link kceditor.style} instance).
		 *	* `left` (optional) &ndash; a function accepting an element or a {@link kceditor.style}
		 ...
		 *	* `right` &ndash; a function accepting an element and {@link kceditor.filter.transformationsTools}
		 *		or a string containing the name of the {@link kceditor.filter.transformationsTools} method
		 ...
		 *				// executes {@link kceditor.filter.transformationsTools#sizeToStyle} on a table element.
		 ...
		 *
		 * This method is used by the editor to add {@link kceditor.feature#contentTransformations}
		 * when adding a feature by {@link #addFeature} or {@link kceditor.editor#addFeature}.
		 *
		 ...
		 * ```
		 *
		 * **Note:** The `test` argument is of {@link kceditor.filter.contentRule} type, which is
		 * a limited version of {@link kceditor.filter.allowedContentRules}. Read more about it
		 * in the {@link kceditor.filter.contentRule}'s documentation.
		 *
		 * @param {kceditor.filter.contentRule/kceditor.filter.contentRule[]} test
		...
		check: function( test, applyTransformations, strictCheck ) {
			...
			// the rules.
			if ( kceditor.tools.isArray( test ) ) {
			...
			if ( typeof test == 'string' ) {
				...
				// Create test element from kceditor.style.
				element = mockElementFromStyle( test );
			}

			// Make a deep copy.
			var clone = kceditor.tools.clone( element ),
				toBeRemoved = [],
				transformations;
			...
			if ( toBeRemoved.length > 0 ) {
				...
				result = kceditor.tools.objectCompare( element.attributes, clone.attributes, true );
		 ...
		 * If none of tags is allowed this method will return {@link kceditor#ENTER_BR}.
		 *
		 * @since 4.3.0
		...
		getAllowedEnterMode: ( function() {
			var tagsToCheck = [ 'p', 'div', 'br' ],
				enterModes = {
					p: kceditor.ENTER_P,
					div: kceditor.ENTER_DIV,
					br: kceditor.ENTER_BR
				};
		 ...
		 *
		 * @since 4.7.3
		 * @returns {kceditor.filter}
		 */
		clone: function() {
			var ret = new kceditor.filter(),
				clone = kceditor.tools.clone;
		...
		/**
		 * Destroys the filter instance and removes it from the global {@link kceditor.filter#instances} object.
		 *
		 * @since 4.4.5
		 */
		destroy: function() {
			delete kceditor.filter.instances[ this.id ];
	...
	// Convert kceditor.style to filter's rule.
	function convertStyleToRules( style ) {
		...
		rules[ styleDef.element ] = rule = {
			...
			requiredStyles: styleDef.styles && kceditor.tools.object.keys( styleDef.styles )
		};
		...
		if ( attrs ) {
			...
			rule.requiredAttributes = attrs && kceditor.tools.object.keys( attrs );
		}
	...
	function convertValidatorToHash( validator, delimiter ) {
		...
		if ( typeof validator == 'string' ) {
			...
			if ( validator == '*' )
				...
				return kceditor.tools.convertArrayToObject( validator.split( delimiter ) );
		}
		else if ( kceditor.tools.isArray( validator ) ) {
			if ( validator.length )
				return kceditor.tools.convertArrayToObject( validator );
			else
				return false;
		}
		// If object.
	...
	// to the element.
	//
	// @param {kceditor.filter} that The context.
	// @param {kceditor.htmlParser.element} element
		...
		if ( ( reqs = rule.requiredClasses ) ) {
			...
			for ( i = 0; i < reqs.length; ++i ) {
				...
				if ( typeof req == 'string' ) {
					if ( kceditor.tools.indexOf( existing, req ) == -1 )
						return false;
				}
				// This means regexp.
				else {
					if ( !kceditor.tools.checkIfAnyArrayItemMatches( existing, req ) )
						return false;
				}
			}
		}
	...
	function hasAllRequiredInHash( existing, required ) {
		...
		for ( var i = 0, req; i < required.length; ++i ) {
			...
			// This means regexp.
				...
				if ( !kceditor.tools.checkIfAnyObjectPropertyMatches( existing, req ) )
					return false;
			}
		}
	...
	function mockElementFromString( str ) {
		...
		if ( styles )
			element.attributes.style = kceditor.tools.writeCssText( element.styles );
	...
	function mockElementFromStyle( style ) {
		...
		if ( styles && !kceditor.tools.isEmpty( styles ) ) {
			styles = copy( styles );
			attrs.style = kceditor.tools.writeCssText( styles, true );
		} else {
			styles = {};
		}
	...
	function populateProperties( element ) {
		...
		if ( !element.styles )
			element.styles = kceditor.tools.parseCssText( styles || '', 1 );
		if ( !element.classes )
	...
	function processProtectedElement( that, comment, protectedRegexs, filterOpts ) {
		...
		protectedFrag = kceditor.htmlParser.fragment.fromHtml( source );

		if ( protectedFrag.children.length == 1 && ( node = protectedFrag.children[ 0 ] ).type == kceditor.NODE_ELEMENT )
	...
	//
	// @param {kceditor.filter} that Context.
	// @param {kceditor.htmlParser.element} element The element to be processed.
	...
	function updateAttributes( element ) {
		...
		if ( ( styles = kceditor.tools.writeCssText( element.styles, true ) ) )
			attrs.style = styles;
	...
	function checkChildren( children, newParentName ) {
		...
		for ( var i = 0, l = children.length, child; i < l; ++i ) {
			child = children[ i ];
			if ( child.type == kceditor.NODE_ELEMENT && !allowed[ child.name ] )
				return false;
		}
	...
	function createBr() {
		return new kceditor.htmlParser.element( 'br' );
	}

	// Whether this is an inline element or text.
	function inlineNode( node ) {
		return node.type == kceditor.NODE_TEXT ||
			node.type == kceditor.NODE_ELEMENT && DTD.$inline[ node.name ];
	}
	...
	function stripBlock( element, enterTag, toBeChecked ) {
		...
		var parent = element.parent,
			shouldAutoP = parent.type == kceditor.NODE_DOCUMENT_FRAGMENT || parent.name == 'body',
			i, child, p, parentDtd;
		...
		for ( i = children.length; i > 0; ) {
			...
			if ( shouldAutoP && inlineNode( child )  ) {
				if ( !p ) {
					p = new kceditor.htmlParser.element( enterTag );
			...
			// Child which doesn't need to be auto paragraphed.
				...
				if ( parent.type != kceditor.NODE_DOCUMENT_FRAGMENT &&
					child.type == kceditor.NODE_ELEMENT &&
	...
	// Check whether element matches kceditor.style.
	...
	function getContentFormTransformationGroup( form, preferredForm ) {
		...
		if ( typeof form == 'string' )
			element = form;
		else if ( form instanceof kceditor.style )
			left = form;
		else {
			element = form[ 0 ];
			left = form[ 1 ];
	...
	function optimizeTransformationsGroup( rules ) {
		...
		for ( i = 0; i < rules.length; ++i ) {
			...
			if ( left instanceof kceditor.style )
	 ...
	 *
	 * @class kceditor.filter.transformationsTools
	 * @singleton
	 */
	transformationsTools = kceditor.filter.transformationsTools = {
		/**
		...
		splitBorderShorthand: function( element ) {
			...
			var borderSplittedStyles = kceditor.tools.style.parse.border( element.styles.border );
		...
		/**
		 * Checks whether an element matches a given {@link kceditor.style}.
		 ...
		 *
		 * @param {kceditor.htmlParser.element} el
		 * @param {kceditor.style/String} form
		 */
		transform: function( el, form ) {
			if ( typeof form == 'string' )
				el.name = form;
			// Form is an instance of kceditor.style.
			else {
 ...
 * instantiating {@link kceditor.editor#filter}.
 *
 * The following values are accepted:
 *
 *	* {@link kceditor.filter.allowedContentRules} &ndash; defined rules will be added
 *	to the {@link kceditor.editor#filter}.
 ...
 * {@link kceditor.config#extraAllowedContent}. This option may be especially
 * useful when you want to use the default `allowedContent` value
 * along with some additional rules.
 *
 *		kceditor.replace( 'textarea_id', {
 ...
 *
 * @since 4.1.0
 * @cfg {kceditor.filter.allowedContentRules/Boolean} [allowedContent=null]
 * @member kceditor.config
 */

/**
 * This option makes it possible to set additional allowed
 * content rules for {@link kceditor.editor#filter}.
 *
 * It is especially useful in combination with the default
 * {@link kceditor.config#allowedContent} value:
 *
 *		kceditor.replace( 'textarea_id', {
 ...
 * and see the {@glink examples/acf example}.
 * See also {@link kceditor.config#allowedContent} for more details.
 *
 * @since 4.1.0
 ...
 * and see the {@glink examples/acf example}.
 * See also {@link kceditor.config#allowedContent} and {@link kceditor.config#extraAllowedContent}.
 *
 * @since 4.4.0
 * @cfg {kceditor.filter.disallowedContentRules} disallowedContent
 * @member kceditor.config
 */

/**
 * This event is fired when {@link kceditor.filter} has stripped some
 * content from the data that was loaded (e.g. by {@link kceditor.editor#method-setData}
 * method or in the source mode) or inserted (e.g. when pasting or using the
 * {@link kceditor.editor#method-insertHtml} method).
 *
 * This event is useful when testing whether the {@link kceditor.config#allowedContent}
 * setting is sufficient and correct for a system that is migrating to KCEditor 4.1.0
 ...
 *	* a {@link kceditor.style} instance &ndash; used mainly for integrating plugins with Advanced Content Filter,
 *	* an array of the above formats.
 *
 * @since 4.1.0
 * @class kceditor.filter.allowedContentRules
 * @abstract
 */

/**
 * Virtual class representing the {@link kceditor.filter#disallow} argument and a type of
 * the {@link kceditor.config#disallowedContent} option.
 *
 * This is a simplified version of the {@link kceditor.filter.allowedContentRules} type.
 ...
 *
 * @since 4.4.0
 * @class kceditor.filter.disallowedContentRules
 * @abstract
 */

/**
 * Virtual class representing {@link kceditor.filter#check} argument.
 *
 * This is a simplified version of the {@link kceditor.filter.allowedContentRules} type.
 * It may contain only one element and its styles, classes, and attributes. Only the
 * string format and a {@link kceditor.style} instances are accepted. Required properties
 * are not allowed in this format.
 *
 * Example:
 *
 ...
 *
 * @since 4.1.0
 * @class kceditor.filter.contentRule
 * @abstract
 */
 ...
 *
 *	* {@link kceditor.command}
 *	* {@link kceditor.ui.button}
 *	* {@link kceditor.ui.richCombo}
 *
 * None of them have a `name` property explicitly defined, but
 * it is set by {@link kceditor.editor#addCommand} and {@link kceditor.ui#add}.
 *
 * During editor initialization all features that the editor should activate
 * should be passed to {@link kceditor.editor#addFeature} (shorthand for {@link kceditor.filter#addFeature}).
 *
 ...
 * are checked and registered with {@link kceditor.editor#addFeature}, all styles available in the
 * 'Format' and 'Styles' drop-down lists are checked and registered too and so on.
 *
 * @since 4.1.0
 * @class kceditor.feature
 * @abstract
 */
 ...
 *
 * During the feature activation this value is passed to {@link kceditor.filter#allow}.
 *
 * @property {kceditor.filter.allowedContentRules} [allowedContent=null]
 */
 ...
 *
 * During the feature validation this value is passed to {@link kceditor.filter#check}.
 *
 * If this value is not provided, a feature will be always activated.
 *
 * @property {kceditor.filter.contentRule} [requiredContent=null]
 */

/**
 * The name of the feature.
 *
 * It is used for example to identify which {@link kceditor.filter#allowedContent}
...
/**
 * Feature content forms to be registered in the {@link kceditor.editor#filter}
 * during the feature activation.
 *
 * See {@link kceditor.filter#addContentForms} for more details.
 *
 ...
 * that will be registered in the {@link kceditor.editor#filter} during the feature activation.
 *
 * See {@link kceditor.filter#addTransformations} for more details.
 *
 ...
 * another feature. For example a {@link kceditor.ui.button} often registers
 * a related command. See {@link kceditor.ui.button#toFeature}.
 *
 * This method is executed when a feature is passed to the {@link kceditor.editor#addFeature}.
 *
 * @method toFeature
 * @returns {kceditor.feature}
 */
