...
/**
 * @fileOverview Defines te {@link kceditor.tools.color} class for handling different color formats.
 */
...
( function() {
	 ...
	 *
	 * ```javascript
	 * var color = new kceditor.tools.color( 'rgb( 225, 225, 225 )' );
	 * console.log( color.getHex() ); // #FFFFFF
	 *
	 * var color = new kceditor.tools.color( 'red' );
	 * console.log( color.getHexWithAlpha() ); // #FF0000FF
	 * ```
	 * @since 4.16.0
	 * @class
	 */
	kceditor.tools.color = kceditor.tools.createClass( {

		/**
		 * Creates kceditor.tools.color class instance.
		 *
		 * @constructor
		...
		proto: {
			...
			getHexWithAlpha: function() {
				...
				var alpha = Math.round( this._.alpha * kceditor.tools.color.MAX_RGB_CHANNEL_VALUE );
			...
			getHsl: function() {
				...
				if ( this._.type === kceditor.tools.color.TYPE_HSL && isIntegerAlpha ) {
			...
			getHsla: function() {
				...
				if ( this._.type === kceditor.tools.color.TYPE_HSL ) {
					hsl = [ this._.hue, this._.saturation, this._.lightness ];
				} else {
		...
		_: {
			...
			blendAlphaColor: function( red, green, blue, alpha ) {
				...
				return kceditor.tools.array.map( [ red, green, blue ], function( color ) {
					return Math.round( kceditor.tools.color.MAX_RGB_CHANNEL_VALUE - alpha * ( kceditor.tools.color.MAX_RGB_CHANNEL_VALUE - color ) );
				} );
			},
			...
			parseInput: function( colorCode ) {
				...
				colorCode = kceditor.tools.trim( colorCode );
				...
				if ( colorChannels.type === kceditor.tools.color.TYPE_HSL ) {
			...
			/**
			 * Returns hexadecimal color value from {@link kceditor.tools.color#namedColors} based on provided color name.
			 *
			 * @private
			...
			matchStringToNamedColor: function( colorName ) {
				return kceditor.tools.color.namedColors[ colorName.toLowerCase() ] || null;
			},
			...
			extractColorChannelsFromHex: function( colorCode ) {
				...
				if ( colorCode.match( kceditor.tools.color.hex3CharsRegExp ) ) {
					colorCode = this._.hex3ToHex6( colorCode );
				}

				if ( colorCode.match( kceditor.tools.color.hex4CharsRegExp ) ) {
					colorCode = this._.hex4ToHex8( colorCode );
				}

				if ( !colorCode.match( kceditor.tools.color.hex6CharsRegExp ) && !colorCode.match( kceditor.tools.color.hex8CharsRegExp ) ) {
					return null;
				}
				...
				if ( parts[ 7 ] && parts[ 8 ] ) {
					...
					alpha = alpha / kceditor.tools.color.MAX_RGB_CHANNEL_VALUE;
					alpha = Number( alpha.toFixed( 1 ) );
				}

				return {
					type: kceditor.tools.color.TYPE_RGB,
			...
			extractColorChannelsFromRgba: function( colorCode ) {
				var channels =  this._.extractColorChannelsByPattern( colorCode, kceditor.tools.color.rgbRegExp );
				...
				var red = tryToConvertToValidIntegerValue( channels[ 0 ], kceditor.tools.color.MAX_RGB_CHANNEL_VALUE ),
					green = tryToConvertToValidIntegerValue( channels[ 1 ], kceditor.tools.color.MAX_RGB_CHANNEL_VALUE ),
					blue = tryToConvertToValidIntegerValue( channels[ 2 ], kceditor.tools.color.MAX_RGB_CHANNEL_VALUE ),
					alpha = 1;

				if ( isColorDeclaredWithAlpha ) {
					alpha = tryToConvertToValidFloatValue( channels[ 3 ], kceditor.tools.color.MAX_ALPHA_CHANNEL_VALUE );
				}
			...
			extractColorChannelsFromHsla: function( colorCode ) {
				var channels =  this._.extractColorChannelsByPattern( colorCode, kceditor.tools.color.hslRegExp );
				...
				var hue = tryToConvertToValidIntegerValue( channels[ 0 ], kceditor.tools.color.MAX_HUE_CHANNEL_VALUE ),
					saturation = tryToConvertToValidFloatValue( channels[ 1 ], kceditor.tools.color.MAX_SATURATION_LIGHTNESS_CHANNEL_VALUE ),
					lightness = tryToConvertToValidFloatValue( channels[ 2 ], kceditor.tools.color.MAX_SATURATION_LIGHTNESS_CHANNEL_VALUE ),
					alpha = 1;
				...
				var result = {
					type: kceditor.tools.color.TYPE_HSL,
					red: rgba[ 0 ],
					green: rgba[ 1 ],
					blue: rgba[ 2 ],
					alpha: rgba[ 3 ],
			...
			hex4ToHex8: function( hex4ColorCode ) {
				...
				return hex6 + kceditor.tools.repeat( hex4ColorCode[ 4 ], 2 );
			},
			...
			extractColorChannelsByPattern: function( value, regExp ) {
				...
				values = kceditor.tools.array.reduce( values, function( trimmedValues, value ) {
					var trimmedValue = kceditor.tools.trim( value );
				...
				if ( match[ 2 ] ) {
					var alpha = kceditor.tools.trim( match[ 2 ].replace( /[\/,]/, '' ) );
			 ...
			 *
			 * For red, green and blue channels range is 0 to {@link kceditor.tools.color#MAX_RGB_CHANNEL_VALUE} inclusive.
			 * For alpha channel range is 0 to {@link kceditor.tools.color#MAX_ALPHA_CHANNEL_VALUE} inclusive.
			 *
			 * @private
			 * @param {Number} red Red channel value.
			 * @param {Number} green Green channel value.
			...
			areColorChannelsValid: function( red, green, blue, alpha ) {
				return isValueWithinRange( red, 0, kceditor.tools.color.MAX_RGB_CHANNEL_VALUE ) &&
					isValueWithinRange( green, 0, kceditor.tools.color.MAX_RGB_CHANNEL_VALUE ) &&
					isValueWithinRange( blue, 0, kceditor.tools.color.MAX_RGB_CHANNEL_VALUE ) &&
					isValueWithinRange( alpha, 0, kceditor.tools.color.MAX_ALPHA_CHANNEL_VALUE );
			},
			...
			hslToRgb: function( hue, saturation, lightness ) {
				...
				var calculateValueFromConst = function( fixValue ) {
					...
					return Math.round( normalizedValue * kceditor.tools.color.MAX_RGB_CHANNEL_VALUE );
				};
			...
			rgbToHsl: function( red, green, blue ) {
				...
				var r = red / kceditor.tools.color.MAX_RGB_CHANNEL_VALUE,
					g = green / kceditor.tools.color.MAX_RGB_CHANNEL_VALUE,
					b = blue / kceditor.tools.color.MAX_RGB_CHANNEL_VALUE,
	...
	/**
	 * This list is deprecated, use {@link kceditor.tools.color#namedColors} instead.
	 *
	 * @member kceditor.tools.style.parse
	 * @private
	 * @deprecated 4.16.0
	 */
	kceditor.tools.style.parse._colors = kceditor.tools.color.namedColors;
