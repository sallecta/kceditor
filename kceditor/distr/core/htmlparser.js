		...
		parse: function( html ) {
			...
			while ( ( parts = this._.htmlPartsRegex.exec( html ) ) ) {
				...
				if ( ( tagName = parts[ 1 ] ) ) {
					...
					if ( cdata && kceditor.dtd.$cdata[ tagName ] ) {
						// Send the CDATA data.
				...
				if ( ( tagName = parts[ 3 ] ) ) {
					...
					// simply ignore them when parsing. (https://dev.kceditor.com/ticket/5224)
					if ( /="/.test( tagName ) )
						continue;
					...
					if ( attribsPart ) {
						while ( ( attribMatch = attribsRegex.exec( attribsPart ) ) ) {
							...
							if ( !attValue && emptyAttribs[ attName ] )
								...
								attribs[ attName ] = kceditor.tools.htmlDecodeAttr( attValue );
						}
					}
