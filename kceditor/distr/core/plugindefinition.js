...
/**
 * @fileOverview Defines the "virtual" {@link kceditor.pluginDefinition} class which
 ...
 * passed to the {@link kceditor.plugins#add} method.
 *
 ...
 * * {@glink guide/plugin_sdk_sample Creating a KCEditor plugin in 20 Lines of Code}
 * * {@glink guide/plugin_sdk_sample_1 Creating a Simple Plugin Tutorial}
 *
 * @class kceditor.pluginDefinition
 * @abstract
 */
 ...
 * {@link kceditor.plugins#setLang}.
 *
 * When the plugin is being loaded, the editor checks this list to see if
 * a language file in the current editor language ({@link kceditor.editor#langCode})
 ...
 * for instance code that uses the {@link kceditor}'s methods such as the {@link kceditor#addCss} method.
 *
 *		kceditor.plugins.add( 'sample', {
 *			onLoad: function() {
 *				kceditor.addCss( '.cke_some_class { ... }' );
 *			}
 *		} );
 *
 ...
 *
 *		kceditor.plugins.add( 'sample1', {
 *			beforeInit: function( editor ) {
 *				editor.foo = 'bar';
 *			}
 *		} );
 *
 *		kceditor.plugins.add( 'sample2', {
 ...
 *
 * @method beforeInit
 * @param {kceditor.editor} editor The editor instance being initialized.
 */
 ...
 * 4. The {@link kceditor.editor#pluginsLoaded} event is fired.
 *
 * **Note:** The order in which the `init` methods are called does not depend on the plugins' {@link #requires requirements}
 * or the order set in the {@link kceditor.config#plugins} option. It may be random and therefore it is
 ...
 * [Browser Compatibility](https://github.com/sallecta/kceditor/docs/kceditor4/latest/guide/dev_browsers.html)
 ...
 *			return !kceditor.env.ie || kceditor.env.edge;
 *		}
 * } );
 * ```
 *
