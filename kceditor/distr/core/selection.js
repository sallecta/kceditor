...
( function() {
	var isMSSelection = typeof window.getSelection != 'function',
		nextRev = 1,
		// https://dev.kceditor.com/ticket/13816
		fillingCharSequence = kceditor.tools.repeat( '\u200b', 7 ),
		fillingCharSequenceRegExp = new RegExp( fillingCharSequence + '( )?', 'g' ),
		isSelectingTable;

	function isWidget( element ) {
		return kceditor.plugins.widget && kceditor.plugins.widget.isDomWidget( element );
	}
	...
	function isRealTableSelection( selection, fakeSelection ) {
		...
		function isValidTableSelection( table, fakeTable, ranges, fakeRanges ) {
			var isMenuOpen = ranges.length === 1 && ranges[ 0 ].collapsed,
				...
				isInTable = isTableSelection( ranges, !!kceditor.env.webkit ) && isTableSelection( fakeRanges );
	...
	function performFakeTableSelection( ranges ) {
		...
		// Caches given ranges.
		cache.ranges = new kceditor.dom.rangeList( ranges );
		cache.type = kceditor.SELECTION_TEXT;
	...
	function checkSelectionChange() {
		...
		if ( !sel ) {
			...
			if ( !sel || sel.getType() == kceditor.SELECTION_NONE )
				return;
		}
		...
		if ( !currentPath.compare( this._.selectionPreviousPath ) ) {
			// Handle case when dialog inserts new element but parent block and path (so also focus context) does not change. (https://dev.kceditor.com/ticket/13362)
			...
			if ( ( kceditor.env.webkit || kceditor.env.gecko ) && !sameBlockParent ) {
	...
	function checkSelectionChangeTimeout() {
		...
		checkSelectionChangeTimer = kceditor.tools.setTimeout( checkSelectionChangeTimeoutExec, 200, this );
	}
	...
	function checkSelectionChangeTimeoutExec() {
		...
		if ( checkSelectionChangeTimeoutPending ) {
			...
			// with Firefox.
			kceditor.tools.setTimeout( checkSelectionChange, 0, this );
	...
	var isVisible = kceditor.dom.walker.invisible( 1 );
	...
	//	See https://dev.kceditor.com/ticket/12621.
	function mayAbsorbCaret( node ) {
		if ( isVisible( node ) )
			return true;

		if ( node.type == kceditor.NODE_ELEMENT && !node.is( kceditor.dtd.$empty ) )
			return true;
	...
	function rangeRequiresFix( range ) {
		...
		if ( !( range.root instanceof kceditor.editable ) )
			return false;
		...
		// Empty block/inline element is also affected. <span>^</span>, <p>^</p> (https://dev.kceditor.com/ticket/7222)
		// If you found this line confusing check https://dev.kceditor.com/ticket/12655.
		if ( !( previous || next ) && !( ct.type == kceditor.NODE_ELEMENT && ct.isBlockBoundary() && ct.getBogus() ) )
			return true;
	...
	// Checks if a filling char has been used, eventually removing it (https://dev.kceditor.com/ticket/1272).
	...
	function removeFillingCharSequenceNode( editable, keepSelection ) {
		...
		if ( fillingChar ) {
			...
			// subsequent dom modification, save it now for restoring. (https://dev.kceditor.com/ticket/8617)
	...
	function removeFillingCharSequenceString( str, nbspAware ) {
		if ( nbspAware ) {
			return str.replace( fillingCharSequenceRegExp, function( m, p ) {
				// https://dev.kceditor.com/ticket/10291 if filling char is followed by a space replace it with NBSP.
				return p ? '\xa0' : '';
			} );
		} else {
	...
	function hideSelection( editor, ariaLabel ) {
		var content = ariaLabel && kceditor.tools.htmlEncode( ariaLabel ) || '&nbsp;',
			style = kceditor.env.ie && kceditor.env.version < 14 ? 'display:none' : 'position:fixed;top:0;left:-1000px;width:0;height:0;overflow:hidden;',
			hiddenEl = kceditor.dom.element.createFromHtml(
		...
		editor.editable().append( hiddenEl );
			...
			// Always use real selection to avoid overriding locked one (https://dev.kceditor.com/ticket/11104#comment:13).
		...
		range.setStartAt( hiddenEl, kceditor.POSITION_AFTER_START );
		range.setEndAt( hiddenEl, kceditor.POSITION_BEFORE_END );
	...
	function getOnKeyDownListener( editor ) {
		...
		return function( evt ) {
			...
			if ( next && next.type == kceditor.NODE_ELEMENT && next.getAttribute( 'contenteditable' ) == 'false' ) {
	...
	// kceditor.dom.element which should gain fake selection.
	function getNonEditableFakeSelectionReceiver( ranges ) {
		...
		if ( ranges.length == 1 && !( range = ranges[ 0 ] ).collapsed &&
			( enclosedNode = range.getEnclosedNode() ) && enclosedNode.type == kceditor.NODE_ELEMENT ) {
			...
			// innermost element (https://dev.kceditor.com/ticket/11114).
			clone = range.clone();
			clone.shrink( kceditor.SHRINK_ELEMENT, true );

			// If shrinked range still encloses an element, check this one (shrink stops only on non-editable elements).
			if ( ( shrinkedNode = clone.getEnclosedNode() ) && shrinkedNode.type == kceditor.NODE_ELEMENT )
	...
	// @param {kceditor.dom.rangeList} ranges
	function extractEditableRanges( ranges ) {
		for ( var i = 0; i < ranges.length; i++ ) {
			...
			if ( range.startContainer.isReadOnly() ) {
				...
				while ( current ) {
					isElement = current.type == kceditor.NODE_ELEMENT;
			...
			walker.evaluator = function( node ) {
				if ( node.type == kceditor.NODE_ELEMENT && node.isReadOnly() ) {
					...
					if ( !( node.getPosition( walkerRange.endContainer ) & kceditor.POSITION_CONTAINS ) ) {
	...
	kceditor.on( 'instanceCreated', function( ev ) {
		var editor = ev.editor;

		editor.on( 'contentDom', function() {
			var doc = editor.document,
				outerDoc = kceditor.document,
			...
			// of the contents. (https://dev.kceditor.com/ticket/9507)
			if ( kceditor.env.gecko ) {
				editable.attachListener( editable, 'focus', function( evt ) {
					...
					if ( restoreSel !== 0 ) {
						...
						// place (at the very start of the editable). https://dev.kceditor.com/ticket/10119
			...
			editable.attachListener( editable, kceditor.env.webkit || kceditor.env.gecko ? 'focusin' : 'focus' , function() {
				...
				if ( restoreSel && ( kceditor.env.webkit || kceditor.env.gecko ) ) {
					...
					// position should be restored after focusing back editable area. (https://dev.kceditor.com/ticket/14659)
					var requiresScrollFix = editor._.previousScrollTop != null && editor._.previousScrollTop != editable.$.scrollTop;
					if ( kceditor.env.webkit && restoreSel && requiresScrollFix ) {
			...
			function saveSel() {
				lastSel = new kceditor.dom.selection( editor.getSelection() );
				lastSel.lock();
			}
			...
			if ( kceditor.env.ie || kceditor.env.gecko || isInline ) {
				...
				editable.attachListener( editable, kceditor.env.webkit || kceditor.env.gecko ? 'focusout' : 'blur', function() {
					...
					if ( kceditor.env.gecko && !isInline && isFakeOrSingleSelection ) {
						return;
					}
			...
			if ( kceditor.env.ie && !isInline ) {
				var scroll;
				editable.attachListener( editable, 'mousedown', function( evt ) {
					// IE scrolls document to top on right mousedown
					// when editor has no focus, remember this scroll
					// position and revert it before context menu opens. (https://dev.kceditor.com/ticket/5778)
				...
				// the normal behavior on old IEs. (https://dev.kceditor.com/ticket/1659, https://dev.kceditor.com/ticket/7932)
				if ( doc.$.compatMode != 'BackCompat' ) {
					if ( kceditor.env.ie7Compat || kceditor.env.ie6Compat ) {
						var textRng,
							startRng;
						...
						html.on( 'mousedown', function( evt ) {
							...
							function onSelectEnd() {
								...
								// Make it in effect on mouse up. (https://dev.kceditor.com/ticket/9022)
								textRng.select();
							}
					...
					if ( kceditor.env.version > 7 && kceditor.env.version < 11 ) {
						html.on( 'mousedown', function( evt ) {
							if ( evt.data.getTarget().is( 'html' ) ) {
								// Limit the text selection mouse move inside of editable. (https://dev.kceditor.com/ticket/9715)
								outerDoc.on( 'mouseup', onSelectEnd );
								html.on( 'mouseup', onSelectEnd );
							}
						} );
			...
			if ( kceditor.env.ie ) {
				// https://dev.kceditor.com/ticket/14407 - Don't even let anything happen if the selection is in a non-editable element.
			...
			// https://dev.kceditor.com/ticket/9699: On Webkit&Gecko in inline editor we have to check selection when it was changed
			// by dragging and releasing mouse button outside editable. Dragging (mousedown)
			// has to be initialized in editable, but for mouseup we listen on document element.
			if ( isInline && ( kceditor.env.webkit || kceditor.env.gecko ) ) {
				var mouseDown;
				editable.attachListener( editable, 'mousedown', function() {
					mouseDown = 1;
				} );
			...
			// In all other cases listen on simple mouseup over editable, as we did before https://dev.kceditor.com/ticket/9699.
			//
			...
			// of the editor (https://dev.kceditor.com/ticket/9851).
			else {
				editable.attachListener( kceditor.env.ie ? editable : doc.getDocumentElement(), 'mouseup', checkSelectionChangeTimeout, editor );
			}
			...
			function moveRangeToPoint( range, x, y ) {
				// Error prune in IE7. (https://dev.kceditor.com/ticket/9034, https://dev.kceditor.com/ticket/9110)
				try {
			...
			function onSelectEnd() {
				...
				var sel = kceditor.document.$.selection,
					range = sel.createRange();

				// The selection range is reported on host, but actually it should applies to the content doc.
				// The parentElement may be null for read only mode in IE10 and below (https://dev.kceditor.com/ticket/9780).
			...
			function getNonEditableAscendant( sel ) {
				...
				var ascendant = range.startContainer.getAscendant( function( parent ) {
					return parent.type == kceditor.NODE_ELEMENT && parent.hasAttribute( 'contenteditable' );
				}, true );
		...
		editor.on( 'setData', function() {
			...
			// (https://dev.kceditor.com/ticket/9521, https://dev.kceditor.com/ticket/5217#comment:32 and https://dev.kceditor.com/ticket/11500#comment:11)
		...
		// IE9 might cease to work if there's an object selection inside the iframe (https://dev.kceditor.com/ticket/7639).
		if ( kceditor.env.ie9Compat )
		...
		editor.on( 'loadSnapshot', function() {
			var isElement = kceditor.dom.walker.nodeType( kceditor.NODE_ELEMENT ),
				// TODO replace with el.find() which will be introduced in https://dev.kceditor.com/ticket/9764,
	...
	// (https://dev.kceditor.com/ticket/1272). Here we set the events that should invalidate that char.
	if ( kceditor.env.webkit ) {
		kceditor.on( 'instanceReady', function( evt ) {
			var editor = evt.editor;
			...
			// Note: kceditor.dom.range.createBookmark2() normalizes snapshot's
			// bookmarks to anticipate the removal of FCSeq from the snapshot's HTML (https://dev.kceditor.com/ticket/13816).
			...
			// which should be pruned by the dataProcessor (https://dev.kceditor.com/ticket/13816).
	 ...
	 * the {@link kceditor.editor#event-selectionChange} event.
	 *
	 * @method
	 ...
	 */
	kceditor.editor.prototype.selectionChange = function( checkNow ) {
	 ...
	 * this method will return a null value unless the {@link kceditor.editor#lockSelection}
	 * has been called beforehand so the saved selection is retrieved.
	 *
	 *		var selection = kceditor.instances.editor1.getSelection();
	 *		alert( selection.getType() );
	 *
	 * @method
	 * @member kceditor.editor
	 * @param {Boolean} forceRealSelection Return real selection, instead of saved or fake one.
	 * @returns {kceditor.dom.selection/null} A selection object or null if not available for the moment.
	 */
	kceditor.editor.prototype.getSelection = function( forceRealSelection ) {
		...
		return editable && this.mode == 'wysiwyg' && this.status !== 'recreating' ? new kceditor.dom.selection( editable ) : null;
	};

	/**
	 * Retrieves the {@link kceditor.dom.range} instances that represent the current selection.
	 *
	 * **Note:** This function is an alias for the {@link kceditor.dom.selection#getRanges} method.
	 *
	 * @method
	 * @since 4.14.0
	 * @member kceditor.editor
	 ...
	 */
	kceditor.editor.prototype.getSelectedRanges = function( onlyEditables ) {
	 ...
	 * {@link kceditor.editor#unlockSelection} method.
	 *
	 * @method
	 * @member kceditor.editor
	 * @param {kceditor.dom.selection} [sel] Specify the selection to be locked.
	 * @returns {Boolean} `true` if selection was locked.
	 */
	kceditor.editor.prototype.lockSelection = function( sel ) {
		sel = sel || this.getSelection( 1 );
		if ( sel.getType() != kceditor.SELECTION_NONE ) {
	 ...
	 * {@link kceditor.editor#lockSelection} method. An unlocked selection
	 ...
	 * {@link kceditor.dom.selection#lock} method.
	 */
	kceditor.editor.prototype.unlockSelection = function( restore ) {
	 ...
	 * @todo
	 */
	kceditor.editor.prototype.forceNextSelectionCheck = function() {
	 ...
	 *
	 *		var selection = kceditor.instances.editor1.document.getSelection();
	 *		alert( selection.getType() );
	 *
	 * @method
	 * @member kceditor.dom.document
	 * @returns {kceditor.dom.selection} A selection object.
	 */
	kceditor.dom.document.prototype.getSelection = function() {
		return new kceditor.dom.selection( this );
	};

	/**
	 * Select this range as the only one with {@link kceditor.dom.selection#selectRanges}.
	 *
	 * @method
	 * @returns {kceditor.dom.selection}
	 * @member kceditor.dom.range
	 */
	kceditor.dom.range.prototype.select = function() {
		var sel = this.root instanceof kceditor.editable ? this.root.editor.getSelection() : new kceditor.dom.selection( this.root );
	 ...
	 * No selection.
	 *
	 *		if ( editor.getSelection().getType() == kceditor.SELECTION_NONE )
	 ...
	 * @member kceditor
	 */
	kceditor.SELECTION_NONE = 1;

	/**
	 * A text or a collapsed selection.
	 *
	 *		if ( editor.getSelection().getType() == kceditor.SELECTION_TEXT )
	 ...
	 * @member kceditor
	 */
	kceditor.SELECTION_TEXT = 2;

	/**
	 * Element selection.
	 *
	 *		if ( editor.getSelection().getType() == kceditor.SELECTION_ELEMENT )
	 ...
	 *		var sel = editor.getSelection();
	 *
	 * See {@link kceditor.editor#getSelection}.
	 *
	 * @class
	 * @constructor Creates a selection class instance.
	 *
	 *		// Selection scoped in document.
	 *		var sel = new kceditor.dom.selection( kceditor.document );
	 *
	 *		// Selection scoped in element with 'editable' id.
	 *		var sel = new kceditor.dom.selection( kceditor.document.getById( 'editable' ) );
	 *
	 *		// Cloning selection.
	 *		var clone = new kceditor.dom.selection( sel );
	 *
	 * @param {kceditor.dom.document/kceditor.dom.element/kceditor.dom.selection} target
	 ...
	 *
	 * If {@link kceditor.dom.selection} is passed, then its clone will be created.
	 */
	kceditor.dom.selection = function( target ) {
		// Target is a selection - clone it.
		if ( target instanceof kceditor.dom.selection ) {
			var selection = target;
			target = target.root;
		}

		var isElement = target instanceof kceditor.dom.element,
			root;

		this.rev = selection ? selection.rev : nextRev++;
		this.document = target instanceof kceditor.dom.document ? target : target.getDocument();
		...
		// Clone selection.
		if ( selection ) {
			kceditor.tools.extend( this._.cache, selection._.cache );
		...
		if ( nativeSel ) {
			if ( nativeSel.getRangeAt ) {
				...
				rangeParent = range && new kceditor.dom.node( range.commonAncestorContainer );
			}
			// For old IEs.
			else {
				...
				rangeParent = range && kceditor.dom.element.get( range.item && range.item( 0 ) || range.parentElement() );
			}
		}

		// Selection out of concerned range, empty the selection.
		// TODO check whether this condition cannot be reverted to its old
		// form (commented out) after we closed https://dev.kceditor.com/ticket/10438.
		//if ( !( rangeParent && ( root.equals( rangeParent ) || root.contains( rangeParent ) ) ) ) {
		if ( !(
			rangeParent &&
			( rangeParent.type == kceditor.NODE_ELEMENT || rangeParent.type == kceditor.NODE_TEXT ) &&
			( this.root.equals( rangeParent ) || this.root.contains( rangeParent ) )
		) ) {

			this._.cache.type = kceditor.SELECTION_NONE;
			this._.cache.startElement = null;
			this._.cache.selectedElement = null;
			this._.cache.selectedText = '';
			this._.cache.ranges = new kceditor.dom.rangeList();
		}
	...
	kceditor.tools.extend( kceditor.dom.selection, {
	...
	kceditor.dom.selection.prototype = {
		/**
		 ...
		 * available:
		 *
		 * * {@link kceditor#SELECTION_NONE} (1): No selection.
		 * * {@link kceditor#SELECTION_TEXT} (2): A text or a collapsed selection is selected.
		 * * {@link kceditor#SELECTION_ELEMENT} (3): An element is selected.
		 *
		 * Example:
		 *
		 *		if ( editor.getSelection().getType() == kceditor.SELECTION_TEXT )
		 ...
		 *
		 * @method
		 * @returns {Number} One of the following constant values: {@link kceditor#SELECTION_NONE},
		 * {@link kceditor#SELECTION_TEXT} or {@link kceditor#SELECTION_ELEMENT}.
		 */
		...
		function() {
			...
			var type = kceditor.SELECTION_NONE;
			...
			try {
				...
				if ( ieType == 'Text' )
					type = kceditor.SELECTION_TEXT;

				if ( ieType == 'Control' )
					type = kceditor.SELECTION_ELEMENT;
			...
			var type = kceditor.SELECTION_TEXT;

			var sel = this.getNative();

			if ( !( sel && sel.rangeCount ) )
				type = kceditor.SELECTION_NONE;
		...
		getRanges: ( function() {
			var func = isMSSelection ? ( function() {
				function getNodeIndex( node ) {
					return new kceditor.dom.node( node ).getIndex();
				}
				...
				var getBoundaryInformation = function( range, start ) {
					...
					// or to the right hand of test range are all text nodes. (https://dev.kceditor.com/ticket/6992)
					if ( index == -1 || index == siblings.length - 1 && position < 0 ) {
						...
						// breaking character counting logic below. (https://dev.kceditor.com/ticket/3949)
						...
						if ( !distance ) {
							...
							if ( child.nodeType != kceditor.NODE_TEXT )
						...
						while ( distance > 0 && i > 0 ) {
							sibling = siblings[ --i ];
							if ( sibling.nodeType == kceditor.NODE_TEXT ) {
								container = sibling;
					...
					// Test range was one offset beyond OR behind the anchored text node.
						...
						while ( distance > 0 ) {
							...
							// Measurement in IE could be sometimes wrong because of <select> element. (https://dev.kceditor.com/ticket/4611)
							catch ( e ) {
								return { container: parent, offset: getNodeIndex( child ) };
							}
						}
				...
				return function() {
					...
					if ( type == kceditor.SELECTION_TEXT ) {
						range = new kceditor.dom.range( this.root );

						var boundaryInfo = getBoundaryInformation( nativeRange, true );
						range.setStart( new kceditor.dom.node( boundaryInfo.container ), boundaryInfo.offset );

						boundaryInfo = getBoundaryInformation( nativeRange );
						range.setEnd( new kceditor.dom.node( boundaryInfo.container ), boundaryInfo.offset );

						// Correct an invalid IE range case on empty list item. (https://dev.kceditor.com/ticket/5850)
						if ( range.endContainer.getPosition( range.startContainer ) & kceditor.POSITION_PRECEDING && range.endOffset <= range.startContainer.getIndex() )
							range.collapse();

						return [ range ];
					} else if ( type == kceditor.SELECTION_ELEMENT ) {
						var retval = [];
						...
						for ( var i = 0; i < nativeRange.length; i++ ) {
							...
							range.setStart( new kceditor.dom.node( parentElement ), j );
							range.setEnd( new kceditor.dom.node( parentElement ), j + 1 );
							retval.push( range );
						}
			...
			function() {
				...
				// transform the native ranges in kceditor.dom.range
				// instances.
				...
				for ( var i = 0; i < sel.rangeCount; i++ ) {
					...
					range.setStart( new kceditor.dom.node( nativeRange.startContainer ), nativeRange.startOffset );
					range.setEnd( new kceditor.dom.node( nativeRange.endContainer ), nativeRange.endOffset );
					ranges.push( range );
				}
				return ranges;
			};
			...
			return function( onlyEditables ) {
				...
				if ( !ranges )
					cache.ranges = ranges = new kceditor.dom.rangeList( func.call( this ) );

				if ( !onlyEditables )
					return ranges;

				// Split range into multiple by read-only nodes.
				// Clone ranges array to avoid changing cached ranges (https://dev.kceditor.com/ticket/11493).
				return extractEditableRanges( new kceditor.dom.rangeList( ranges.slice() ) );
			};
		} )(),
		 ...
		 *
		 * @returns {kceditor.dom.element} The element at the beginning of the selection.
		 */
		getStartElement: function() {
			...
			switch ( this.getType() ) {
				case kceditor.SELECTION_ELEMENT:
					return this.getSelectedElement();

				case kceditor.SELECTION_TEXT:
					...
					if ( range ) {
						if ( !range.collapsed ) {
							...
							// visual impact. ( https://dev.kceditor.com/ticket/3231 )
							while ( 1 ) {
								var startContainer = range.startContainer,
									startOffset = range.startOffset;
								// Limit the fix only to non-block elements.(https://dev.kceditor.com/ticket/3950)
							...
							if ( !node || node.type != kceditor.NODE_ELEMENT )
								node = range.startContainer;
							else {
								var child = node.getFirst();
								while ( child && child.type == kceditor.NODE_ELEMENT ) {
									node = child;
			...
			return cache.startElement = ( node ? new kceditor.dom.element( node ) : null );
		},
		 ...
		 *		alert( element.getName() );
		 *
		 * @returns {kceditor.dom.element/null} The selected element. `null` if no
		 * selection is available or the selection type is not {@link kceditor#SELECTION_ELEMENT}.
		 */
		getSelectedElement: function() {
			...
			var node = kceditor.tools.tryThese(
				...
				function() {
					...
					for ( var i = 2; i && !( ( enclosed = range.getEnclosedNode() ) && ( enclosed.type == kceditor.NODE_ELEMENT ) && styleObjectElements[ enclosed.getName() ] && ( selected = enclosed ) ); i-- ) {
						// Then check any deep wrapped element, e.g. [<b><i><img /></i></b>]
						range.shrink( kceditor.SHRINK_ELEMENT );
					}
			...
			return cache.selectedElement = ( node ? new kceditor.dom.element( node ) : null );
		},
		...
		reset: function() {
			...
			if ( editor && editor._.fakeSelection ) {
				...
				if ( this.rev == editor._.fakeSelection.rev ) {
					...
					kceditor.warn( 'selection-fake-reset' );
				}
			}
		...
		/**
		 * Makes the current selection of type {@link kceditor#SELECTION_ELEMENT} by enclosing the specified element.
		 *
		 *		var element = editor.document.getById( 'sampleElement' );
		 *		editor.getSelection().selectElement( element );
		 *
		 * @param {kceditor.dom.element} element The element to enclose in the selection.
		 */
		selectElement: function( element ) {
			var range = new kceditor.dom.range( this.root );
		 ...
		 *		range.moveToPosition( range.root, kceditor.POSITION_BEFORE_END );
		 *		editor.getSelection().selectRanges( [ ranges ] );
		 *
		 * @param {Array} ranges An array of {@link kceditor.dom.range} instances
		...
		selectRanges: function( ranges ) {
			...
			// may need new endOffset (one element less within the range) (https://dev.kceditor.com/ticket/11021 https://dev.kceditor.com/ticket/11393).
			...
			if ( this.isLocked ) {
				...
				var focused = kceditor.document.getActive();
				this.unlock();
				this.selectRanges( ranges );
				this.lock();
			...
			if ( isMSSelection ) {
				var notWhitespaces = kceditor.dom.walker.whitespaces( true ),
					fillerTextRegex = /\ufeff|\u00a0/,
				...
				if ( selected && selected.type == kceditor.NODE_ELEMENT && selected.getName() in styleObjectElements &&
				...
				if ( range.startContainer.type == kceditor.NODE_ELEMENT && range.startContainer.getName() in nonCells ||
					range.endContainer.type == kceditor.NODE_ELEMENT && range.endContainer.getName() in nonCells ) {
					range.shrink( kceditor.NODE_ELEMENT, true );
					// The range might get collapsed (https://dev.kceditor.com/ticket/7975). Update cached variable.
				...
				if ( endNode ) {
					...
					// inline elements, like <b></b> (https://dev.kceditor.com/ticket/253).
					// It is also needed when placing the selection right after an inline
					// element to avoid the selection moving inside of it.
					dummySpan = range.document.createElement( 'span' );
					dummySpan.setHtml( '&#65279;' ); // Zero Width No-Break Space (U+FEFF). See https://dev.kceditor.com/ticket/1359.
					...
					if ( isStartMarkerAlone ) {
						...
						// selection.
						// \ufeff = Zero Width No-Break Space (U+FEFF). (https://dev.kceditor.com/ticket/1359)
				...
				if ( collapsed ) {
					...
					range.moveToPosition( dummySpan, kceditor.POSITION_BEFORE_START );
					dummySpan.remove();
				} else {
				...
				// getNative() returns null if iframe is "display:none" in FF. (https://dev.kceditor.com/ticket/6577)
				if ( !sel )
					return;
				...
				for ( var i = 0; i < ranges.length; i++ ) {
					...
					if ( i < ranges.length - 1 ) {
						...
						if ( !between.collapsed ) {
							between.shrink( kceditor.NODE_ELEMENT, true );
							var ancestor = between.getCommonAncestor(),
								enclosed = between.getEnclosedNode();

							// The following cases has to be considered:
							// 1. <span contenteditable="false">[placeholder]</span>
							// 2. <input contenteditable="false"  type="radio"/> (https://dev.kceditor.com/ticket/6621)
					...
					if ( range.collapsed && kceditor.env.webkit && rangeRequiresFix( range ) ) {
						// Append a zero-width space so WebKit will not try to
						// move the selection by itself (https://dev.kceditor.com/ticket/1272).
						...
						if ( next && !fillingChar.getPrevious() && next.type == kceditor.NODE_ELEMENT && next.getName() == 'br' ) {
							removeFillingCharSequenceNode( this.root );
							range.moveToPosition( next, kceditor.POSITION_BEFORE_START );
						} else {
							range.moveToPosition( fillingChar, kceditor.POSITION_AFTER_END );
						}
					}
		 ...
		 *
		 * @param {kceditor.dom.element} element The element to be "selected".
		 * @param {String} [ariaLabel] A string to be used by the screen reader to describe the selection.
		 */
		fake: function( element, ariaLabel ) {
			var editor = this.root.editor;

			// Attempt to retrieve aria-label if possible (https://dev.kceditor.com/ticket/14539).
			...
			range.setStartBefore( element );
			range.setEndAfter( element );
			cache.ranges = new kceditor.dom.rangeList( range );

			// Put this element in the cache.
			cache.selectedElement = cache.startElement = element;
			cache.type = kceditor.SELECTION_ELEMENT;
		...
		isHidden: function() {
			...
			if ( el && el.type == kceditor.NODE_TEXT )
				el = el.getParent();
		 ...
		 * by calling the {@link kceditor.dom.range#createBookmark} method,
		...
		/**
		 * Creates a bookmark for each range of this selection (from {@link #getRanges})
		 * by calling the {@link kceditor.dom.range#createBookmark2} method,
		 * with extra care taken to avoid interference among those ranges. The arguments
		 * received are the same as with the underlying range method.
		 *
		 ...
		 * @returns {kceditor.dom.selection} This selection object, after the ranges were selected.
		 */
		selectBookmarks: function( bookmarks ) {
			...
			if ( bookmarks.isFake ) {
				...
				if ( !node || node.type != kceditor.NODE_ELEMENT ) {
					kceditor.warn( 'selection-not-fake' );
					bookmarks.isFake = 0;
				}
			}
		 ...
		 *
		 * @returns {kceditor.dom.element} The common ancestor of the selection or `null` if selection is empty.
		 */
		...
		scrollIntoView: function() {
			...
			if ( this.getType() != kceditor.SELECTION_NONE ) {
		...
		removeAllRanges: function() {
			// Don't clear selection outside this selection's root (https://dev.kceditor.com/ticket/11500).
			if ( this.getType() == kceditor.SELECTION_NONE )
				return;
 ...
 * (the {@link kceditor.editor#elementPath elements path} holding selection's start) change.
 *
 * @event selectionChange
 * @member kceditor.editor
 * @param {kceditor.editor} editor This editor instance.
 * @param data
 * @param {kceditor.dom.selection} data.selection
 * @param {kceditor.dom.elementPath} data.path
 */
 ...
 *
 * @readonly
 * @property {kceditor.dom.element} root
 */
