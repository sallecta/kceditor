...
( function() {
	...
	function iterator( range ) {
		...
		// (https://dev.kceditor.com/ticket/3730).
		/**
		 * Whether to include `<br>` elements in the enlarged range. Should be
		 * set to `false` when using the iterator in the {@link kceditor#ENTER_BR} mode.
		 *
	...
	var beginWhitespaceRegex = /^[\r\n\t ]+$/,
		// Ignore bookmark nodes.(https://dev.kceditor.com/ticket/3783)
		bookmarkGuard = kceditor.dom.walker.bookmark( false, true ),
		whitespacesGuard = kceditor.dom.walker.whitespaces( true ),
	...
	iterator.prototype = {
		...
		getNextParagraph: function( blockTag ) {
			...
			while ( currentNode ) {
				...
				var includeNode = ( currentNode.type != kceditor.NODE_ELEMENT ),
				...
				if ( !includeNode ) {
					...
					if ( kceditor.dtd.$block[ nodeName ] && currentNode.getAttribute( 'contenteditable' ) == 'false' ) {
						block = currentNode;
						...
						// including possibly skipped empty spaces. (https://dev.kceditor.com/ticket/1603)
						if ( range ) {
							range.setEndAt( currentNode, kceditor.POSITION_BEFORE_START );

							// The found boundary must be set as the next one at this
							// point. (https://dev.kceditor.com/ticket/1717)
							if ( nodeName != 'br' ) {
						...
						if ( currentNode.getFirst() ) {
							...
							if ( !range ) {
								...
								range.setStartAt( currentNode, kceditor.POSITION_BEFORE_START );
							}
				...
				} else if ( currentNode.type == kceditor.NODE_TEXT ) {
				...
				if ( range && !closeRange ) {
					while ( !currentNode.getNext( skipGuard ) && !isLast ) {
						...
						if ( parentNode.isBlockBoundary( this.forceBrBreak && !parentPre && { br: 1 } ) ) {
							...
							// Make sure range includes bookmarks at the end of the block. (https://dev.kceditor.com/ticket/7359)
							range.setEndAt( parentNode, kceditor.POSITION_BEFORE_END );
							break;
						}
				...
				if ( includeNode )
					range.setEndAt( currentNode, kceditor.POSITION_AFTER_END );
			...
			if ( !block ) {
				...
				var startPath = new kceditor.dom.elementPath( range.startContainer, range.root );
			...
			if ( removePreviousBr ) {
				...
				if ( previousSibling && previousSibling.type == kceditor.NODE_ELEMENT ) {
			...
			if ( removeLastBr ) {
				...
				if ( lastChild && lastChild.type == kceditor.NODE_ELEMENT && lastChild.getName() == 'br' ) {
					// Remove br filler on browser which do not need it.
					if ( !kceditor.env.needsBrFiller || lastChild.getPrevious( bookmarkGuard ) || lastChild.getNext( bookmarkGuard ) )
						lastChild.remove();
				}
			}
		 ...
		 * {@link #range}'s {@link kceditor.dom.range#root root} is reached. Bookmarks are skipped.
		 *
		 * @since 4.4.6
		 * @private
		 * @param {kceditor.dom.node} node
		 * @param {Boolean} startFromSibling
		 * @param {kceditor.dom.node} lastNode
		...
		_getNextSourceNode: function( node, startFromSibling, lastNode ) {
			...
			// https://dev.kceditor.com/ticket/12484
	...
	function startIterator() {
		var range = this.range.clone(),
			...
			// (https://dev.kceditor.com/ticket/12178)
			...
			// By checking if range is collapsed we also prevent https://dev.kceditor.com/ticket/12308.
		...
		// Shrink the range to exclude harmful "noises" (https://dev.kceditor.com/ticket/4087, https://dev.kceditor.com/ticket/4450, https://dev.kceditor.com/ticket/5435).
		range.shrink( kceditor.SHRINK_ELEMENT, true );

		if ( startAtInnerBoundary )
			range.setStartAt( startPath.block, kceditor.POSITION_BEFORE_END );
		if ( endAtInnerBoundary )
			range.setEndAt( endPath.block, kceditor.POSITION_AFTER_START );

		touchPre = range.endContainer.hasAscendant( 'pre', true ) || range.startContainer.hasAscendant( 'pre', true );

		range.enlarge( this.forceBrBreak && !touchPre || !this.enlargeBr ? kceditor.ENLARGE_LIST_ITEM_CONTENTS : kceditor.ENLARGE_BLOCK_CONTENTS );

		if ( !range.collapsed ) {
			var walker = new kceditor.dom.walker( range.clone() ),
				ignoreBookmarkTextEvaluator = kceditor.dom.walker.bookmark( true, true );
			// Avoid anchor inside bookmark inner text.
			walker.evaluator = ignoreBookmarkTextEvaluator;
			this._.nextNode = walker.next();
			// TODO: It's better to have walker.reset() used here.
			walker = new kceditor.dom.walker( range.clone() );
			walker.evaluator = ignoreBookmarkTextEvaluator;
			...
			// next block.(https://dev.kceditor.com/ticket/3887)
			if ( this._.lastNode && this._.lastNode.type == kceditor.NODE_TEXT && !kceditor.tools.trim( this._.lastNode.getText() ) && this._.lastNode.getParent().isBlockBoundary() ) {
				var testRange = this.range.clone();
				testRange.moveToPosition( this._.lastNode, kceditor.POSITION_AFTER_END );
				if ( testRange.checkEndOfBlock() ) {
					var path = new kceditor.dom.elementPath( testRange.endContainer, testRange.root ),
	...
	// @param {kceditor.dom.element[]} [remainingEditables]
	...
	function startNestedEditableIterator( parentIterator, blockTag, editablesContainer, remainingEditables ) {
		...
		var filter = kceditor.filter.instances[ editable.element.data( 'cke-filter' ) ];
		...
		var range = new kceditor.dom.range( editable.element );
	...
	function rangeAtInnerBlockBoundary( range, block, checkEnd ) {
		...
		return testRange.checkBoundaryOfElement( block, checkEnd ? kceditor.START : kceditor.END );
	}

	/**
	 * Creates a {@link kceditor.dom.iterator} instance for this range.
	 *
	 * @member kceditor.dom.range
	 * @returns {kceditor.dom.iterator}
	 */
	kceditor.dom.range.prototype.createIterator = function() {
