 ...
 *		var range = new kceditor.dom.range( editor.document );
 *		range.selectNodeContents( editor.document.getBody() );
 *		// Delete the contents.
 *		range.deleteContents();
 *
 * Usually you will want to work on a ranges rooted in the editor's {@link kceditor.editable editable}
 * element. Such ranges can be created with a shorthand method &ndash; {@link kceditor.editor#createRange editor.createRange}.
 *
 ...
 * the {@link kceditor.editor#createRange} method is highly recommended.
 *
 * ### Selection
 *
 * Range is only a logical representation of a piece of content in a DOM. It should not
 * be confused with a {@link kceditor.dom.selection selection} which represents "physically
 ...
 *
 * The editor selection may be retrieved using the {@link kceditor.editor#getSelection} method:
 *
 *		var sel = editor.getSelection(),
 *			ranges = sel.getRanges(); // kceditor.dom.rangeList instance.
 *
 *		var range = ranges[ 0 ];
 ...
 *
 * @class
 * @constructor Creates a {@link kceditor.dom.range} instance that can be used inside a specific DOM Document.
 * @param {kceditor.dom.document/kceditor.dom.element} root The document or element
 ...
 */
kceditor.dom.range = function( root ) {
	/**
	 ...
	 *
	 * @readonly
	 * @property {kceditor.dom.element/kceditor.dom.text}
	 */
	...
	var isDocRoot = root instanceof kceditor.dom.document;
	/**
...
( function() {
	...
	function execContentsAction( range, action, docFrag, mergeThen, cloneId ) {
		...
		if ( isClone &&
			endNode.type == kceditor.NODE_TEXT &&
			( startNode.equals( endNode ) || ( startNode.type === kceditor.NODE_ELEMENT && startNode.getFirst().equals( endNode ) ) ) ) {
		...
		if ( endNode.type == kceditor.NODE_TEXT ) {
			// If Extract or Delete we can split the text node,
			// but if Clone (2), then we cannot modify the DOM (https://dev.kceditor.com/ticket/11586) so we mark the text node for cloning.
			if ( !isClone ) {
		...
		if ( startNode.type == kceditor.NODE_TEXT ) {
			// If Extract or Delete we can split the text node,
			// but if Clone (2), then we cannot modify the DOM (https://dev.kceditor.com/ticket/11586) so we mark
		...
		for ( level = minLevel; level <= maxLevelRight; level++ ) {
			...
			if ( !rightNode.equals( startParents[ level ] ) ) {
				...
				// that this will be the only child on this level. (https://dev.kceditor.com/ticket/13568)
		...
		function mergeAndUpdate() {
			...
			if ( commonLevel < ( maxLevelLeft - 1 ) || commonLevel < ( maxLevelRight - 1 ) || boundariesInEmptyNode ) {
				if ( boundariesInEmptyNode ) {
					range.moveToPosition( endNode, kceditor.POSITION_BEFORE_START );
				} else if ( ( maxLevelRight == commonLevel + 1 ) && cloneEndNode ) {
					// The maxLevelRight + 1 element could be already removed so we use the fact that
					// we know that it was the last element in its parent.
					range.moveToPosition( endParents[ commonLevel ], kceditor.POSITION_BEFORE_END );
				} else {
					range.moveToPosition( endParents[ commonLevel + 1 ], kceditor.POSITION_BEFORE_START );
				}
				...
				if ( mergeThen ) {
					...
					if ( topLeft && topLeft.type == kceditor.NODE_ELEMENT ) {
						var span = kceditor.dom.element.createFromHtml( '<span ' +
	...
	function getCheckStartEndBlockEvalFunction() {
		var skipBogus = false,
			whitespaces = kceditor.dom.walker.whitespaces(),
			bookmarkEvaluator = kceditor.dom.walker.bookmark( true ),
		...
		return function( node ) {
			...
			if ( node.type == kceditor.NODE_TEXT &&
				( node.hasAscendant( 'pre' ) ||
					kceditor.tools.trim( node.getText() ).length ) ) {
				return false;
			}

			// If there are non-empty inline elements (e.g. <img />), then we're not
			// at the start.
			if ( node.type == kceditor.NODE_ELEMENT && !node.is( inlineChildReqElements ) )
				return false;
	...
	var isBogus = kceditor.dom.walker.bogus(),
		nbspRegExp = /^[\t\r\n ]*(?:&nbsp;|\xa0)$/,
		editableEval = kceditor.dom.walker.editable(),
		notIgnoredEval = kceditor.dom.walker.ignored( true );

	// Evaluator for kceditor.dom.element::checkBoundaryOfElement, reject any
	// text node and non-empty elements unless it's being bookmark text.
	function elementBoundaryEval( checkStart ) {
		var whitespaces = kceditor.dom.walker.whitespaces(),
			bookmark = kceditor.dom.walker.bookmark( 1 );
		...
		return function( node ) {
			...
			// OR it's spaces.
			// Reject any element unless it's being invisible empty. (https://dev.kceditor.com/ticket/3883)
			return !checkStart && isBogus( node ) ||
				node.type == kceditor.NODE_ELEMENT &&
				node.is( kceditor.dtd.$removeEmpty );
		};
	}
	...
	kceditor.dom.range.prototype = {
		/**
		 * Clones this range.
		 *
		 * @returns {kceditor.dom.range}
		 */
		clone: function() {
			var clone = new kceditor.dom.range( this.root );
		 ...
		 * @returns {kceditor.dom.documentFragment} Document fragment containing a clone of range's content.
		 */
		cloneContents: function( cloneId ) {
			var docFrag = new kceditor.dom.documentFragment( this.document );
		 ...
		 * @returns {kceditor.dom.documentFragment} Document fragment containing extracted content.
		 */
		 ...
		 *
		 * @since 4.13.0
		 * @param {kceditor.dom.range} range A range to be compared with this range.
		 ...
		 * @returns {kceditor.dom.node/String} return.startNode Node or element ID.
		 * @returns {kceditor.dom.node/String} return.endNode Node or element ID.
		...
		createBookmark: function( serializable ) {
			...
			if ( serializable ) {
				baseId = 'cke_bm_' + kceditor.tools.getNextNumber();
			...
			if ( !collapsed ) {
				...
				if ( isTemporary( endContainer ) ) {
					...
					clone.moveToPosition( temporary, kceditor.POSITION_AFTER_END );
				}
			...
			if ( isTemporary( startContainer ) ) {
				...
				clone.moveToPosition( temporary, kceditor.POSITION_BEFORE_START );
			}
			...
			if ( endNode ) {
				...
				this.moveToPosition( startNode, kceditor.POSITION_AFTER_END );
			}
			...
			function isClipboardBin( node ) {
				...
				return kceditor.tools.array.indexOf( ids, node.getAttribute( 'id' ) ) !== -1;
			}
		},
		 ...
		 * @returns {Array} return.start Start container's address (see {@link kceditor.dom.node#getAddress}).
		...
		createBookmark2: ( function() {
			var isNotText = kceditor.dom.walker.nodeType( kceditor.NODE_TEXT, true );
			...
			function betweenTextNodes( container, offset ) {
				...
				if ( container.type != kceditor.NODE_ELEMENT || offset === 0 || offset == container.getChildCount() )
					return 0;

				return container.getChild( offset - 1 ).type == kceditor.NODE_TEXT &&
					container.getChild( offset ).type == kceditor.NODE_TEXT;
			}
			...
			function getLengthOfPrecedingTextNodes( node ) {
				...
				while ( ( node = node.getPrevious() ) && node.type == kceditor.NODE_TEXT )
					sum += node.getText().replace( kceditor.dom.selection.FILLING_CHAR_SEQUENCE, '' ).length;
			...
			function normalizeTextNodes( limit ) {
				...
				if ( container.type == kceditor.NODE_ELEMENT && offset > 0 ) {
					offset = getPrecedingSafeNodeIndex( container, offset ) + 1;
				}

				// The last step - fix the offset inside text node by adding
				// lengths of preceding text nodes which will be merged with container.
				if ( container.type == kceditor.NODE_TEXT ) {
			...
			function normalizeFCSeq( limit, root ) {
				...
				if ( fcseq.equals( container ) ) {
					limit.offset -= kceditor.dom.selection.FILLING_CHAR_SEQUENCE.length;
		 ...
		 *
		 * @returns {Object}
		 * @returns {kceditor.dom.node} return.startNode
		 * @returns {kceditor.dom.node} return.endNode
		...
		getBoundaryNodes: function() {
			...
			if ( startNode.type == kceditor.NODE_ELEMENT ) {
				...
				// startOffset > childCount but childCount is not 0
					...
					startNode = new kceditor.dom.node( startNode );
			...
			if ( endNode.type == kceditor.NODE_ELEMENT ) {
				...
				// endOffset > childCount but childCount is not 0.
					...
					endNode = new kceditor.dom.node( endNode );
				}
			}

			// Sometimes the endNode will come right before startNode for collapsed
			// ranges. Fix it. (https://dev.kceditor.com/ticket/3780)
			if ( startNode.getPosition( endNode ) & kceditor.POSITION_FOLLOWING )
				startNode = endNode;
		 ...
		 * @param {Boolean} [ignoreTextNode=false] Whether ignore {@link kceditor#NODE_TEXT} type.
		 * @returns {kceditor.dom.element}
		 */
		getCommonAncestor: function( includeSelf, ignoreTextNode ) {
			...
			if ( start.equals( end ) ) {
				if ( includeSelf && start.type == kceditor.NODE_ELEMENT && this.startOffset == this.endOffset - 1 )
		...
		optimize: function() {
			...
			if ( container.type != kceditor.NODE_ELEMENT ) {
				if ( !offset )
		...
		optimizeBookmark: function() {
			...
			if ( startNode.is && startNode.is( 'span' ) && startNode.data( 'cke-bookmark' ) )
				this.setStartAt( startNode, kceditor.POSITION_BEFORE_START );
			if ( endNode && endNode.is && endNode.is( 'span' ) && endNode.data( 'cke-bookmark' ) )
				this.setEndAt( endNode, kceditor.POSITION_AFTER_END );
		},

		/**
		 * Change start/end container to its parent or to a new node created from container split.
		 *
		 * Works on the container if it is a {@link kceditor#NODE_TEXT text node} and the range is collapsed
		...
		trim: function( ignoreStart, ignoreEnd ) {
			...
			if ( ( !ignoreStart || collapsed ) && startContainer && startContainer.type == kceditor.NODE_TEXT ) {
				if ( !startOffset ) {
			...
			if ( !( ignoreEnd || collapsed ) && endContainer && endContainer.type == kceditor.NODE_TEXT ) {
				if ( !endOffset ) {
		 ...
		 *
		 * @param {Number} unit The unit type to expand with. Use one of following values: {@link kceditor#ENLARGE_BLOCK_CONTENTS},
		 * {@link kceditor#ENLARGE_ELEMENT}, {@link kceditor#ENLARGE_INLINE}, {@link kceditor#ENLARGE_LIST_ITEM_CONTENTS}.
		...
		enlarge: function( unit, excludeBrs ) {
			...
			switch ( unit ) {
				case kceditor.ENLARGE_INLINE:
					var enlargeInlineOnly = 1;

				/* falls through */
				case kceditor.ENLARGE_ELEMENT:
					...
					if ( container.type == kceditor.NODE_TEXT ) {
						if ( offset ) {
							...
							container = !kceditor.tools.trim( container.substring( 0, offset ) ).length && container;
					...
					while ( enlargeable || sibling ) {
						...
						while ( sibling ) {
							...
							if ( sibling.type == kceditor.NODE_COMMENT ) {
								sibling = sibling.getPrevious();
								continue;
							} else if ( sibling.type == kceditor.NODE_TEXT ) {
								...
								var offsetWidth0 = kceditor.env.webkit ? 1 : 0;

								// If this is a visible element.
								// We need to check for the bookmark attribute because IE insists on
								// rendering the display:none nodes we use for bookmarks. (https://dev.kceditor.com/ticket/3363)
								// Line-breaks (br) are rendered with zero width, which we don't want to include. (https://dev.kceditor.com/ticket/7041)
								if ( ( sibling.$.offsetWidth > offsetWidth0 || excludeBrs && sibling.is( 'br' ) ) && !sibling.data( 'cke-bookmark' ) ) {
									// We'll accept it only if we need
									// whitespace, and this is an inline
									// element with whitespace only.
									if ( needsWhiteSpace && kceditor.dtd.$removeEmpty[ sibling.getName() ] ) {
										...
										sibling = null;
											...
											for ( var i = 0, child; child = allChildren[ i++ ]; ) {
												if ( !kceditor.dtd.$removeEmpty[ child.nodeName.toLowerCase() ] ) {
													sibling = null;
													break;
												}
											}
					...
					if ( container.type == kceditor.NODE_TEXT ) {
						...
						if ( kceditor.tools.trim( container.substring( offset ) ).length ) {
							// If we found only whitespace in the node, it
							// means that we'll need more whitespace to be able
							// to expand. For example, <i> can be expanded in
							// "A <i> [B]</i>", but not in "A<i> [B]</i>".
					...
					while ( enlargeable || sibling ) {
						...
						while ( sibling ) {
							...
							if ( sibling.type == kceditor.NODE_TEXT ) {
								siblingText = sibling.getText();
								...
								isWhiteSpace = /^[\s\ufeff]/.test( siblingText );
							} else if ( sibling.type == kceditor.NODE_ELEMENT ) {
								// If this is a visible element.
								// We need to check for the bookmark attribute because IE insists on
								// rendering the display:none nodes we use for bookmarks. (https://dev.kceditor.com/ticket/3363)
								// Line-breaks (br) are rendered with zero width, which we don't want to include. (https://dev.kceditor.com/ticket/7041)
				...
				case kceditor.ENLARGE_BLOCK_CONTENTS:
				case kceditor.ENLARGE_LIST_ITEM_CONTENTS:

					// Enlarging the start boundary.
					var walkerRange = new kceditor.dom.range( this.root );

					boundary = this.root;

					walkerRange.setStartAt( boundary, kceditor.POSITION_AFTER_START );
					walkerRange.setEnd( this.startContainer, this.startOffset );

					var walker = new kceditor.dom.walker( walkerRange ),
						blockBoundary, // The node on which the enlarging should stop.
						tailBr, // In case BR as block boundary.
						notBlockBoundary = kceditor.dom.walker.blockBoundary( ( unit == kceditor.ENLARGE_LIST_ITEM_CONTENTS ) ? { br: 1 } : null ),
						inNonEditable = null,
						...
						boundaryGuard = function( node ) {
							...
							if ( node.type == kceditor.NODE_ELEMENT && node.getAttribute( 'contenteditable' ) == 'false' ) {
								if ( inNonEditable ) {
					...
					this.setStartAt( blockBoundary, !blockBoundary.is( 'br' ) && ( !enlargeable && this.checkStartOfBlock() ||
						enlargeable && blockBoundary.contains( enlargeable ) ) ? kceditor.POSITION_AFTER_START : kceditor.POSITION_AFTER_END );

					// Avoid enlarging the range further when end boundary spans right after the BR. (https://dev.kceditor.com/ticket/7490)
					if ( unit == kceditor.ENLARGE_LIST_ITEM_CONTENTS ) {
						var theRange = this.clone();
						walker = new kceditor.dom.walker( theRange );

						var whitespaces = kceditor.dom.walker.whitespaces(),
							bookmark = kceditor.dom.walker.bookmark();

						walker.evaluator = function( node ) {
							return !whitespaces( node ) && !bookmark( node );
						};
						var previous = walker.previous();
						if ( previous && previous.type == kceditor.NODE_ELEMENT && previous.is( 'br' ) )
							return;
					}
					...
					walkerRange.setEndAt( boundary, kceditor.POSITION_BEFORE_END );
					walker = new kceditor.dom.walker( walkerRange );

					// tailBrGuard only used for on range end.
					walker.guard = ( unit == kceditor.ENLARGE_LIST_ITEM_CONTENTS ) ? tailBrGuard : boundaryGuard;
					...
					this.setEndAt( blockBoundary, ( !enlargeable && this.checkEndOfBlock() ||
						enlargeable && blockBoundary.contains( enlargeable ) ) ? kceditor.POSITION_BEFORE_END : kceditor.POSITION_BEFORE_START );
			...
			// @param {kceditor.dom.element} enlargeable
			// @returns {kceditor.dom.element/null}
			function getValidEnlargeable( enlargeable ) {
				return enlargeable && enlargeable.type == kceditor.NODE_ELEMENT && enlargeable.hasAttribute( 'contenteditable' ) ?
					null : enlargeable;
			}
			...
			function onlyWhiteSpaces( startContainer, startOffset, boundary ) {
				...
				var walkerRange = new kceditor.dom.range( boundary );
				...
				walker.guard = function( node ) {
					...
					return !( node.type == kceditor.NODE_ELEMENT && node.isBlockBoundary() );
				};
		 ...
		 *
		 * @param {Number} mode The shrinking mode ({@link kceditor#SHRINK_ELEMENT} or {@link kceditor#SHRINK_TEXT}).
		 *
		 * * {@link kceditor#SHRINK_ELEMENT} &ndash; Shrinks the range boundaries to the edge of the innermost element.
		 * * {@link kceditor#SHRINK_TEXT} &ndash; Shrinks the range boundaries to anchor by the side of enclosed text
		 ...
		 * `mode` is set to {@link kceditor#SHRINK_TEXT}. This option was added in 4.7.0.
		 */
		shrink: function( mode, selectContents, options ) {
			...
			if ( !this.collapsed ) {
				mode = mode || kceditor.SHRINK_TEXT;
				...
				if ( startContainer && startContainer.type == kceditor.NODE_TEXT ) {
					if ( !startOffset )
				...
				if ( endContainer && endContainer.type == kceditor.NODE_TEXT ) {
					if ( !endOffset )
				...
				walker.evaluator = function( node ) {
					return node.type == ( mode == kceditor.SHRINK_ELEMENT ? kceditor.NODE_ELEMENT : kceditor.NODE_TEXT );
				};

				var currentElement;
				walker.guard = function( node, movingOut ) {
					// Skipping bogus before other cases (https://dev.kceditor.com/ticket/17010).
					...
					if ( mode == kceditor.SHRINK_ELEMENT && node.type == kceditor.NODE_TEXT )
						return false;
					...
					if ( shrinkOnBlockBoundary === false && node.type == kceditor.NODE_ELEMENT && node.isBlockBoundary() )
						return false;

					// Stop shrinking when encountering an editable border.
					if ( node.type == kceditor.NODE_ELEMENT && node.hasAttribute( 'contenteditable' ) )
						return false;

					if ( !movingOut && node.type == kceditor.NODE_ELEMENT )
						currentElement = node;
				...
				if ( moveStart ) {
					var textStart = walker[ mode == kceditor.SHRINK_ELEMENT ? 'lastForward' : 'next' ]();
					textStart && this.setStartAt( textStart, selectContents ? kceditor.POSITION_AFTER_START : kceditor.POSITION_BEFORE_START );
				}

				if ( moveEnd ) {
					walker.reset();
					var textEnd = walker[ mode == kceditor.SHRINK_ELEMENT ? 'lastBackward' : 'previous' ]();
					textEnd && this.setEndAt( textEnd, selectContents ? kceditor.POSITION_BEFORE_END : kceditor.POSITION_AFTER_END );
				}
		 ...
		 *		range.moveToPosition( elB, kceditor.POSITION_BEFORE_START );
		 *		// Range will be moved to: <p>Foo ^<b>bar</b></p>
		 *
		 * See also {@link #setStartAt} and {@link #setEndAt}.
		 *
		 * @param {kceditor.dom.node} node The node according to which the position will be set.
		 * @param {Number} position One of {@link kceditor#POSITION_BEFORE_START},
		 * {@link kceditor#POSITION_AFTER_START}, {@link kceditor#POSITION_BEFORE_END},
		 * {@link kceditor#POSITION_AFTER_END}.
		 */
		...
		selectNodeContents: function( node ) {
			...
			this.setEnd( node, node.type == kceditor.NODE_TEXT ? node.getLength() : node.getChildCount() );
		},

		/**
		 * Sets the start position of a range.
		 *
		 * @param {kceditor.dom.node} startNode The node to start the range.
		...
		setStart: function( startNode, startOffset ) {
			...
			if ( startNode.type == kceditor.NODE_ELEMENT && kceditor.dtd.$empty[ startNode.getName() ] )
		 ...
		 *
		 * @param {kceditor.dom.node} endNode The node to end the range.
		...
		setEnd: function( endNode, endOffset ) {
			...
			if ( endNode.type == kceditor.NODE_ELEMENT && kceditor.dtd.$empty[ endNode.getName() ] )
		 ...
		 *
		 *		// HTML: <p>Foo <b>bar</b>^</p>
		 *		range.setStartAt( elB, kceditor.POSITION_AFTER_START );
		 *		// The range will be changed to:
		 *		// <p>Foo <b>[bar</b>]</p>
		 *
		 * See also {@link #setEndAt} and {@link #moveToPosition}.
		 *
		 * @param {kceditor.dom.node} node The node according to which position will be set.
		 * @param {Number} position One of {@link kceditor#POSITION_BEFORE_START},
		 * {@link kceditor#POSITION_AFTER_START}, {@link kceditor#POSITION_BEFORE_END},
		 * {@link kceditor#POSITION_AFTER_END}.
		 */
		setStartAt: function( node, position ) {
			switch ( position ) {
				case kceditor.POSITION_AFTER_START:
					this.setStart( node, 0 );
					break;

				case kceditor.POSITION_BEFORE_END:
					if ( node.type == kceditor.NODE_TEXT )
				...
				case kceditor.POSITION_BEFORE_START:
					this.setStartBefore( node );
					break;

				case kceditor.POSITION_AFTER_END:
		 ...
		 *
		 *		// HTML: <p>^Foo <b>bar</b></p>
		 *		range.setEndAt( textBar, kceditor.POSITION_BEFORE_START );
		 *		// The range will be changed to:
		 *		// <p>[Foo <b>]bar</b></p>
		 *
		 ...
		 * For example: `'p'`.
		 * @returns {kceditor.dom.element} Created block wrapper.
		 */
		fixBlock: function( isStart, blockTag ) {
			...
			this.enlarge( kceditor.ENLARGE_BLOCK_CONTENTS );
			...
			// We remove incorrectly placed one and add a brand new one. (https://dev.kceditor.com/ticket/13001)
		...
		splitBlock: function( blockTag, cloneId ) {
			var startPath = new kceditor.dom.elementPath( this.startContainer, this.root ),
				endPath = new kceditor.dom.elementPath( this.endContainer, this.root );
			...
			if ( blockTag != 'br' ) {
				if ( !startBlock ) {
					...
					endBlock = new kceditor.dom.elementPath( this.endContainer, this.root ).block;
				}
			...
			if ( startBlock && startBlock.equals( endBlock ) ) {
				if ( isEndOfBlock ) {
					elementPath = new kceditor.dom.elementPath( this.startContainer, this.root );
					this.moveToPosition( endBlock, kceditor.POSITION_AFTER_END );
					endBlock = null;
				} else if ( isStartOfBlock ) {
					elementPath = new kceditor.dom.elementPath( this.startContainer, this.root );
					this.moveToPosition( startBlock, kceditor.POSITION_BEFORE_START );
					startBlock = null;
				} else {
		 ...
		 * @returns {kceditor.dom.element} Root element of the new branch after the split.
		 */
		splitElement: function( toSplit, cloneId ) {
			...
			// of its contents.
			this.setEndAt( toSplit, kceditor.POSITION_BEFORE_END );
			...
			this.moveToPosition( toSplit, kceditor.POSITION_AFTER_END );
			return clone;
		},
		...
		removeEmptyBlocksAtEnd: ( function() {
			...
			var whitespace = kceditor.dom.walker.whitespaces(),
					bookmark = kceditor.dom.walker.bookmark( false );
			...
			return function( atEnd ) {
				...
				while ( block && !block.equals( path.root ) &&
					...
					this[ atEnd ? 'setEndAt' : 'setStartAt' ]( block, kceditor.POSITION_AFTER_END );
					block.remove( 1 );
					block = parent;
				}
		...
		/**
		 * Gets {@link kceditor.dom.elementPath} for the {@link #startContainer}.
		 *
		 * @returns {kceditor.dom.elementPath}
		 */
		startPath: function() {
			return new kceditor.dom.elementPath( this.startContainer, this.root );
		},

		/**
		 * Gets {@link kceditor.dom.elementPath} for the {@link #endContainer}.
		 *
		 * @returns {kceditor.dom.elementPath}
		 */
		endPath: function() {
			return new kceditor.dom.elementPath( this.endContainer, this.root );
		},
		 ...
		 * element.
		 *
		 * @param {kceditor.dom.element} element The target element to check.
		 * @param {Number} checkType The boundary to check for both the range
		 * and the element. It can be {@link kceditor#START} or {@link kceditor#END}.
		 * @returns {Boolean} `true` if the range boundary is at the inner
		 * boundary of the element.
		 */
		checkBoundaryOfElement: function( element, checkType ) {
			var checkStart = ( checkType == kceditor.START );
			...
			walkerRange[ checkStart ? 'setStartAt' : 'setEndAt' ]( element, checkStart ? kceditor.POSITION_AFTER_START : kceditor.POSITION_BEFORE_END );
		...
		checkStartOfBlock: function( skipTrimming ) {
			...
			if ( kceditor.env.ie && startOffset && startContainer.type == kceditor.NODE_TEXT ) {
				var textBefore = kceditor.tools.ltrim( startContainer.substring( 0, startOffset ) );
			...
			var path = new kceditor.dom.elementPath( this.startContainer, this.root );

			// Creates a range starting at the block start until the range start.
			var walkerRange = this.clone();
			walkerRange.collapse( true );
			walkerRange.setStartAt( path.block || path.blockLimit, kceditor.POSITION_AFTER_START );
		...
		checkEndOfBlock: function( skipTrimming ) {
			...
			// we it to be isolated, for bogus check.
			if ( kceditor.env.ie && endContainer.type == kceditor.NODE_TEXT ) {
				var textAfter = kceditor.tools.rtrim( endContainer.substring( endOffset ) );
			...
			// We need to grab the block element holding the start boundary, so
			// let's use an element path for it.
			var path = new kceditor.dom.elementPath( this.endContainer, this.root );

			// Creates a range starting at the block start until the range start.
			var walkerRange = this.clone();
			walkerRange.collapse( false );
			walkerRange.setEndAt( path.block || path.blockLimit, kceditor.POSITION_BEFORE_END );
		...
		/**
		 * Traverse with {@link kceditor.dom.walker} to retrieve the previous element before the range start.
		 *
		 * @param {Function} evaluator Function used as the walker's evaluator.
		 * @param {Function} [guard] Function used as the walker's guard.
		 * @param {kceditor.dom.element} [boundary] A range ancestor element in which the traversal is limited,
		 * default to the root editable if not defined.
		 * @returns {kceditor.dom.element/null} The returned node from the traversal.
		 */
		getPreviousNode: function( evaluator, guard, boundary ) {
			var walkerRange = this.clone();
			walkerRange.collapse( 1 );
			walkerRange.setStartAt( boundary || this.root, kceditor.POSITION_AFTER_START );
		...
		/**
		 * Traverse with {@link kceditor.dom.walker} to retrieve the next element before the range start.
		 *
		 * @param {Function} evaluator Function used as the walker's evaluator.
		 * @param {Function} [guard] Function used as the walker's guard.
		 * @param {kceditor.dom.element} [boundary] A range ancestor element in which the traversal is limited,
		...
		getNextNode: function( evaluator, guard, boundary ) {
			var walkerRange = this.clone();
			walkerRange.collapse();
			walkerRange.setEndAt( boundary || this.root, kceditor.POSITION_BEFORE_END );
		 ...
		 *
		 * @param {kceditor.dom.element} el The element into which look for the
		 * editing spot.
		...
		moveToElementEditablePosition: function( el, isMoveToEnd ) {
			...
			function nextDFS( node, childOnly ) {
				...
				if ( node.type == kceditor.NODE_ELEMENT && node.isEditable( false ) )
			...
			if ( el.type == kceditor.NODE_ELEMENT && !el.isEditable( false ) ) {
				this.moveToPosition( el, isMoveToEnd ? kceditor.POSITION_AFTER_END : kceditor.POSITION_BEFORE_START );
				return true;
			}
			...
			while ( el ) {
				...
				if ( el.type == kceditor.NODE_ELEMENT ) {
					if ( el.isEditable() ) {
						this.moveToPosition( el, isMoveToEnd ? kceditor.POSITION_BEFORE_END : kceditor.POSITION_AFTER_START );
						found = 1;
					}
					...
					else if ( el.getAttribute( 'contenteditable' ) == 'false' && el.is( kceditor.dtd.$block ) ) {
		 ...
		 *
		 * @since 4.3.0
		 * @param {kceditor.dom.element} [element] The starting element. If not specified, the current range
		...
		moveToClosestEditablePosition: function( element, isMoveForward ) {
			...
			var range,
				...
				sibling,
				isElement,
				positions = [ kceditor.POSITION_AFTER_END, kceditor.POSITION_BEFORE_START ];
			...
			if ( element ) {
				...
				range = new kceditor.dom.range( this.root );
			...
			// next to it.
			if ( element && !element.is( kceditor.dtd.$block ) )
				found = 1;
			else {
				...
				if ( sibling ) {
					found = 1;
					isElement = sibling.type == kceditor.NODE_ELEMENT;
					...
					// can't handle).
					if ( isElement && sibling.is( kceditor.dtd.$block ) && sibling.getAttribute( 'contenteditable' ) == 'false' ) {
						range.setStartAt( sibling, kceditor.POSITION_BEFORE_START );
						range.setEndAt( sibling, kceditor.POSITION_AFTER_END );
					}
					// Handle empty blocks which can be selection containers on old IEs.
					else if ( !kceditor.env.needsBrFiller && isElement && sibling.is( kceditor.dom.walker.validEmptyBlockContainers ) ) {
		...
		getEnclosedNode: function() {
			...
			// Optimize and analyze the range to avoid DOM destructive nature of walker. (https://dev.kceditor.com/ticket/5780)
			walkerRange.optimize();
			if ( walkerRange.startContainer.type != kceditor.NODE_ELEMENT || walkerRange.endContainer.type != kceditor.NODE_ELEMENT )
				return null;

			var walker = new kceditor.dom.walker( walkerRange ),
				isNotBookmarks = kceditor.dom.walker.bookmark( false, true ),
				isNotWhitespaces = kceditor.dom.walker.whitespaces( true );
		...
		getTouchedStartNode: function() {
			...
			if ( this.collapsed || container.type != kceditor.NODE_ELEMENT )
				return container;
		 ...
		 *
		 * @since 4.3.0
		 * @returns {kceditor.dom.element/kceditor.dom.text}
		 */
		...
		scrollIntoView: function() {
			...
			// to this element.
			var reference = new kceditor.dom.element.createFromHtml( '<span>&nbsp;</span>', this.document ),
			...
			if ( isStartText = range.startContainer.type == kceditor.NODE_TEXT ) {
		...
		/**
		 * Returns an array of {@link kceditor.dom.rect} elements that are represented as rectangles which are covered by ranges.
		...
		getClientRects: ( function() {
			if ( document.getSelection !== undefined ) {
				return function( isAbsolute ) {
					...
					return kceditor.tools.array.map( rectList, function( item ) {
			...
			function fixWidgetsRects( rectList, context ) {
				var rectArray = kceditor.tools.array.map( rectList, function( item ) {
						return item;
					} ),
					newRange = new kceditor.dom.range( context.root ),
					widgetElements,
					widgetRects,
					widgetRange,
					documentFragment,
				...
				if ( context.startContainer instanceof kceditor.dom.element ) {
					moveStart = context.startOffset === 0 && context.startContainer.hasAttribute( 'data-widget' );
				}
				if ( context.endContainer instanceof kceditor.dom.element ) {
				...
				widgetElements = kceditor.tools.array.map( widgetElements, function( item ) {
				...
				widgetRects = kceditor.tools.array.map( widgetElements, function( element ) {
					var rects,
				...
				kceditor.tools.array.forEach( widgetRects, function( item ) {
					var found;
					cleanWidgetRects( 0 );

					function cleanWidgetRects( startIndex ) {
						kceditor.tools.array.forEach( rectArray, function( rectArrayItem, index ) {
							var compare = kceditor.tools.objectCompare( item[ startIndex ], rectArrayItem );

							if ( !compare ) {
								compare = kceditor.tools.objectCompare( item.widgetRect, rectArrayItem );
							}
			...
			function fixEmptyRectList( rectList, range, context ) {
				...
				if ( !range.collapsed ) {
					...
					rectList = range.getClientRects();
					textNode.remove();
				} else if ( context.startContainer instanceof kceditor.dom.text ) {
			...
			// Extending empty object with rect, to prevent inheriting from DOMRect, same approach as in kceditor.dom.element.getClientRect().
			function convertRect( rect, isAbsolute, context ) {
				var newRect = kceditor.tools.extend( {}, rect );

				if ( isAbsolute ) {
					newRect = kceditor.tools.getAbsoluteRectPosition( context.document.getWindow(), newRect );
				}
		 ...
		 * @since 4.4.6
		 * @private
		 * @param {kceditor.dom.element} startContainer
		 */
		_setStartContainer: function( startContainer ) {
			...
			if ( !isRootAscendantOrSelf ) {
				kceditor.warn( 'range-startcontainer', { startContainer: startContainer, root: this.root } );
			}
			// %REMOVE_END%
		 ...
		 * @since 4.4.6
		 * @private
		 * @param {kceditor.dom.element} endContainer
		 */
		_setEndContainer: function( endContainer ) {
			...
			if ( !isRootAscendantOrSelf ) {
				kceditor.warn( 'range-endcontainer', { endContainer: endContainer, root: this.root } );
			}
			// %REMOVE_END%
		...
		_find: function( query, includeNonEditables ) {
			var ancestor = this.getCommonAncestor(),
				...
				// Contrary to kceditor.dom.element#find we're returning array, that's because NodeList is immutable, and we need
			...
			if ( ancestor && ancestor.find ) {
				...
				for ( i = 0; i < initialMatches.count(); i++ ) {
					...
					isStartGood = ( curItem.getPosition( boundaries.startNode ) & kceditor.POSITION_FOLLOWING ) || boundaries.startNode.equals( curItem );
					isEndGood = ( curItem.getPosition( boundaries.endNode ) & ( kceditor.POSITION_PRECEDING + kceditor.POSITION_IS_CONTAINED ) ) || boundaries.endNode.equals( curItem );
	 ...
	 *
	 * Note that each range in the returned value will be enlarged with `kceditor.ENLARGE_ELEMENT` value.
	 *
	 * @since 4.7.0
	 * @static
	 * @param {kceditor.dom.range[]} ranges
	 * @returns {kceditor.dom.range[]} Set of merged ranges.
	 * @member kceditor.dom.range
	 */
	kceditor.dom.range.mergeRanges = function( ranges ) {
		return kceditor.tools.array.reduce( ranges, function( ret, rng ) {
			// Last range ATM.
			...
			rng = rng.clone();
			rng.enlarge( kceditor.ENLARGE_ELEMENT );
			...
			if ( lastRange ) {
				...
				var gapRange = new kceditor.dom.range( rng.root ),
					walker = new kceditor.dom.walker( gapRange ),
					isWhitespace = kceditor.dom.walker.whitespaces(),
					nodeInBetween;
 ...
 *
 * It is used as a parameter of methods like: {@link kceditor.dom.range#moveToPosition},
 * {@link kceditor.dom.range#setStartAt} and {@link kceditor.dom.range#setEndAt}.
 *
 * @readonly
 * @member kceditor
 * @property {Number} [=1]
 */
kceditor.POSITION_AFTER_START = 1;
 ...
 * @property {Number} [=2]
 */
kceditor.POSITION_BEFORE_END = 2;
 ...
 * @property {Number} [=3]
 */
kceditor.POSITION_BEFORE_START = 3;
 ...
 * @property {Number} [=2]
 */
kceditor.ENLARGE_BLOCK_CONTENTS = 2;
 ...
 * @property {Number} [=3]
 */
kceditor.ENLARGE_LIST_ITEM_CONTENTS = 3;
 ...
 * @property {Number} [=4]
 */
kceditor.ENLARGE_INLINE = 4;

// Check boundary types.

/**
 * See {@link kceditor.dom.range#checkBoundaryOfElement}.
 *
 * @readonly
 * @member kceditor
 * @property {Number} [=1]
...
/**
 * See {@link kceditor.dom.range#shrink}.
 *
 * @readonly
 * @member kceditor
 * @property {Number} [=1]
 */
kceditor.SHRINK_ELEMENT = 1;
