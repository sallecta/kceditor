...
/**
 * @fileOverview Defines the {@link kceditor.scriptLoader} object, used to load scripts
 *		asynchronously.
 */
 ...
 * @singleton
 */
kceditor.scriptLoader = ( function() {
	var uniqueScripts = {},
		waitingList = {};
	...
	return {
		 ...
		 *
		 *		kceditor.scriptLoader.load( '/myscript.js' );
		 *
		 *		kceditor.scriptLoader.load( '/myscript.js', function( success ) {
		 *			// Alerts true if the script has been properly loaded.
		 *			// HTTP error 404 should return false.
		 *			alert( success );
		 *		} );
		 *
		 *		kceditor.scriptLoader.load( [ '/myscript1.js', '/myscript2.js' ], function( completed, failed ) {
		 ...
		 * the callback call. Defaults to {@link kceditor}.
		...
		load: function( scriptUrl, callback, scope, showBusy ) {
			...
			var checkLoaded = function( url, success ) {
					...
					if ( --scriptCountDoCallback <= 0 ) {
						showBusy && kceditor.document.getDocumentElement().removeStyle( 'cursor' );
						doCallback( success );
					}
				};
			...
			var loadScript = function( url ) {
					...
					var script = new kceditor.dom.element( 'script' );
					...
					if ( callback ) {
						// The onload or onerror event does not fire in IE8 and IE9 Quirks Mode (https://dev.kceditor.com/ticket/14849).
						if ( kceditor.env.ie && ( kceditor.env.version <= 8 || kceditor.env.ie9Compat ) ) {
							...
							script.$.onload = function() {
								...
								// immediately. This will break the loading sequence. (https://dev.kceditor.com/ticket/3661)
								setTimeout( function() {
					...
					// Append it to <head>.
					script.appendTo( kceditor.document.getHead() );

					kceditor.fire( 'download', url ); // %REMOVE_LINE%

				};

			showBusy && kceditor.document.getDocumentElement().setStyle( 'cursor', 'wait' );
		 ...
		 *
		 * @see kceditor.scriptLoader#load
		 */
		queue: ( function() {
			var pending = [];
			...
			function loadNext() {
				...
				if ( ( script = pending[ 0 ] ) )
					this.load( script.scriptUrl, script.callback, kceditor, 0 );
			}
