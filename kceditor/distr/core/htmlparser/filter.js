...
( function() {
	/**
	 * Filter is a configurable tool for transforming and filtering {@link kceditor.htmlParser.node nodes}.
	 * It is mainly used during data processing phase which is done not on real DOM nodes,
	 * but on their simplified form represented by {@link kceditor.htmlParser.node} class and its subclasses.
	 *
	 *		var filter = new kceditor.htmlParser.filter( {
	 *			text: function( value ) {
	 *				return '@' + value + '@';
	 *			},
	 *			elements: {
	 ...
	 *		} );
	 *
	 *		var fragment = kceditor.htmlParser.fragment.fromHtml( '<p>Foo<b>bar!</b></p>' ),
	 ...
	 * @class
	 */
	kceditor.htmlParser.filter = kceditor.tools.createClass( {
		/**
		 * @constructor Creates a filter class instance.
		 * @param {kceditor.htmlParser.filterRulesDefinition} [rules]
		 */
		$: function( rules ) {
			/**
			 ...
			 *
			 * @property {kceditor.htmlParser.filterRulesGroup}
			 * @readonly
			 */
			...
			/**
			 * Hash of elementName => {@link kceditor.htmlParser.filterRulesGroup rules for elements}.
			 *
			 * @readonly
			 */
			this.elementsRules = {};
			...
			/**
			 * Hash of attributeName => {@link kceditor.htmlParser.filterRulesGroup rules for attributes}.
			 *
			 * @readonly
			 */
		...
		proto: {
			 ...
			 *
			 * @param {kceditor.htmlParser.filterRulesDefinition} rules Object containing filter rules.
			 * @param {Object/Number} [options] Object containing rules' options or a priority
			 * (for a backward compatibility with KCEditor versions up to 4.2.x).
			 ...
			 *
			 * @param {kceditor.htmlParser.node} node The node to be filtered.
			 */
			...
			onNode: function( context, node ) {
				...
				return type == kceditor.NODE_ELEMENT ? this.onElement( context, node ) :
					type == kceditor.NODE_TEXT ? new kceditor.htmlParser.text( this.onText( context, node.value, node ) ) :
					type == kceditor.NODE_COMMENT ? new kceditor.htmlParser.comment( this.onComment( context, node.value, node ) ) : null;
			},
	 ...
	 *
	 * @class kceditor.htmlParser.filterRulesGroup
	 */
	...
	kceditor.htmlParser.filterRulesGroup = filterRulesGroup;
	...
	filterRulesGroup.prototype = {
		 ...
		 *
		 * @param {kceditor.htmlParser.node/kceditor.htmlParser.fragment/String} currentValue The value to be filtered.
		 * @returns {kceditor.htmlParser.node/kceditor.htmlParser.fragment/String} Filtered value.
		 */
		exec: function( context, currentValue ) {
			var isNode = currentValue instanceof kceditor.htmlParser.node || currentValue instanceof kceditor.htmlParser.fragment,
