 ...
 * this element.
 */
kceditor.htmlParser.element = function( name, attributes ) {
	/**
	 * The element name.
	 *
	 * @property {String}
	...
	// Reveal the real semantic of our internal custom tag name (https://dev.kceditor.com/ticket/6639),
	// when resolving whether it's block like.
	var realName = name || '',
		prefixed = realName.match( /^cke:(.*)/ );
	prefixed && ( realName = prefixed[ 1 ] );

	var isBlockLike = !!( kceditor.dtd.$nonBodyContent[ realName ] || kceditor.dtd.$block[ realName ] ||
		kceditor.dtd.$listItem[ realName ] || kceditor.dtd.$tableContent[ realName ] ||
		kceditor.dtd.$nonEditable[ realName ] || realName == 'br' );

	this.isEmpty = !!kceditor.dtd.$empty[ name ];
	this.isUnknown = !kceditor.dtd[ name ];
 ...
 * @param {kceditor.htmlParser.element/String} elementOrStyleText
 * An HTML parser element or the inline style text.
 */
kceditor.htmlParser.cssStyle = function() {
	var styleText,
		arg = arguments[ 0 ],
		rules = {};

	styleText = arg instanceof kceditor.htmlParser.element ? arg.attributes.style : arg;
	...
	return {
		 ...
		 *
		 * @param {kceditor.htmlParser.element/kceditor.dom.element/Object} obj
		 */
		populate: function( obj ) {
			var style = this.toString();
			if ( style )
				obj instanceof kceditor.dom.element ? obj.setAttribute( 'style', style ) : obj instanceof kceditor.htmlParser.element ? obj.attributes.style = style : obj.style = style;
...
/** @class kceditor.htmlParser.element */
( function() {
	...
	var sortAttribs = function( a, b ) {
		...
		fragProto = kceditor.htmlParser.fragment.prototype;

	kceditor.htmlParser.element.prototype = kceditor.tools.extend( new kceditor.htmlParser.node(), {
		/**
		 ...
		 * Clones this element.
		 *
		 * @returns {kceditor.htmlParser.element} The element clone.
		 */
		clone: function() {
			return new kceditor.htmlParser.element( this.name, this.attributes );
		},
		...
		filter: function( filter, context ) {
			...
			while ( true ) {
				...
				if ( element.type != kceditor.NODE_ELEMENT ) {
					this.replaceWith( element );
					return false;
				}
		...
		/**
		 * Writes the element HTML to the kceditor.htmlWriter.
		 *
		 * @param {kceditor.htmlParser.basicWriter} writer The writer to which HTML will be written.
		 ...
		 *		var elP = kceditor.htmlParser.fragment.fromHtml( 'foo<b>bar</b>bom', 'p' );
		 ...
		 * @param {kceditor.htmlParser.node} callback.node Node passed as an argument.
		 ...
		 * @returns {kceditor.htmlParser.node}
		 */
		 ...
		 */
		getHtml: function() {
			var writer = new kceditor.htmlParser.basicWriter();
			this.writeChildrenHtml( writer );
		...
		setHtml: function( html ) {
			var children = this.children = kceditor.htmlParser.fragment.fromHtml( html ).children;
		 ...
		 * @returns {kceditor.htmlParser.element} The new element following this one.
		 */
		 ...
		 * @returns {kceditor.htmlParser.node/null} The first matched child, `null` otherwise.
		 */
		findOne: function( criteria, recursive ) {
			var nestedMatch = null,
				match = kceditor.tools.array.find( this.children, function( child ) {
		...
		removeClass: function( className ) {
			...
			classes = kceditor.tools.trim( classes.replace( new RegExp( '(?:\\s+|^)' + className + '(?:\\s+|$)' ), ' ' ) );
		...
		getFilterContext: function( ctx ) {
			...
			// A context to be given nestedEditable must be nonEditable first (by inheritance) (https://dev.kceditor.com/ticket/11372, https://dev.kceditor.com/ticket/11698).
			// Special case: https://dev.kceditor.com/ticket/11504 - filter starts on <body contenteditable=true>,
	...
	function nameCondition( condition ) {
		return function( el ) {
			return el.type == kceditor.NODE_ELEMENT &&
