...
( function() {
	 ...
	 *
	 * @class kceditor.editable
	 * @extends kceditor.dom.element
	 */
	kceditor.editable = kceditor.tools.createClass( {
		base: kceditor.dom.element,
		/**
		 ...
		 * @param {kceditor.editor} editor The editor instance on which the editable operates.
		 * @param {HTMLElement/kceditor.dom.element} element Any DOM element that was as the editor's
		 * editing container, e.g. it could be either an HTML element with the `contenteditable` attribute
		 * set to the `true` that handles WYSIWYG editing or a `<textarea>` element that handles source editing.
		 */
		$: function( editor, element ) {
			// Transform the element into a kceditor.dom.element instance.
			 ...
			 *	the first {@link kceditor.editor#method-setData} is called.
		...
		proto: {
			focus: function() {
				...
				if ( kceditor.env.webkit && !this.hasFocus ) {
				...
				// Scroll only happens if the editor is focused for the first time. (https://dev.kceditor.com/ticket/14825)
				if ( kceditor.env.edge && kceditor.env.version > 14 && !this.hasFocus && this.getDocument().equals( kceditor.document ) ) {
				...
				try {
					if ( kceditor.env.ie && !( kceditor.env.edge && kceditor.env.version > 14 ) && this.getDocument().equals( kceditor.document ) ) {
						this.$.setActive();
					} else {
				...
				// Remedy if Safari doens't applies focus properly. (https://dev.kceditor.com/ticket/279)
				if ( kceditor.env.safari && !this.isInline() ) {
					active = kceditor.document.getActive();
			...
			/**
			 * Overrides {@link kceditor.dom.element#on} to have special `focus/blur` handling.
			...
			on: function( name, fn ) {
				...
				if ( kceditor.env.ie && ( /^focus|blur$/ ).exec( name ) ) {
					name = name == 'focus' ? 'focusin' : 'focusout';

					// The "focusin/focusout" events bubbled, e.g. If there are elements with layout
					// they fire this event when clicking in to edit them but it must be ignored
					// to allow edit their contents. (https://dev.kceditor.com/ticket/4682)
					fn = isNotBubbling( fn, this );
					args[ 0 ] = name;
					args[ 1 ] = fn;
				}

				return kceditor.dom.element.prototype.on.apply( this, args );
			},
			 ...
			 * for example on {@link kceditor.editor#setMode changing editor mode} or destroying editor.
			 *
			 * Except for `obj` all other arguments have the same meaning as in {@link kceditor.event#on}.
			 *
			 * This method is strongly related to the {@link kceditor.editor#contentDom} and
			 * {@link kceditor.editor#contentDomUnload} events, because they are fired
			 ...
			 *		} );
			 *
			 * @param {kceditor.event} obj The element/object to which the listener will be attached. Every object
			 * which inherits from {@link kceditor.event} may be used including {@link kceditor.dom.element},
			 * {@link kceditor.dom.document}, and {@link kceditor.editable}.
			 * @param {String} eventName The name of the event that will be listened to.
			 * @param {Function} listenerFunction The function listening to the
			 * event. A single {@link kceditor.eventInfo} object instance
			 * containing all the event data is passed to this function.
			 * @param {Object} [scopeObj] The object used to scope the listener
			 * call (the `this` object). If omitted, the current object is used.
			 * @param {Object} [listenerData] Data to be sent as the
			 * {@link kceditor.eventInfo#listenerData} when calling the listener.
			 ...
			 * @see kceditor.dom.element#addClass
			 */
			 ...
			 * See the {@link kceditor.editor#method-insertText} method which is the editor-level API
			 * for this purpose.
			 *
			 * @param {String} text
			 */
			insertText: function( text ) {
				// Focus the editor before calling transformPlainTextToHtml. (https://dev.kceditor.com/ticket/12726)
				this.editor.focus();
				this.insertHtml( this.transformPlainTextToHtml( text ), 'text' );
			},

			/**
			 * Transforms plain text to HTML based on current selection and {@link kceditor.editor#activeEnterMode}.
			 *
			 * @since 4.5.0
			...
			transformPlainTextToHtml: function( text ) {
				...
				return kceditor.tools.transformPlainTextToHtml( text, enterMode );
			},

			/**
			 * Low-level method for inserting HTML into the editable.
			 * See the {@link kceditor.editor#method-insertHtml} method which is the editor-level API
			 * for this purpose.
			 *
			 ...
			 *
			 * Fires the {@link kceditor.editor#event-afterInsertHtml} event.
			 *
			 * @param {String} data The HTML to be inserted.
			 * @param {String} [mode='html'] See {@link kceditor.editor#method-insertHtml}'s param.
			 * @param {kceditor.dom.range} [range] If specified, the HTML will be inserted into the range
			 * instead of into the selection. The selection will be placed at the end of the insertion (like in the normal case).
			 * Introduced in KCEditor 4.5.0.
			 */
			 ...
			 *
			 * **Note:** This method does not {@link kceditor.editor#saveSnapshot save undo snapshots} nor selects inserted
			 ...
			 * @param {kceditor.dom.range} range The range as a place of insertion.
			 * @param {String} [mode='html'] Mode in which HTML will be inserted.
			 * See {@link kceditor.editor#method-insertHtml}.
			 */
			 ...
			 * See the {@link kceditor.editor#method-insertElement} method which is the editor-level API
			 * for this purpose.
			 *
			 ...
			 *
			 * @param {kceditor.dom.element} element The element to insert.
			 * @param {kceditor.dom.range} [range] If specified, the element will be inserted into the range
			 * instead of into the selection.
			 */
			insertElement: function( element, range ) {
				var editor = this.editor;

				// Prepare for the insertion. For example - focus editor (https://dev.kceditor.com/ticket/11848).
				editor.focus();
				editor.fire( 'saveSnapshot' );
				...
				var enterMode = editor.activeEnterMode,
					...
					isBlock = kceditor.dtd.$block[ elementName ];

				if ( !range ) {
					range = selection.getRanges()[ 0 ];
				}

				// Insert element into first range only and ignore the rest (https://dev.kceditor.com/ticket/11183).
				if ( this.insertElementIntoRange( element, range ) ) {
					range.moveToPosition( element, kceditor.POSITION_AFTER_END );

					// If we're inserting a block element, the new cursor position must be
					// optimized. (https://dev.kceditor.com/ticket/3100,https://dev.kceditor.com/ticket/5436,https://dev.kceditor.com/ticket/8950)
					if ( isBlock ) {
						...
						if ( next && next.type == kceditor.NODE_ELEMENT && next.is( kceditor.dtd.$block ) ) {
						...
						else if ( !next && enterMode != kceditor.ENTER_BR ) {
							next = range.fixBlock( true, enterMode == kceditor.ENTER_DIV ? 'div' : 'p' );
							range.moveToElementEditStart( next );
						}
					}
				}
			 ...
			 *
			 * @deprecated
			 * @param {kceditor.dom.element} element The element to be inserted.
			 */
			 ...
			 *
			 * **Note:** This method does not {@link kceditor.editor#saveSnapshot save undo snapshots} nor selects the inserted
			...
			insertElementIntoRange: function( element, range ) {
				...
				if ( range.startContainer.type == kceditor.NODE_ELEMENT ) {
					...
					// (https://dev.kceditor.com/ticket/11183#comment:8),
					...
					} else if ( range.startContainer.is( kceditor.dtd.$list ) ) {
				...
				if ( isBlock ) {
					while ( ( current = range.getCommonAncestor( 0, 1 ) ) &&
							( dtd = kceditor.dtd[ current.getName() ] ) &&
							!( dtd && dtd[ elementName ] ) ) {
						// Split up inline elements.
						if ( current.getName() in kceditor.dtd.span ) {
						...
						// simply replace it with the inserting block (https://dev.kceditor.com/ticket/3664).
						} else if ( range.checkStartOfBlock() && range.checkEndOfBlock() ) {
							range.setStartBefore( current );
							range.collapse( true );
							current.remove();
						} else {
							range.splitBlock( enterMode == kceditor.ENTER_DIV ? 'div' : 'p', editor.editable() );
						}
					}
				}
			...
			detach: function() {
				...
				try {
					...
					if ( !kceditor.env.ie || error.number !== -2146828218 ) {
						throw( error );
					}
				}
			 ...
			 */
			isInline: function() {
				return this.getDocument().equals( kceditor.document );
			},
			 ...
			 *
			 * * http://tests.kceditor.dev:1030/tests/core/selection/manual/focusaftersettingdata
			 * * http://tests.kceditor.dev:1030/tests/core/selection/manual/focusafterundoing
			 * * http://tests.kceditor.dev:1030/tests/core/selection/manual/selectionafterfocusing
			 * * http://tests.kceditor.dev:1030/tests/plugins/newpage/manual/selectionafternewpage
			 *
			 * @since 4.4.6
			 * @private
			 */
			fixInitialSelection: function() {
				...
				if ( kceditor.env.ie && ( kceditor.env.version < 9 || kceditor.env.quirks ) ) {
					if ( this.hasFocus ) {
						this.focus();
						fixMSSelection();
					}
				...
				function fixSelection() {
					...
					if ( requiresFix( $sel ) ) {
						var range = new kceditor.dom.range( that );
				...
				function fixMSSelection() {
					...
					if ( $sel.type == 'None' && active.equals( that ) ) {
						...
						if ( parentElement.type != kceditor.NODE_ELEMENT ) {
			...
			/**
			 * The base of the {@link kceditor.editor#getSelectedHtml} method.
			 *
			 * @since 4.5.0
			 * @method getHtmlFromRange
			 * @param {kceditor.dom.range} range
			 * @returns {kceditor.dom.documentFragment}
			 */
			getHtmlFromRange: function( range ) {
				// There's nothing to return if range is collapsed.
				if ( range.collapsed )
					return new kceditor.dom.documentFragment( range.document );
				...
				return new kceditor.dom.documentFragment( that.fragment.$ );
			},

			/**
			 * The base of the {@link kceditor.editor#extractSelectedHtml} method.
			 *
			 ...
			 * @since 4.5.0
			 * @param {kceditor.dom.range} range
			 * @param {Boolean} [removeEmptyBlock=false] See {@link kceditor.editor#extractSelectedHtml}'s parameter.
			 * Note that the range will not be modified if this parameter is set to `true`.
			 * @returns {kceditor.dom.documentFragment} The extracted fragment of the editable content.
			 */
			extractHtmlFromRange: function( range, removeEmptyBlock ) {
				...
				range.enlarge( kceditor.ENLARGE_INLINE, 1 );
				...
				targetRange.moveToPosition( that.bookmark.startNode, kceditor.POSITION_BEFORE_START );
				...
				if ( removeEmptyBlock ) {
					...
					if (
						...
						// Do not remove a block with bookmarks. (https://dev.kceditor.com/ticket/13465)
						!hasBookmarks( path.block ) ) {
						range.moveToPosition( path.block, kceditor.POSITION_BEFORE_START );
						path.block.remove();
					}
				} else {
			...
			setup: function() {
				...
				this.attachListener( editor, 'beforeFocus', function() {
					...
					// selection in such case. (https://dev.kceditor.com/ticket/5812) (https://dev.kceditor.com/ticket/8949)
				...
				} else if ( editor.elementMode == kceditor.ELEMENT_MODE_REPLACE ||
					editor.elementMode == kceditor.ELEMENT_MODE_APPENDTO ) {
				...
				if ( kceditor.env.webkit ) {
					// [WebKit] Save scrollTop value so it can be used when restoring locked selection. (https://dev.kceditor.com/ticket/14659)
				...
				// scrollTop value and removes listener which is not needed anymore. (https://dev.kceditor.com/ticket/14825)
				if ( kceditor.env.edge && kceditor.env.version > 14 ) {

					var fixScrollOnFocus = function() {
						var editable = editor.editable();

						if ( editor._.previousScrollTop != null && editable.getDocument().equals( kceditor.document ) ) {
				...
				if ( this.equals( kceditor.document.getActive() ) ) {
					this.hasFocus = true;
				...
				if ( styles ) {
					...
					if ( !stylesElement ) {
						...
						sheet = new kceditor.dom.element( sheet.ownerNode || sheet.owningElement );
						head.setCustomData( 'stylesheet', sheet );
						sheet.data( 'cke-temp', 1 );
					} else if ( styles != stylesElement.getText() ) {
						kceditor.env.ie && kceditor.env.version < 9 ? stylesElement.$.styleSheet.cssText = styles : stylesElement.setText( styles );
					}
				}
				...
				// Prevent the browser opening read-only links. (https://dev.kceditor.com/ticket/6032 & https://dev.kceditor.com/ticket/10912)
				this.attachListener( this, 'click', function( evt ) {
					evt = evt.data;

					var link = new kceditor.dom.elementPath( evt.getTarget(), this ).contains( 'a' );
				...
				//  on fully selected element . (https://dev.kceditor.com/ticket/4047) (https://dev.kceditor.com/ticket/7645)
				this.attachListener( editor, 'key', function( evt ) {
					if ( editor.readOnly )
						return true;

					// Use getKey directly in order to ignore modifiers.
					// Justification: https://dev.kceditor.com/ticket/11861#comment:13
					var keyCode = evt.data.domEvent.getKey(),
						isHandled;

					// Prevent of reading path of empty range (https://dev.kceditor.com/ticket/13096, #457).
					var sel = editor.getSelection();
					...
					if ( keyCode in backspaceOrDelete ) {
						...
						// [IE<11] Remove selected image/anchor/etc here to avoid going back in history. (https://dev.kceditor.com/ticket/10055)
						if ( kceditor.env.ie && kceditor.env.version < 11 && sel.getSelectedElement() ) {
						...
						// Check it selection contain list or table.
							...
							if ( !isMultipleListSelection ) {
								// Remove the entire list/table on fully selected content. (https://dev.kceditor.com/ticket/7645)
						...
						if ( selected || isMultipleListSelection ) {
							...
							if ( isMultipleListSelection ) {
								...
								// break up the selection, safely manage it here. (https://dev.kceditor.com/ticket/4795)
								range.moveToPosition( selected, kceditor.POSITION_BEFORE_START );
							...
							// Handle the following special cases: (https://dev.kceditor.com/ticket/6217)
							// 1. Del/Backspace key before/after table;
							// 2. Backspace Key after start of table.
							if ( ( block = path.block ) &&
									( next = block[ rtl ? 'getPrevious' : 'getNext' ]( isNotWhitespace ) ) &&
									( next.type == kceditor.NODE_ELEMENT ) &&
									next.is( 'table' ) &&
								...
								isHandled = 1;
									...
									range.checkBoundaryOfElement( parent, rtl ? kceditor.START : kceditor.END ) &&
				...
				if ( editor.blockless && kceditor.env.ie && kceditor.env.needsBrFiller ) {
					this.attachListener( this, 'keyup', function( evt ) {
						if ( evt.data.getKeystroke() in backspaceOrDelete && !this.getFirst( isNotEmpty ) ) {
							...
							range.moveToPosition( this, kceditor.POSITION_AFTER_START );
							range.select();
						}
					} );
				}
				...
				// Prevent automatic submission in IE https://dev.kceditor.com/ticket/6336
				kceditor.env.ie && this.attachListener( this, 'click', blockInputClick );

				// Gecko/Webkit need some help when selecting control type elements. (https://dev.kceditor.com/ticket/3448)
				// We apply same behavior for IE Edge. (https://dev.kceditor.com/ticket/13386)
				if ( !kceditor.env.ie || kceditor.env.edge ) {
					this.attachListener( this, 'mousedown', function( ev ) {
						var control = ev.data.getTarget();
						// https://dev.kceditor.com/ticket/11727. Note: htmlDP assures that input/textarea/select have contenteditable=false
						...
						if ( control.is( 'img', 'hr', 'input', 'textarea', 'select' ) && !control.isReadOnly() ) {
							...
							// Prevent focus from stealing from the editable. (https://dev.kceditor.com/ticket/9515)
				...
				// For some reason, after click event is done, IE Edge loses focus on the selected element. (https://dev.kceditor.com/ticket/13386)
				// Additional check for readonly disabled selecting of non-editable images (#2129).
				if ( kceditor.env.edge ) {
				...
				// when selection is anchored inside it. (https://dev.kceditor.com/ticket/5845)
				...
				// blocks on BACKSPACE/DEL (https://dev.kceditor.com/ticket/11861,https://dev.kceditor.com/ticket/9998).
				if ( kceditor.env.webkit ) {
					this.attachListener( editor, 'key', function( evt ) {
						if ( editor.readOnly ) {
							return true;
						...
						// Scroll to the new position of the caret (https://dev.kceditor.com/ticket/11960).
			...
			/**
			 * @inheritdoc kceditor.dom.domObject#getUniqueId
			 */
			getUniqueId: function() {
				...
				try {
					this._.expandoNumber = expandoNumber = kceditor.dom.domObject.prototype.getUniqueId.call( this );
				} catch ( e ) {
	 ...
	 * This method should always be used instead of calling {@link kceditor.editable} directly.
	 *
	 * @method editable
	 * @member kceditor.editor
	 * @param {kceditor.dom.element/kceditor.editable} [elementOrEditable] The
	 * DOM element to become the editable or a {@link kceditor.editable} object.
	 * @returns {kceditor.dom.element/null} The editor's editable element, or `null` if not available.
	 */
	kceditor.editor.prototype.editable = function( element ) {
		...
		if ( element ) {
			editable = element instanceof kceditor.editable ? element : new kceditor.editable( this, element );
		} else {
	...
	kceditor.on( 'instanceLoaded', function( evt ) {
		var editor = evt.editor;

		// and flag that the element was locked by our code so it'll be editable by the editor functions (https://dev.kceditor.com/ticket/6046).
		editor.on( 'insertElement', function( evt ) {
			var element = evt.data;
			if ( element.type == kceditor.NODE_ELEMENT && ( element.is( 'input' ) || element.is( 'textarea' ) ) ) {
		...
		editor.on( 'selectionChange', function( evt ) {
			...
			// Auto fixing on some document structure weakness to enhance usabilities. (https://dev.kceditor.com/ticket/3190 and https://dev.kceditor.com/ticket/3189)
			var sel = editor.getSelection();
			// Do it only when selection is not locked. (https://dev.kceditor.com/ticket/8222)
			if ( sel && !sel.isLocked ) {
	...
	kceditor.on( 'instanceCreated', function( evt ) {
		var editor = evt.editor;
		...
		editor.on( 'mode', function() {
			...
			if ( editable && editable.isInline() ) {
				...
				if ( helpLabel ) {
					...
					var ct = this.ui.space( this.elementMode == kceditor.ELEMENT_MODE_INLINE ? 'top' : 'contents' );
					if ( ct ) {
						var ariaDescId = kceditor.tools.getNextId(),
							desc = kceditor.dom.element.createFromHtml( '<span id="' + ariaDescId + '" class="cke_voice_label">' + helpLabel + '</span>' );
						ct.append( desc );
	...
	// https://dev.kceditor.com/ticket/9222: Show text cursor in Gecko.
	// Show default cursor over control elements on all non-IEs.
	kceditor.addCss( '.cke_editable{cursor:text}.cke_editable img,.cke_editable input,.cke_editable textarea{cursor:default}' );
	...
	isNotWhitespace = kceditor.dom.walker.whitespaces( true ),
	isNotBookmark = kceditor.dom.walker.bookmark( false, true ),
	isEmpty = kceditor.dom.walker.empty(),
	isBogus = kceditor.dom.walker.bogus(),
	// Matching an empty paragraph at the end of document.
	emptyParagraphRegexp = /(^|<body\b[^>]*>)\s*<(p|div|address|h\d|center|pre)[^>]*>\s*(?:<br[^>]*>|&nbsp;|\u00A0|&#160;)?\s*(:?<\/\2>)?\s*(?=$|<\/body>)/gi;

	// Auto-fixing block-less content by wrapping paragraph (https://dev.kceditor.com/ticket/3190), prevent
	// non-exitable-block by padding extra br.(https://dev.kceditor.com/ticket/3189)
	...
	function fixDom( evt ) {
		...
		if ( kceditor.env.gecko || ( kceditor.env.ie && kceditor.env.needsBrFiller ) ) {
			...
			if ( blockNeedsFiller ) {
				...
				selectionUpdateNeeded = ( kceditor.env.ie && !kceditor.env.edge ) ||
					( kceditor.env.edge && editor._.previousActive );
			}
		}

		// When we're in block enter mode, a new paragraph will be established
		// to encapsulate inline contents inside editable. (https://dev.kceditor.com/ticket/3657)
		...
		if ( shouldAutoParagraph( editor, path.block, blockLimit ) && range.collapsed && !range.getCommonAncestor().isReadOnly() ) {
			...
			testRng.enlarge( kceditor.ENLARGE_BLOCK_CONTENTS );
			var walker = new kceditor.dom.walker( testRng );
			walker.guard = function( node ) {
				return !isNotEmpty( node ) ||
					node.type == kceditor.NODE_COMMENT ||
					node.isReadOnly();
			};
			...
			if ( !walker.checkForward() || testRng.checkStartOfBlock() && testRng.checkEndOfBlock() ) {
				var fixedBlock = range.fixBlock( true, editor.activeEnterMode == kceditor.ENTER_DIV ? 'div' : 'p' );

				// For IE<11, we should remove any filler node which was introduced before.
				if ( !kceditor.env.needsBrFiller ) {
				...
				// Cancel this selection change in favor of the next (correct). (https://dev.kceditor.com/ticket/6811)
				evt.cancel();
			}
		}
	...
	function needsBrFiller( selection, path ) {
		...
		// Ensure bogus br could help to move cursor (out of styles) to the end of block. (https://dev.kceditor.com/ticket/7041)
		var pathBlock = path.block || path.blockLimit,
			lastNode = pathBlock && pathBlock.getLast( isNotEmpty );

		// Check some specialities of the current path block:
		// 1. It is really displayed as block; (https://dev.kceditor.com/ticket/7221)
		// 2. It doesn't end with one inner block; (https://dev.kceditor.com/ticket/7467)
		// 3. It doesn't have bogus br yet.
		if (
			pathBlock && pathBlock.isBlockBoundary() &&
			!( lastNode && lastNode.type == kceditor.NODE_ELEMENT && lastNode.isBlockBoundary() ) &&
	...
	function isNbsp( node ) {
		return node.type == kceditor.NODE_TEXT && kceditor.tools.trim( node.getText() ).match( /^(?:&nbsp;|\xa0)$/ );
	}
	...
	function isNotBubbling( fn, src ) {
		return function( evt ) {
			...
			other = ( other && other.nodeType == kceditor.NODE_ELEMENT ) ? new kceditor.dom.element( other ) : null;
	...
	function getSelectedTableList( sel ) {
		...
		if ( isListOrTableInSelection( sel ) ) {
			...
			walkerRng.setStartAt( editable, kceditor.POSITION_AFTER_START );

			// Create a new walker.
			var walker = new kceditor.dom.walker( walkerRng );
			...
			if ( selected ) {
				...
				walkerRng.collapse();
				walkerRng.setEndAt( selected, kceditor.POSITION_AFTER_END );

				// Create a new walker.
				walker = new kceditor.dom.walker( walkerRng );
		...
		function guard( forwardGuard ) {
			...
			return function( node, isWalkOut ) {
				...
				if ( isWalkOut && node.type == kceditor.NODE_ELEMENT && node.is( structural ) )
					selected = node;
	...
	function areMultipleListsInRange( range ) {
		...
		var walker = new kceditor.dom.walker( range ),
	...
	function startsAtFirstListItem( range ) {
		...
		var possibleListItems = [ 'dd', 'dt', 'li' ],
			...
			isListItem = kceditor.tools.array.indexOf( possibleListItems, blockName ) !== -1;
	...
	function getRangeForSelectedLists( range ) {
		...
		range.enlarge( kceditor.ENLARGE_ELEMENT );
	...
	function shouldAutoParagraph( editor, pathBlock, pathBlockLimit ) {
		// Check whether pathBlock equals pathBlockLimit to support nested editable (https://dev.kceditor.com/ticket/12162).
		return editor.config.autoParagraph !== false &&
			editor.activeEnterMode != kceditor.ENTER_BR &&
			(
	...
	function autoParagraphTag( editor ) {
		return ( editor.activeEnterMode != kceditor.ENTER_BR && editor.config.autoParagraph !== false ) ? editor.activeEnterMode == kceditor.ENTER_DIV ? 'div' : 'p' : false;
	}
	...
	insert = ( function() {
		...
		function insert( editable, type, data, range ) {
			...
			var path = new kceditor.dom.elementPath( range.startContainer, range.root ),
			...
			if ( editor.enterMode === kceditor.ENTER_DIV && isEmptyEditable ) {
			...
			// any insertion is pointless (https://dev.kceditor.com/ticket/10339).
		...
		function clearEditable( editable, range ) {
			...
			first && first.remove();
			range.setStartAt( editable, kceditor.POSITION_AFTER_START );
			range.collapse( true );
		}
		...
		function prepareRangeToDataInsertion( that ) {
			...
			if ( that.type == 'text' && range.shrink( kceditor.SHRINK_ELEMENT, true, false ) ) {
				marker = kceditor.dom.element.createFromHtml( '<span>&nbsp;</span>', range.document );
			...
			startPath = new kceditor.dom.elementPath( range.startContainer );
			that.endPath = endPath = new kceditor.dom.elementPath( range.endContainer );
			...
			while (
				...
				range.moveToPosition( previous, kceditor.POSITION_BEFORE_END );
		...
		function processDataForInsertion( that, data ) {
			...
			if ( /^\s+|\s+$/.test( data ) && 'span' in kceditor.dtd[ context ] ) {
		...
		function insertDataIntoRange( that ) {
			var range = that.range,
				...
				separateEndContainer = !!endContainer.getCommonAncestor( startContainer ) && // endC is not detached.
					pos != kceditor.POSITION_IDENTICAL && !( pos & kceditor.POSITION_CONTAINS + kceditor.POSITION_IS_CONTAINED ); // endC & endS are in separate branches.

			nodesData = extractNodesData( that.dataWrapper, that );

			// Keep br's when kceditor.ENTER_BR is active for proper spacing. (#3858)
			if ( that.editor.enterMode !== kceditor.ENTER_BR ) {
			...
			for ( ; nodeIndex < nodesData.length; nodeIndex++ ) {
				...
				if ( !nodeData.isBlock && shouldAutoParagraph( that.editor, path.block, path.blockLimit ) && ( fixBlock = autoParagraphTag( that.editor ) ) ) {
					...
					if ( kceditor.env.needsBrFiller && ( bogus = fixBlock.getBogus() ) )
						bogus.remove();
					range.moveToPosition( fixBlock, kceditor.POSITION_BEFORE_END );
				}
				...
				if ( nodeData.lastNotAllowed && nodeIndex < nodesData.length - 1 ) {
					...
					newContainer && range.setEndAt( newContainer, kceditor.POSITION_AFTER_START );
					splittingContainer = 0;
				}
			...
			if ( isSingleNonEditableElement( nodesData ) ) {
				...
				range.setStartAt( node, kceditor.POSITION_BEFORE_START );
				range.setEndAt( node, kceditor.POSITION_AFTER_END );
			}
		...
		function cleanupAfterInsertion( that ) {
			...
			if ( kceditor.env.webkit && range.startPath() ) {
				var path = range.startPath();
			...
			if ( !that.dontMoveCaret ) {
				...
				while ( node && checkIfElement( node ) && !node.is( DTD.$empty ) ) {
					if ( node.isBlockBoundary() )
						range.moveToPosition( node, kceditor.POSITION_BEFORE_END );
					else {
						...
						movedIntoInline.moveToPosition( node, kceditor.POSITION_BEFORE_END );
					}
		...
		function checkIfElement( node ) {
			return node.type == kceditor.NODE_ELEMENT;
		}
		...
		function extractNodesData( dataWrapper, that ) {
			...
			for ( ; nodeIndex < nodesCount; ++nodeIndex ) {
				...
				if ( checkIfElement( node ) ) {
					...
					// inside of a list, reads as rearrange list items. (https://dev.kceditor.com/ticket/7957)
					if ( insideOfList && nodeName in kceditor.dtd.$list ) {
			...
			for ( ; nodeIndex < nodesCount; nodeIndex++ ) {
				...
				if ( node == ' ' ) {
					...
					if ( !afterSpace && !( isFirst && !nodeIndex ) ) {
						nodes2.push( new kceditor.dom.text( ' ' ) );
		...
		function mergeAncestorElementsOfSelectionEnds( range, blockLimit, startPath, endPath ) {
			...
			walkerRange.setEndAt( blockLimit, kceditor.POSITION_BEFORE_END );
			walker = new kceditor.dom.walker( walkerRange );
		...
		function splitOnLineBreak( range, blockLimit, nodeData ) {
			...
			if ( pos == kceditor.POSITION_IDENTICAL || pos == kceditor.POSITION_CONTAINS )
				return 0;

			var newContainer = range.splitElement( firstBlockAscendant );
			range.moveToPosition( newContainer, kceditor.POSITION_AFTER_START );

			return 1;
		}

		var stripSingleBlockTags = { p: 1, div: 1, h1: 1, h2: 1, h3: 1, h4: 1, h5: 1, h6: 1 },
			inlineButNotBr = kceditor.tools.extend( {}, DTD.$inline );
		...
		function wrapDataWithInlineStyles( data, that ) {
			...
			// characters ($$, $&, etc.) are in data (https://dev.kceditor.com/ticket/10367).
	...
	fixTableAfterContentsDeletion = ( function() {
		...
		function getFixTableSelectionWalker( testRange ) {
			var walker = new kceditor.dom.walker( testRange );
			walker.guard = function( node, isMovingOut ) {
				if ( isMovingOut )
					return false;
				if ( node.type == kceditor.NODE_ELEMENT )
					return node.is( kceditor.dtd.$tableContent );
			};
		...
		// * remove empty text nodes on IE8, because it will crash (https://dev.kceditor.com/ticket/11183#comment:8).
		function fixEmptyCells( cells ) {
			...
			for ( i; i-- > 0; ) {
				...
				if ( !kceditor.tools.trim( cell.getHtml() ) ) {
					cell.appendBogus();
					if ( kceditor.env.ie && kceditor.env.version < 9 && cell.getChildCount() )
		...
		return function( range ) {
			...
			if ( !deeperSibling ) {
				testRange = range.clone();
				testRange.setEndAt( container, kceditor.POSITION_BEFORE_END );
			...
			if ( deeperSibling.is( 'table' ) ) {
				range.setStartAt( deeperSibling, kceditor.POSITION_BEFORE_START );
				range.collapse( true );
			...
			range.moveToPosition( deeperSibling, appendToStart ? kceditor.POSITION_AFTER_START : kceditor.POSITION_BEFORE_END );
		};
	} )();
	...
	fixListAfterContentsDelete = ( function() {
		...
		function getFixListSelectionWalker( testRange ) {
			...
			walker.guard = function( node, isMovingOut ) {
				if ( isMovingOut )
					return false;
				if ( node.type == kceditor.NODE_ELEMENT )
					return node.is( kceditor.dtd.$list ) || node.is( kceditor.dtd.$listItem );
			};
			walker.evaluator = function( node ) {
				return node.type == kceditor.NODE_ELEMENT && node.is( kceditor.dtd.$listItem );
			};
		...
		return function( range ) {
			...
			if ( deeperSibling.is( kceditor.dtd.$list ) ) {
				range.setStartAt( deeperSibling, kceditor.POSITION_BEFORE_START );
				range.collapse( true );
				deeperSibling.remove();
				return;
	...
	function mergeBlocksCollapsedSelection( editor, range, backspace, startPath, skipRangeTrimming ) {
		...
		// in the same place (https://dev.kceditor.com/ticket/11861#comment:9).
		if ( range.startContainer.type == kceditor.NODE_ELEMENT ) {
			var touched = range.startContainer.getChild( range.startOffset - ( backspace ? 1 : 0 ) );
			if ( touched && touched.type  == kceditor.NODE_ELEMENT && touched.is( 'hr' ) ) {
				editor.fire( 'saveSnapshot' );
				touched.remove();
				return true;
			}
	...
	function mergeBlocksNonCollapsedSelection( editor, range, startPath ) {
		...
		// Changing end container to element from text node (https://dev.kceditor.com/ticket/12503).
		range.enlarge( kceditor.ENLARGE_INLINE );
		...
		// Optimizing range containers from text nodes to elements (https://dev.kceditor.com/ticket/12503).
		range.optimize();
	...
	//
	// @param {kceditor.dom.element} first
	// @param {kceditor.dom.element} second
	...
	getHtmlFromRangeHelpers = {
		eol: {
			detect: function( that, editable ) {
				var range = that.range,
					...
					startPath = new kceditor.dom.elementPath( range.startContainer, editable ),
					endPath = new kceditor.dom.elementPath( range.endContainer, editable );

				// Note: checkBoundaryOfElement will not work on original range as kceditor.START|END
				...
				if ( startPath.block && rangeStart.checkBoundaryOfElement( startPath.block, kceditor.END ) ) {
					range.setStartAfter( startPath.block );
					that.prependEolBr = 1;
				}

				if ( endPath.block && rangeEnd.checkBoundaryOfElement( endPath.block, kceditor.START ) ) {
		...
		tree: {
			rebuild: function( that, editable ) {
				var range = that.range,
					...
					commonPath = new kceditor.dom.elementPath( node, editable ),
					startPath = new kceditor.dom.elementPath( range.startContainer, editable ),
					endPath = new kceditor.dom.elementPath( range.endContainer, editable ),
					limit;
				...
				// instead of
				// 		b<ul><li>c</li></ul>
				else if ( commonPath.block && commonPath.block.is( kceditor.dtd.$listItem ) ) {
					var startList = startPath.contains( kceditor.dtd.$list ),
						endList = endPath.contains( kceditor.dtd.$list );

					if ( !startList.equals( endList ) ) {
						var listParent = commonPath.contains( kceditor.dtd.$list ).getParent();
		...
		cell: {
			...
			shrink: function( that ) {
				...
				if ( startContainer.type == kceditor.NODE_ELEMENT && startContainer.equals( endContainer ) && startContainer.is( 'tr' ) && ++startOffset == endOffset ) {
					range.shrink( kceditor.SHRINK_TEXT );
				}
			}
		}
	};
	...
	extractHtmlFromRangeHelpers = ( function() {
		function optimizeBookmarkNode( node, toStart ) {
			...
			if ( parent.is( kceditor.dtd.$inline ) )
		...
		function mergeElements( merged, startBookmark, endBookmark ) {
			...
			while ( ( next = endBookmark.getNext() ) ) {
				...
				// Update startBookmark after insertion to avoid the reversal of nodes (https://dev.kceditor.com/ticket/13449).
				startBookmark = next;
			}
		...
		var list = {
			detectMerge: function( that, editable ) {
				var range = createRangeFromBookmark( editable, that.bookmark ),
					...
					startList = startPath.contains( kceditor.dtd.$list ),
					endList = endPath.contains( kceditor.dtd.$list );
				...
				that.mergeListItems =
					...
					startPath.block.is( kceditor.dtd.$listItem ) && endPath.block.is( kceditor.dtd.$listItem );
			...
			merge: function( that, editable ) {
				...
				if ( that.mergeList ) {
					var firstList = startPath.contains( kceditor.dtd.$list ),
						secondList = endPath.contains( kceditor.dtd.$list );
				...
				if ( that.mergeListItems ) {
					var firstListItem = startPath.contains( kceditor.dtd.$listItem ),
						secondListItem = endPath.contains( kceditor.dtd.$listItem );
		...
		var block = {
			...
			detectMerge: function( that, editable ) {
				...
				var rangeClone = new kceditor.dom.range( editable );
		...
		var table = ( function() {
			...
			function findTableContentsRanges( range ) {
				...
				var contentsRanges = [],
					editableRange,
					walker = new kceditor.dom.walker( range ),
				...
				walker.guard = function( node, leaving ) {
					...
					// what results in creating more than one range for each selected cell. (https://dev.kceditor.com/ticket/12964)
					if ( node.type == kceditor.NODE_ELEMENT ) {
						...
						kceditor.dom.element.setMarker( database, node, key, 1 );
					}
					...
					if ( leaving && startCell && node.equals( startCell ) ) {
						...
						editableRange.setEndAt( startCell, kceditor.POSITION_BEFORE_END );
					...
					if ( !leaving && endCell && node.equals( endCell ) ) {
						editableRange = range.clone();
						editableRange.setStartAt( endCell, kceditor.POSITION_AFTER_START );
						contentsRanges.push( editableRange );
						return;
					}
				...
				kceditor.dom.element.clearAllMarkers( database );
				...
				function checkRemoveCellContents( node ) {
					return (
						// Must be a cell.
						node.type == kceditor.NODE_ELEMENT && node.is( tableEditable ) &&
			...
			function getNormalizedAncestor( range ) {
				...
				if ( common.is( kceditor.dtd.$tableContent ) && !common.is( tableEditable ) ) {
			...
			function checkDisjointNodes( node1, node2 ) {
				var disallowedPositions = kceditor.POSITION_CONTAINS + kceditor.POSITION_IS_CONTAINED,
					pos = node1.getPosition( node2 );

				// Baaah... IDENTICAL is 0, so we can't simplify this ;/.
				return pos === kceditor.POSITION_IDENTICAL ?
					false :
			...
			return {
				...
				detectPurge: function( that ) {
					...
					walkerRange.enlarge( kceditor.ENLARGE_ELEMENT );

					var walker = new kceditor.dom.walker( walkerRange ),
						editablesCount = 0;
					...
					walker.evaluator = function( node ) {
						if ( node.type == kceditor.NODE_ELEMENT && node.is( tableEditable ) ) {
							++editablesCount;
						}
					};
					...
					if ( editablesCount > 1 ) {
						...
						if ( startTable && endTable && range.checkBoundaryOfElement( startTable, kceditor.START ) && range.checkBoundaryOfElement( endTable, kceditor.END ) ) {
				...
				detectRanges: function( that, editable ) {
					var range = createRangeFromBookmark( editable, that.bookmark ),
						...
						startPath = new kceditor.dom.elementPath( range.startContainer, commonAncestor ),
						endPath = new kceditor.dom.elementPath( range.endContainer, commonAncestor ),
					...
					if ( startTable && endTable && checkDisjointNodes( startTable, endTable ) ) {
						...
						surroundingRange.setStartAt( startTable, kceditor.POSITION_AFTER_END );
						surroundingRange.setEndAt( endTable, kceditor.POSITION_BEFORE_START );

						leftRange = range.clone();
						leftRange.setEndAt( startTable, kceditor.POSITION_AFTER_END );

						rightRange = range.clone();
						rightRange.setStartAt( endTable, kceditor.POSITION_BEFORE_START );
					...
					// <p>a{b</p>][<table>..]..</table><p>cd</p>
						...
						range.setStartAt( endTable, kceditor.POSITION_AFTER_START );
					}
					...
					// <p>ab</p><table>..[..</table>][<p>c}d</p>
						...
						range.setEndAt( startTable, kceditor.POSITION_AFTER_END );
					}
				...
				purge: function( that ) {
					...
					that.range.moveToPosition( block, kceditor.POSITION_AFTER_START );
				}
			};
		} )();
		...
		return {
			...
			detectExtractMerge: function( that ) {
				...
				return !(
					that.range.startPath().contains( kceditor.dtd.$listItem ) &&
					that.range.endPath().contains( kceditor.dtd.$listItem )
				);
			},
			...
			autoParagraph: function( editor, range ) {
				...
				if ( shouldAutoParagraph( editor, path.block, path.blockLimit ) && ( fixBlock = autoParagraphTag( editor ) ) ) {
					...
					fixBlock.appendBogus();
					range.insertNode( fixBlock );
					range.moveToPosition( fixBlock, kceditor.POSITION_AFTER_START );
				}
			}
		};
	} )();
 ...
 *		} );
 *
 * **Note:** To handle double-click on a widget use {@link kceditor.plugins.widget#doubleclick}.
 *
 * @event doubleclick
 * @param data
 * @param {kceditor.dom.element} data.element The double-clicked element.
