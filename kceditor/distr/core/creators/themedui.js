 ...
 * by KCEditor instances. Set it to empty/`null` to disable this feature.
 *
 *		kceditor.replaceClass = 'rich_editor';
 *
 * @cfg {String} [replaceClass='kceditor']
 */
kceditor.replaceClass = 'kceditor';

( function() {
	/**
	 * Replaces a `<textarea>` or a DOM element (`<div>`) with a KCEditor
	 ...
	 *		...
	 *		kceditor.replace( 'myfield' );
	 *
	 *		var textarea = document.body.appendChild( document.createElement( 'textarea' ) );
	 *		kceditor.replace( textarea );
	 *
	 ...
	 * to cancel the interval set by the {@link kceditor.config#delayIfDetached} and
	 * {@link kceditor.config#delayIfDetached_interval} options.
	 *
	 * ```javascript
	 * var cancelInterval = kceditor.replace( 'editor', {
	 ...
	 * editor instance. Configuration set here will override the global KCEditor settings
	 * (see {@link kceditor.config}).
	 * @returns {kceditor.editor/Function/null} The editor instance or a cancellation function.
	 ...
	 */
	kceditor.replace = function( element, config ) {
		return createInstance( element, config, null, kceditor.ELEMENT_MODE_REPLACE );
	};
	 ...
	 * 			<head>
	 * 				<meta charset="utf-8">
	 * 				<title>KCEditor</title>
	 * 				<!-- Make sure the path to KCEditor is correct. -->
	 *				<script src="/kceditor/kceditor.js"></script>
	 *			</head>
	 *			<body>
	 *				<div id="editorSpace"></div>
	 *				<script>
	 *					kceditor.appendTo( 'editorSpace' );
	 *				</script>
	 *			</body>
	 *		</html>
	 *
	 ...
	 *
	 * ```javascript
	 * var cancelInterval = kceditor.appendTo( 'editorSpace', {
	 * 	delayIfDetached: true,
	 * 	delayIfDetached_interval: 50 // Default value, you can skip that option.
	 * } );
	 *
	 ...
	 * @returns {kceditor.editor/Function/null} The editor instance or a cancelation function.
	 * If {@glink features/delayed_creation Delayed Editor Creation} feature has not been set and
	 * element is missing in DOM, this function will return `null`.
	 */
	kceditor.appendTo = function( element, config, data ) {
		return createInstance( element, config, data, kceditor.ELEMENT_MODE_APPENDTO );
	};
	 ...
	 *		kceditor.replaceAll( 'myClassName' );
	 *
	 *		// Selectively replace <textarea> elements, based on a custom evaluation function.
	 *		kceditor.replaceAll( function( textarea, config ) {
	 ...
	 *				<script>
	 *					// Replace all three <textarea> elements above with KCEditor instances.
	 *					kceditor.replaceAll();
	 *				</script>
	 *			</body>
	 *		</html>
	 ...
	 */
	kceditor.editor.prototype.addMode = function( mode, exec ) {
	 ...
	 *		kceditor.instances.editor1.setMode( 'source' );
	 *		// Switch to "wysiwyg" view and be notified on completion.
	 *		kceditor.instances.editor1.setMode( 'wysiwyg', function() { alert( 'wysiwyg mode loaded!' ); } );
	 *
	 * @param {String} [newMode] If not specified, the {@link kceditor.config#startupMode} will be used.
	 * @param {Function} [callback] Optional callback function which is invoked once the mode switch has succeeded.
	 */
	kceditor.editor.prototype.setMode = function( newMode, callback ) {
		var editor = this;
		...
		if ( editor.mode ) {
			...
			// https://dev.kceditor.com/ticket/5217#comment:20
			// Tested by:
	 ...
	 */
	kceditor.editor.prototype.resize = function( width, height, isContentHeight, resizeInner ) {
		var container = this.container,
			contents = this.ui.space( 'contents' ),
			contentsFrame = kceditor.env.webkit && this.document && this.document.getWindow().$.frameElement,
			outer;

		if ( resizeInner ) {
			outer = this.container.getFirst( function( node ) {
				return node.type == kceditor.NODE_ELEMENT && node.hasClass( 'cke_inner' );
			} );
		} else {
			outer = container;
		}
		...
		// Set as border box width. (https://dev.kceditor.com/ticket/5353)
		outer.setSize( 'width', width, true );

		// WebKit needs to refresh the iframe size to avoid rendering issues. (1/2) (https://dev.kceditor.com/ticket/8348)
		...
		contents.setStyle( 'height', kceditor.tools.cssLength( resultContentsHeight ) );

		// WebKit needs to refresh the iframe size to avoid rendering issues. (2/2) (https://dev.kceditor.com/ticket/8348)
	 ...
	 * is mainly used by the [Editor Resize](https://github.com/sallecta/kceditor/cke4/addon/resize) plugin, which adds
	 * a UI handle that can be used to resize the editor.
	 *
	 * @param {Boolean} forContents Whether to return the "contents" part of the theme instead of the container.
	 * @returns {kceditor.dom.element} The resizable element.
	 */
	kceditor.editor.prototype.getResizable = function( forContents ) {
		return forContents ? this.ui.space( 'contents' ) : this.container;
	};

	function convertCssUnitToPx( unit ) {
		return kceditor.tools.convertToPx( kceditor.tools.cssLength( unit ) );
	}

	function createInstance( element, config, data, mode ) {
		element = kceditor.editor._getEditorElement( element );
		...
		// (#4461)
		if ( kceditor.editor.shouldDelayEditorCreation( element, config ) ) {
			return kceditor.editor.initializeDelayedEditorCreation( element, config, 'replace' );
		}

		// Create the editor instance.
		var editor = new kceditor.editor( config, element, mode );

		if ( mode == kceditor.ELEMENT_MODE_REPLACE ) {
			// Do not replace the textarea right now, just hide it. The effective
			// replacement will be done later in the editor creation lifecycle.
			element.setStyle( 'visibility', 'hidden' );

			// https://dev.kceditor.com/ticket/8031 Remember if textarea was required and remove the attribute.
		...
		editor.on( 'loaded', function() {
			...
			if ( mode == kceditor.ELEMENT_MODE_REPLACE && editor.config.autoUpdateElement && element.$.form )
			...
			editor.setMode( editor.config.startupMode, function() {
				...
				kceditor.fire( 'instanceReady', null, editor );
			} );
		} );
	...
	function destroy() {
		...
		if ( element ) {
			...
			if ( editor.elementMode == kceditor.ELEMENT_MODE_REPLACE ) {
				element.show();
	...
	function loadTheme( editor ) {
		...
		var themedTpl = new kceditor.template(
			'<{outerEl}' +
				' id="cke_{name}"' +
				' class="{id} cke cke_reset cke_chrome cke_editor_{name} cke_{langDir} ' + kceditor.env.cssClass + '" ' +
				' dir="{langDir}"' +
				' lang="{langCode}"' +
				' role="application"' +
		...
		var container = kceditor.dom.element.createFromHtml( themedTpl.output( {
			id: editor.id,
			name: name,
			...
			outerEl: kceditor.env.ie ? 'span' : 'div'	// https://dev.kceditor.com/ticket/9571
		} ) );

		if ( elementMode == kceditor.ELEMENT_MODE_REPLACE ) {
			element.hide();
		...
		if ( width )
			container.setStyle( 'width', kceditor.tools.cssLength( width ) );

		// The editor height is applied to the contents space.
		if ( height )
			editor.ui.space( 'contents' ).setStyle( 'height', kceditor.tools.cssLength( height ) );

		// Disable browser context menu for editor's chrome.
		container.disableContextMenu();

		// Redirect the focus into editor for webkit. (https://dev.kceditor.com/ticket/5713)
		kceditor.env.webkit && container.on( 'focus', function() {
			editor.focus();
		} );
 ...
 *
 *		alert( kceditor.instances.editor1.mode ); // (e.g.) 'wysiwyg'
 *
 * @readonly
 ...
 */
kceditor.config.startupMode = 'wysiwyg';

/**
 * Fired after the editor instance is resized through
 * the {@link kceditor.editor#method-resize kceditor.resize} method.
 *
 * @event resize
 * @param {kceditor.editor} editor This editor instance.
 * @param {Object} data Available since KCEditor 4.5.0.
 ...
 * perform when KCEditor replaces `<textarea>` elements.
 *
 ...
 *
 * For example, it is used in the [Editor Resize](https://github.com/sallecta/kceditor/cke4/addon/resize) plugin
 * to add the HTML element used to resize the editor.
 *
 * @event uiSpace
 * @param {Object} data
 * @param {String} data.space The name of the {@link kceditor.ui#space space} for which the event is fired.
