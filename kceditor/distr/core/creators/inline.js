...
( function() {
	 ...
	 * KCEditor instance. Check {@link kceditor.dtd#$editable} for a list of
	 * allowed element names.
	 *
	 * **Note:** If the DOM element for which inline editing is being enabled does not have
	 * the `contenteditable` attribute set to `true` or the {@link kceditor.config#readOnly `config.readOnly`}
	 * configuration option is set to `true`, the editor will start in read-only mode.
	 *
	 *		<div contenteditable="true" id="content">...</div>
	 *		...
	 *		kceditor.inline( 'content' );
	 *
	 ...
	 *
	 * ```javascript
	 * var cancelInterval = kceditor.inline( 'editor', {
	 ...
	 */
	kceditor.inline = function( element, instanceConfig ) {
		...
		// (#4461)
		if ( kceditor.editor.shouldDelayEditorCreation( element, instanceConfig ) ) {
			return kceditor.editor.initializeDelayedEditorCreation( element, instanceConfig, 'inline' );
		}

		var textarea = element.is( 'textarea' ) ? element : null,
			editorData = textarea ? textarea.getValue() : element.getHtml(),
			editor = new kceditor.editor( instanceConfig, element, kceditor.ELEMENT_MODE_INLINE );

		if ( textarea ) {
			editor.setData( editorData, null, true );

			//Change element from textarea to div
			element = kceditor.dom.element.createFromHtml(
	...
	/**
	 * Calls the {@link kceditor#inline `kceditor.inline()`} method for all page elements with the `contenteditable` attribute set to
	 * `true` that are allowed in the {@link kceditor.dtd#$editable} object.
	 *
	 * Since 4.17 this function also supports the {@glink features/delayed_creation Delayed Editor Creation} feature
	 * allowing to postpone the editor initialization.
	 * Read more about this feature in the {@glink features/delayed_creation documentation}.
	 */
	kceditor.inlineAll = function() {
		var el,
			data;

		for ( var name in kceditor.dtd.$editable ) {
			var elements = kceditor.document.getElementsByTag( name );
			...
			for ( var i = 0, len = elements.count(); i < len; i++ ) {
				...
				if ( el.getAttribute( 'contenteditable' ) == 'true' && !el.getEditor() ) {
					...
					if ( kceditor.fire( 'inline', data ) !== false ) {
						kceditor.inline( el, data.config );
					}
				}
			}
		}
 ...
 *
 *		kceditor.disableAutoInline = true;
 *
