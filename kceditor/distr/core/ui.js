...
// PACKAGER_RENAME( kceditor.ui )

kceditor.ui.prototype = {
	/**
	 * Adds a UI item to the items collection. These items can be later used in
	 * the interface.
	 *
	 *		// Add a new button named 'MyBold'.
	 *		editorInstance.ui.add( 'MyBold', kceditor.UI_BUTTON, {
	 *			label: 'My Bold',
	 *			command: 'bold'
	 *		} );
	 *
	...
	add: function( name, type, definition ) {
		...
		var item = this.items[ name ] = {
			type: type,
			// The name of {@link kceditor.command} which associate with this UI.
			command: definition.command || null,
			args: Array.prototype.slice.call( arguments, 2 )
		};

		kceditor.tools.extend( item, definition );
	},
	 ...
	 * @returns {kceditor.dom.element} The element that represents the space.
	 */
	space: function( name ) {
		return kceditor.document.getById( this.spaceId( name ) );
	},
...
kceditor.event.implementOn( kceditor.ui );
 ...
 * passed to the {@link kceditor.ui#addHandler} function.
 * This class is not really a part of the API, so do not call its constructor.
 *
 * @class kceditor.ui.handlerDefinition
 */

/**
 * Transforms an item definition into a UI item object.
 *
 *		editorInstance.ui.addHandler( kceditor.UI_BUTTON, {
...
/**
 * The element in the {@link kceditor#document host page's document} that contains the editor content.
 ...
 * or {@link kceditor.editable} (in case of inline editor). Otherwise it is set to the {@link kceditor.editable} itself.
 *
 ...
 * @since 4.5.0
 * @readonly
 * @property {kceditor.dom.element} contentsElement
 */
