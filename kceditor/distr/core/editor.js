...
/**
 * @fileOverview Defines the {@link kceditor.editor} class that represents an
 *		editor instance.
 */

( function() {
	// Override the basic constructor defined at editor_basic.js.
	Editor.prototype = kceditor.editor.prototype;
	kceditor.editor = Editor;

	/**
	 * Represents an editor instance. This constructor should be rarely
	 * used in favor of the {@link kceditor} editor creation functions.
	 *
	 * @class kceditor.editor
	 ...
	 * @param {kceditor.dom.element} [element] The DOM element upon which this editor
	 * will be created.
	...
	function Editor( instanceConfig, element, mode ) {
		...
		// Make a clone of the config object, to avoid having it touched by our code. (https://dev.kceditor.com/ticket/9636)
		instanceConfig = instanceConfig && kceditor.tools.clone( instanceConfig );

		// if editor is created off one page element.
		if ( element !== undefined ) {
			// Asserting element and mode not null.
			if ( !( element instanceof kceditor.dom.element ) )
				throw new Error( 'Expect element of type kceditor.dom.element.' );
			else if ( !mode )
				throw new Error( 'One of the element modes must be specified.' );

			if ( kceditor.env.ie && kceditor.env.quirks && mode == kceditor.ELEMENT_MODE_INLINE )
			 ...
			 * See also {@link kceditor#ELEMENT_MODE_INLINE} and {@link kceditor#ELEMENT_MODE_REPLACE}.
			 *
			 * @readonly
			 * @property {Number}
			 */
			...
			this.name = ( this.elementMode != kceditor.ELEMENT_MODE_APPENDTO ) && ( element.getId() || element.getNameAtt() );
		} else {
			this.elementMode = kceditor.ELEMENT_MODE_NONE;
		}
		 ...
		 * @property {String}
		 */
		this.id = kceditor.tools.getNextId();
		 ...
		 *	* **loaded**: The editor components were loaded &mdash; see the {@link kceditor.editor#loaded} event.
		 *	* **recreating**: The editor editable area is recreating due to iframe reloading &mdash; see the {@link kceditor.config#observableParent} configuration option.
		 *	* **ready**: The editor is fully initialized and ready &mdash; see the {@link kceditor.editor#instanceReady} event.
		 *	* **destroyed**: The editor was destroyed &mdash; see the {@link kceditor.editor#method-destroy} method.
		 *
		 * @since 4.1.0
		 * @readonly
		 * @property {String}
		 ...
		 * settings defined in {@link kceditor.config}, combined with settings
		 ...
		 *
		 * @readonly
		 * @property {kceditor.config}
		 */
		this.config = kceditor.tools.prototypedCopy( kceditor.config );

		/**
		 * The namespace containing UI features related to this editor instance.
		 *
		 * @readonly
		 * @property {kceditor.ui}
		 */
		this.ui = new kceditor.ui( this );
		 ...
		 *
		 * @readonly
		 * @property {kceditor.focusManager}
		 */
		this.focusManager = new kceditor.focusManager( this );

		/**
		 * Controls keystroke typing in this editor instance.
		 *
		 * @readonly
		 * @property {kceditor.keystrokeHandler}
		 */
		this.keystrokeHandler = new kceditor.keystrokeHandler( this );
		...
		kceditor.dom.selection.setupEditorOptimization( this );
		...
		this.on( 'instanceReady', function() {
			if ( this.config.startupFocus ) {
				if ( this.config.startupFocus === 'end' ) {
					...
					range.shrink( kceditor.SHRINK_ELEMENT, true );
					range.collapse();
		...
		kceditor.fire( 'instanceCreated', null, this );

		// Add this new editor to the kceditor.instances collections.
		kceditor.add( this );
	...
	function isSupportedElement( element, mode ) {
		if ( mode == kceditor.ELEMENT_MODE_INLINE )
			return element.is( kceditor.dtd.$editable ) || element.is( 'textarea' );
		else if ( mode == kceditor.ELEMENT_MODE_REPLACE )
			return !element.is( kceditor.dtd.$nonBodyContent );
		return 1;
	}
	...
	function updateCommandsContext( editor, path, forceRefresh ) {
		...
		// For example when active filter is changed in https://dev.kceditor.com/ticket/10877.
		if ( !path )
			return;
	...
	// kceditor.editorConfig functions defined on them, so there is no need to
	...
	function loadConfig( editor ) {
		...
		customConfig = kceditor.getUrl( customConfig );

		var loadedConfig = loadConfigLoaded[ customConfig ] || ( loadConfigLoaded[ customConfig ] = {} );

		// If the custom config has already been downloaded, reuse it.
		if ( loadedConfig.fn ) {
			// Call the cached kceditor.editorConfig defined in the custom
			// config file for the editor instance depending on it.
			loadedConfig.fn.call( editor, editor.config );

			// If there is no other customConfig in the chain, fire the
			// "configLoaded" event.
			if ( kceditor.getUrl( editor.config.customConfig ) == customConfig || !loadConfig( editor ) ) {
			...
			// instead of scriptLoader#load (https://dev.kceditor.com/ticket/6504).
			kceditor.scriptLoader.queue( customConfig, function() {
				...
				loadedConfig.fn = loadedConfig.fn || kceditor.editorConfig || function() {};
	...
	function initConfig( editor, instanceConfig ) {
		...
		editor.on( 'customConfigLoaded', function() {
			if ( instanceConfig ) {
				...
				kceditor.tools.extend( editor.config, instanceConfig, true );
	...
	function onConfigLoaded( editor ) {
		 ...
		 * See also {@link kceditor.editor#setReadOnly}.
		 *
		 * @since 3.6.0
		 * @readonly
		 * @property {Boolean}
		 ...
		 * @property {Boolean}
		 */
		editor.blockless = editor.elementMode == kceditor.ELEMENT_MODE_INLINE ?
			!( editor.element.is( 'textarea' ) || kceditor.dtd[ editor.element.getName() ].p ) :
			false;

		/**
		 * The [tabbing navigation](http://en.wikipedia.org/wiki/Tabbing_navigation) order determined for this editor instance.
		 * This can be set by the <code>{@link kceditor.config#tabIndex}</code>
		...
		// Set kceditor.skinName. Note that it is not possible to have
		// different skins on the same page, so the last one to set it "wins".
		if ( config.skin )
			kceditor.skinName = config.skin;
	...
	function initComponents( editor ) {
		...
		editor.dataProcessor = new kceditor.htmlDataProcessor( editor );

		// Set activeFilter directly to avoid firing event.
		editor.filter = editor.activeFilter = new kceditor.filter( editor );

		loadSkin( editor );
	}

	function loadSkin( editor ) {
		kceditor.skin.loadPart( 'editor', function() {
			loadLang( editor );
		} );
	}

	function loadLang( editor ) {
		kceditor.lang.load( editor.config.language, editor.config.defaultLanguage, function( languageCode, lang ) {
			...
			editor.lang = kceditor.tools.prototypedCopy( lang );

			/**
			 * Indicates the human-readable title of this editor. Although this is a read-only property,
			 * it can be initialized with {@link kceditor.config#title}.
			 *
			 * **Note:** Please do not confuse this property with {@link kceditor.editor#name editor.name}
			 * which identifies the instance in the {@link kceditor#instances} literal.
			 *
			 * @since 4.2.0
			 * @readonly
			 ...
			 * it can be initialized with {@link kceditor.config#applicationTitle}.
			 *
			 * **Note:** Please do not confuse this property with {@link kceditor.editor#name editor.name}
			 * which identifies the literal instance in the {@link kceditor#instances}.
			 *
			 * @since 4.19.0
			 * @readonly
			 * @property {String/Boolean}
			...
			if ( !editor.config.contentsLangDirection ) {
				...
				editor.config.contentsLangDirection = editor.elementMode == kceditor.ELEMENT_MODE_INLINE ? editor.element.getDirection( 1 ) : editor.lang.dir;
			}
	...
	function loadPlugins( editor ) {
		...
		kceditor.env.air && ( plugins += ',adobeair' );

		// Load all plugins defined in the "plugins" setting.
		kceditor.plugins.load( plugins.split( ',' ), function( plugins ) {
			// The list of plugins.
			var pluginsArray = [];
			...
			editor.plugins = kceditor.tools.extend( {}, editor.plugins, plugins );
			...
			for ( var pluginName in plugins ) {
				...
				if ( kceditor.tools.isArray( requires ) )
					requires = requires.join( ',' );

				if ( requires && ( match = requires.match( removeRegex ) ) ) {
					while ( ( name = match.pop() ) ) {
						kceditor.error( 'editor-plugin-required', { plugin: name.replace( ',', '' ), requiredBy: pluginName } );
					}
				}
				...
				if ( pluginLangs && !editor.lang[ pluginName ] ) {
					...
					if ( kceditor.tools.indexOf( pluginLangs, editor.langCode ) >= 0 )
						...
						if ( langPart != editor.langCode && kceditor.tools.indexOf( pluginLangs, langPart ) >= 0 )
							lang = langPart;
						// Try the only "generic" option we have: English.
						else if ( kceditor.tools.indexOf( pluginLangs, 'en' ) >= 0 )
							lang = 'en';
						else
							lang = pluginLangs[ 0 ];
					}
					...
					if ( !plugin.langEntries || !plugin.langEntries[ lang ] ) {
						...
						// get downloaded.
						languageFiles.push( kceditor.getUrl( plugin.path + 'lang/' + lang + '.js' ) );
					} else {
			...
			kceditor.scriptLoader.load( languageFiles, function() {
				...
				kceditor.fire( 'instanceLoaded', null, editor );
			} );
		} );
		...
		function parsePluginsOption( option ) {
			...
			if ( kceditor.tools.isArray( option ) ) {
				option = option.join( ',' );
			}
	...
	function updateEditorElement() {
		...
		// associated element.
		if ( element && this.elementMode != kceditor.ELEMENT_MODE_APPENDTO ) {
			var data = this.getData();

			if ( this.config.htmlEncodeOutput )
				data = kceditor.tools.htmlEncode( data );
	...
	function validateEnterMode( editor, enterMode ) {
		return editor.blockless ? kceditor.ENTER_BR : enterMode;
	}

	// Create DocumentFragment from specified ranges. For now it handles only tables
	// and returns DocumentFragment from the 1. range for other cases. (https://dev.kceditor.com/ticket/13884)
	function createDocumentFragmentFromRanges( ranges, editable ) {
		var docFragment = new kceditor.dom.documentFragment(),
			tableClone,
			currentRow,
			currentRowClone;
	...
	kceditor.tools.extend( kceditor.editor.prototype, {
		 ...
		 * editor instance.
		 *
		 *		alert( editor.plugins.dialog.path ); // e.g. 'http://kceditor.com/kceditor/plugins/dialog/'
		 *
		 ...
		 *
		 * @readonly
		 * @property {kceditor.editor.plugins}
		 */
		plugins: {
			/**
			 * Checks the plugin for conflicts with other plugins.
			 *
			 * If a conflict occurs, this function will send a {@link kceditor#warn console warning}
			 ...
			 * ```
			 *
			 * @member kceditor.editor.plugins
			 * @since 4.10.1
			...
			detectConflict: function( plugin, conflicted ) {
				for ( var i = 0; i < conflicted.length; i++ ) {
					...
					if ( this[ pluginName ] ) {
						kceditor.warn( 'editor-plugin-conflict', {
							plugin: plugin,
		 ...
		 * 		} );
		 *
		 * Since 4.10.0 this method also accepts a {@link kceditor.command} instance as a parameter.
		 *
		 * @param {String} commandName The indentifier name of the command.
		 * @param {kceditor.commandDefinition/kceditor.command} commandDefinition The command definition or a `kceditor.command` instance.
		 */
		addCommand: function( commandName, commandDefinition ) {
			commandDefinition.name = commandName.toLowerCase();
			var cmd = commandDefinition instanceof kceditor.command ? commandDefinition : new kceditor.command( this, commandDefinition );
			...
			// before instanceReady. https://dev.kceditor.com/ticket/10103, https://dev.kceditor.com/ticket/10249
			if ( this.mode )
		 ...
		 * This method is called by both creators ({@link kceditor#replace replace} and
		 * {@link kceditor#inline inline}), so there is no reason to call it manually.
		 *
		 * @private
		 */
		_attachToForm: function() {
			var editor = this,
				...
				form = new kceditor.dom.element( element.$.form );
			...
			if ( element.is( 'textarea' ) ) {
				if ( form ) {
					...
					if ( isFunction( form.$.submit ) ) {
						...
						// "submit" event.
						form.$.submit = kceditor.tools.override( form.$.submit, function( originalSubmit ) {
							return function() {
								onSubmit();
					...
					// Remove 'submit' events registered on form element before destroying.(https://dev.kceditor.com/ticket/3988)
			...
			function onSubmit( evt ) {
				...
				// https://dev.kceditor.com/ticket/8031 If textarea had required attribute and editor is empty fire 'required' event and if
		 ...
		 *
		 *		alert( kceditor.instances.editor1 ); // e.g. object
		 *		kceditor.instances.editor1.destroy();
		 *		alert( kceditor.instances.editor1 ); // undefined
		 *
		...
		destroy: function( noUpdate ) {
			var filters = kceditor.filter.instances,
				self = this;
			...
			kceditor.tools.array.forEach( kceditor.tools.object.keys( filters ), function( id ) {
			...
			kceditor.remove( this );
			kceditor.fire( 'instanceDestroyed', null, this );
		},

		/**
		 * Returns an {@link kceditor.dom.elementPath element path} for the selection in the editor.
		 *
		 * @param {kceditor.dom.node} [startNode] From which the path should start,
		 * if not specified defaults to editor selection's
		 * start element yielded by {@link kceditor.dom.selection#getStartElement}.
		...
		elementPath: function( startNode ) {
			...
			return startNode ? new kceditor.dom.elementPath( startNode, this.editable() ) : null;
		},

		/**
		 * Shortcut to create a {@link kceditor.dom.range} instance from the editable element.
		 *
		 * @returns {kceditor.dom.range} The DOM range created if the editable has presented.
		 * @see kceditor.dom.range
		 */
		createRange: function() {
			var editable = this.editable();
			return editable ? new kceditor.dom.range( editable ) : null;
		},
		 ...
		 * @see kceditor.editor#addCommand
		 */
		execCommand: function( commandName, data ) {
			...
			if ( command && command.state != kceditor.TRISTATE_DISABLED ) {
		 ...
		 * {@link kceditor.command}, which will then be returned by this function.
		 *
		 * @param {String} commandName The name of the command to be returned.
		 * This is the same name that is used to register the command with `addCommand`.
		 * @returns {kceditor.command} The command object identified by the provided name.
		 */
		 ...
		 *
		 *		if ( kceditor.instances.editor1.getData() == '' )
		 *			alert( 'There is no data available.' );
		 *
		 * @param {Boolean} internal If set to `true`, it will prevent firing the
		 * {@link kceditor.editor#beforeGetData} and {@link kceditor.editor#event-getData} events, so
		...
		getData: function( internal ) {
			...
			if ( typeof eventData != 'string' ) {
				var element = this.element;
				if ( element && this.elementMode == kceditor.ELEMENT_MODE_REPLACE )
		 ...
		 * See also:
		 *
		 * * {@link kceditor.editor#method-getData}.
		 *
		...
		getSnapshot: function() {
			...
			if ( typeof data != 'string' ) {
				...
				if ( element && this.elementMode == kceditor.ELEMENT_MODE_REPLACE ) {
					...
					// as this method is expected to return a string. (https://dev.kceditor.com/ticket/13385)
					data = '';
				}
			}
		 ...
		 *
		 *		kceditor.instances.editor1.setData( '<p>This is the editor data.</p>' );
		 *
		 *		kceditor.instances.editor1.setData( '<p>Some other editor data.</p>', {
		 *			callback: function() {
		 *				this.checkDirty(); // true
		 *			}
		 *		} );
		 *
		 * Note: In **KCEditor 4.4.2** the signature of this method has changed. All arguments
		...
		setReadOnly: function( isReadOnly ) {
			...
			if ( this.readOnly != isReadOnly ) {
				...
				// state to prevent browser's history navigation (https://dev.kceditor.com/ticket/9761).
		 ...
		 * Example:
		 *
		 *		kceditor.instances.editor1.insertHtml( '<p>This is a new paragraph.</p>' );
		 *
		 ...
		 * * `'unfiltered_html'` &ndash; Like `'html'` but the content is not filtered with {@link kceditor.filter}.
		 ...
		 *
		 *		kceditor.instances.editor1.insertText( ' line1 \n\n line2' );
		 *
		 ...
		 *
		 *		var element = kceditor.dom.element.createFromHtml( '<img src="hello.png" border="0" title="Hello" />' );
		 *		kceditor.instances.editor1.insertElement( element );
		 *
		 ...
		 *
		 * @param {kceditor.dom.element} element The element to be inserted into the editor.
		 */
		...
		/**
		 * Gets the selected HTML (it is returned as a {@link kceditor.dom.documentFragment document fragment}
		 ...
		 * * the {@link kceditor.editable#getHtmlFromRange} method.
		 *
		 * @since 4.5.0
		 * @param {Boolean} [toString] If `true`, then stringified HTML will be returned.
		 * @returns {kceditor.dom.documentFragment/String}
		 */
		 ...
		 * * the {@link kceditor.editable#extractHtmlFromRange} method.
		 *
		 * @since 4.5.0
		 * @param {Boolean} [toString] If `true`, then stringified HTML will be returned.
		 ...
		 * @returns {kceditor.dom.documentFragment/String/null}
		 */
		extractSelectedHtml: function( toString, removeEmptyBlock ) {
			var editable = this.editable(),
				ranges = this.getSelection().getRanges(),
				docFragment = new kceditor.dom.documentFragment(),
				i;
		 ...
		 *			if ( kceditor.instances.editor1.checkDirty() )
		 ...
		 * with the {@link kceditor#ELEMENT_MODE_REPLACE} element mode or inline instances
		 * bound to `<textarea>` elements.
		 *
		 *		kceditor.instances.editor1.updateElement();
		 *		alert( document.getElementById( 'editor1' ).value ); // The current editor data.
		 *
		 * @see kceditor.editor#element
		 */
		 ...
		 *
		 *		editor.setKeystroke( kceditor.CTRL + 115, 'save' );	// Assigned Ctrl+S to the "save" command.
		 *		editor.setKeystroke( kceditor.CTRL + 115, false );	// Disabled Ctrl+S keystroke assignment.
		 *		editor.setKeystroke( [
		 *			[ kceditor.ALT + 122, false ],
		 *			[ kceditor.CTRL + 121, 'link' ],
		 *			[ kceditor.SHIFT + 120, 'bold' ]
		 *		] );
		 *
		 ...
		 * 2. Editor extends default keystrokes with {@link kceditor.config#keystrokes}.
		 * 3. Editor blocks keystrokes defined in {@link kceditor.config#blockedKeystrokes}.
		 *
		...
		setKeystroke: function() {
			var keystrokes = this.keystrokeHandler.keystrokes,
				newKeystrokes = kceditor.tools.isArray( arguments[ 0 ] ) ? arguments[ 0 ] : [ [].slice.call( arguments, 0 ) ],
				keystroke, behavior;
			...
			for ( var i = newKeystrokes.length; i--; ) {
				...
				if ( kceditor.tools.isArray( keystroke ) ) {
		...
		/**
		 * Returns the keystroke that is assigned to a specified {@link kceditor.command}. If no keystroke is assigned,
		 * it returns `null`.
		 *
		 * Since version 4.7.0 this function also accepts a `command` parameter as a string.
		 *
		 * @since 4.6.0
		 * @param {kceditor.command/String} command The {@link kceditor.command} instance or a string with the command name.
		...
		getCommandKeystroke: function( command, all ) {
			...
			if ( commandInstance ) {
				var commandName = kceditor.tools.object.findKey( this.commands, commandInstance ),
		...
		/**
		 * Shorthand for {@link kceditor.filter#addFeature}.
		 *
		 * @since 4.1.0
		 * @param {kceditor.feature} feature See {@link kceditor.filter#addFeature}.
		 * @returns {Boolean} See {@link kceditor.filter#addFeature}.
		 */
		 ...
		 *		var filter = new kceditor.filter( 'p strong em br' );
		 *		editor.setActiveFilter( filter );
		 *
		 * Setting a new filter will also change the {@link #setActiveEnterMode active Enter modes} to the first values
		 * allowed by the new filter (see {@link kceditor.filter#getAllowedEnterMode}).
		 *
		 * @since 4.3.0
		 * @param {kceditor.filter} filter Filter instance or a falsy value (e.g. `null`) to reset to the default one.
		 */
		 ...
		 *
		 * Prior to KCEditor 4.3.0 Enter modes were static and it was enough to check {@link kceditor.config#enterMode}
		 * and {@link kceditor.config#shiftEnterMode} when implementing a feature which should depend on the Enter modes.
		 * Since KCEditor 4.3.0 these options are source of initial:
		 *
		 ...
		 * For example, if selection is moved to the {@link kceditor.plugins.widget widget}'s nested editable which
		 * is a {@link #blockless blockless one}, then the active Enter modes should be changed to {@link kceditor#ENTER_BR}
		 * (in this case {@glink guide/dev_widgets Widget System} takes care of that).
		 *
		 * **Note:** This method should not be used to configure the editor &ndash; use {@link kceditor.config#enterMode} and
		 * {@link kceditor.config#shiftEnterMode} instead. This method should only be used to dynamically change
		 ...
		 * &mdash; see {@link kceditor.editor#blockless}) only {@link kceditor#ENTER_BR} is a valid Enter mode. Therefore
		 ...
		 *
		 * @since 4.3.0
		 * @param {Number} enterMode One of {@link kceditor#ENTER_P}, {@link kceditor#ENTER_DIV}, {@link kceditor#ENTER_BR}.
		 ...
		 *
		 * If the [Notification](https://github.com/sallecta/kceditor/cke4/addon/notification) plugin is not enabled, this function shows
		 ...
		 *
		 * See {@link kceditor.plugins.notification}.
		 *
		 * @since 4.5.0
		 ...
		 * needs to close it manually. See {@link kceditor.plugins.notification#duration}.
		 * Note that `warning` notifications will not be closed automatically.
		 * @returns {kceditor.plugins.notification} Created and shown notification.
		 */
		...
		/**
		 * Provides information whether the editor's {@link kceditor.editor#container container}
		 * {@link kceditor.dom.element#isDetached is detached}.
		 *
		 * @since 4.13.0
	 ...
	 * @since 4.12.0
	 * @static
	 * @param {String/kceditor.dom.element} elementOrId
	 * @member kceditor.editor
	 * @returns {kceditor.dom.element/null}
	 */
	kceditor.editor._getEditorElement = function( elementOrId ) {
		if ( !kceditor.env.isCompatible ) {
			return null;
		}

		var element = kceditor.dom.element.get( elementOrId );

		// Throw error on missing target element.
		if ( !element ) {
			kceditor.error( 'editor-incorrect-element', {
				element: elementOrId
			} );
		...
		if ( element.getEditor() ) {
			kceditor.error( 'editor-element-conflict', {
	 ...
	 *
	 * If the {@link kceditor.config#delayIfDetached_callback} function is declared, it will be invoked with a single argument:
	 *
	 ...
	 *
	 * ```js
	 *	kceditor.inline( detachedEditorElement, {
	 ...
	 * @static
	 * @member kceditor.editor
	 * @param {kceditor.dom.element} element The DOM element on which editor should be initialized.
	 * @param {Object} config The specific configuration to apply to the editor instance.
	 * Configuration set here will override the global KCEditor settings.
	 ...
	 * or null if {@link kceditor.config#delayIfDetached_callback} option is set.
	 */
	kceditor.editor.initializeDelayedEditorCreation = function( element, config, editorCreationMethod ) {
		if ( config.delayIfDetached_callback ) {
			kceditor.warn( 'editor-delayed-creation', {
				method: 'callback'
			} );

			config.delayIfDetached_callback( function() {
				kceditor[ editorCreationMethod ]( element, config );

				kceditor.warn( 'editor-delayed-creation-success', {
					method: 'callback'
				} );
			} );
		...
		var interval = config.delayIfDetached_interval === undefined ?
			kceditor.config.delayIfDetached_interval
	 ...
	 */
	kceditor.editor.shouldDelayEditorCreation = function( element, config ) {
		kceditor.editor.mergeDelayedCreationConfigs( config );
		return config && config.delayIfDetached && element.isDetached();
	};

	/**
	 * Merges user provided configuration options for delayed creation with {@link kceditor.config default config}.
	 *
	 ...
	 */
	kceditor.editor.mergeDelayedCreationConfigs = function( userConfig ) {
		if ( !userConfig ) {
			return;
		}

		userConfig.delayIfDetached = typeof userConfig.delayIfDetached === 'boolean' ? userConfig.delayIfDetached : kceditor.config.delayIfDetached;
		userConfig.delayIfDetached_interval = isNaN( userConfig.delayIfDetached_interval ) ? kceditor.config.delayIfDetached_interval : userConfig.delayIfDetached_interval;
		userConfig.delayIfDetached_callback = userConfig.delayIfDetached_callback || kceditor.config.delayIfDetached_callback;
	};
 ...
 * @member kceditor
 */
kceditor.ELEMENT_MODE_REPLACE = 1;
 ...
 * @member kceditor
 */
kceditor.ELEMENT_MODE_APPENDTO = 2;
 ...
 * @member kceditor
 */
kceditor.ELEMENT_MODE_INLINE = 3;
 ...
 * @member kceditor.config
 * @see kceditor.editor#setReadOnly
 */
 ...
 *
 * Since KCEditor 4.9.0, `startupFocus` can be explicitly set to either the `start` or the `end`
 * of the editable:
 *
 *		// Focus at the beginning of the editable.
...
/**
 * Customizes the {@link kceditor.editor#title human-readable title} of this editor. This title is displayed in
 ...
 *
 * **Note:** Please do not confuse this property with {@link kceditor.editor#name}
 * which identifies the instance in the {@link kceditor#instances} literal.
 *
 ...
 *
 * See also:
 * * kceditor.editor#title
 * * kceditor.editor#name
 * * kceditor.editor#applicationTitle
 * * kceditor.config#applicationTitle
 *
 * @since 4.2.0
...
/**
 * Customizes the {@link kceditor.editor#applicationTitle human-readable title} of the application for this
 ...
 * to {@link kceditor#replace}, {@link kceditor#inline}, and other creators.
 *
 ...
 *
 * @readonly
 * @property {kceditor.dom.element} container
 */
 ...
 * * For the inline editor it is equal to {@link kceditor#document}.
 *
 ...
 *
 * @readonly
 * @property {kceditor.dom.document} document
 */
 ...
 *
 * @readonly
 * @property {kceditor.dom.window} window
 */
 ...
 *
 * It points to a {@link kceditor.filter} instance set up based on
 * editor configuration.
 *
 * @since 4.1.0
 * @readonly
 * @property {kceditor.filter} filter
 */
 ...
 * {@link kceditor.feature features} can be enabled.
 *
 ...
 * @since 4.3.0
 * @readonly
 * @property {kceditor.filter} activeFilter
 */

/**
 * The main (static) Enter mode which is a validated version of the {@link kceditor.config#enterMode} setting.
 * Currently only one rule exists &mdash; {@link #blockless blockless editors} may have
 * Enter modes set only to {@link kceditor#ENTER_BR}.
 *
 * @since 4.3.0
 * @readonly
...
/**
 * Event fired when a kceditor instance is created, but still before initializing it.
 * To interact with a fully initialized instance, use the
 * {@link kceditor#instanceReady} event instead.
 *
 * @event instanceCreated
 * @member kceditor
 * @param {kceditor.editor} editor The editor instance that has been created.
 */

/**
 * Event fired when kceditor instance's components (configuration, languages and plugins) are fully
 * loaded and initialized. However, the editor will be fully ready for interaction
 * on {@link kceditor#instanceReady}.
 *
 * @event instanceLoaded
 * @member kceditor
 * @param {kceditor.editor} editor This editor instance that has been loaded.
 */

/**
 * Event fired when a kceditor instance is destroyed.
 *
 * @event instanceDestroyed
 * @member kceditor
 * @param {kceditor.editor} editor The editor instance that has been destroyed.
 */

/**
 * Event fired when a kceditor instance is created, fully initialized and ready for interaction.
 *
 * @event instanceReady
 * @member kceditor
 * @param {kceditor.editor} editor The editor instance that has been created.
 ...
 * @param {kceditor.command} data.command The command itself.
 */
 ...
 * {@link kceditor.config#customConfig} setting. Several files can be loaded
...
/**
 * Event fired when the kceditor instance is completely created, fully initialized
 ...
 * @param {kceditor.dom.range} [data.range] See {@link #method-insertHtml}'s `range` parameter.
 */
 ...
 * @event insertElement
 * @param {kceditor.editor} editor This editor instance.
 * @param {kceditor.dom.element} data The element to insert.
 */

/**
 * Event fired after data insertion using the {@link #method-insertHtml}, {@link kceditor.editable#insertHtml},
 * or {@link kceditor.editable#insertHtmlIntoRange} methods.
 *
 * @since 4.5.0
 * @event afterInsertHtml
 * @param data
 * @param {kceditor.dom.range} [data.intoRange] If set, the HTML was not inserted into the current selection, but into
 * the specified range. This property is set if the {@link kceditor.editable#insertHtmlIntoRange} method was used,
 * but not if for the {@link kceditor.editable#insertHtml} method.
 */
 ...
 * the {@link kceditor.editable} is initialized.
 *
 ...
 * (by {@link kceditor.editor#method-setData}) content DOM structure
 ...
 *
 * **Note:** {@link kceditor.editable#attachListener} is a convenient
 ...
 *
 * For more control over the entire process refer to {@link kceditor.config#delayIfDetached_callback}
 * and {@link kceditor.config#delayIfDetached_interval} configuration options.
 *
 ...
 * @member kceditor.config
 */
kceditor.config.delayIfDetached = false;
 ...
 *
 * Used only if {@link kceditor.config#delayIfDetached} is set to `true`.
 *
 * **Note**: This function (`callback`) should be called only if editor target element is reattached to DOM.
 *
 * If this option is defined, editor will not run the default {@link kceditor.config#delayIfDetached_interval interval checks}.
 *
 ...
 * @member kceditor.config
 */
kceditor.config.delayIfDetached_callback = undefined;

/**
 * The amount of time (in milliseconds) between consecutive checks whether editor's target element is attached to DOM.
 *
 * Used only if {@link kceditor.config#delayIfDetached} is set to `true` and
 * {@link kceditor.config#delayIfDetached_callback delayIfDetached_callback} not set.
 *
 ...
 * @member kceditor.config
 */
kceditor.config.delayIfDetached_interval = 50;
