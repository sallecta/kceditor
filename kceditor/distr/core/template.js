...
/**
 * @fileOverview Defines the {@link kceditor.template} class, which represents
...
( function() {
	 ...
	 *		var tpl = new kceditor.template( '<div class="{cls}">{label}</div>' );
	 *		alert( tpl.output( { cls: 'cke-label', label: 'foo'} ) ); // '<div class="cke-label">foo</div>'
	 *
	 *		// Since 4.12.0 it is possible to pass a callback function that returns a template.
	 *		var tpl2 = new kceditor.template( function( data ) {
	 ...
	 */
	kceditor.template = function( source ) {
		/**
	 ...
	 */
	kceditor.template.prototype.output = function( data, buffer ) {
