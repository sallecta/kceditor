...
/**
 * @fileOverview Defines the {@link kceditor.loader} objects, which is used to
 *		load core scripts and their dependencies from _source.
 */

if ( typeof kceditor == 'undefined' )
	kceditor = {}; // jshint ignore:line
...
if ( !kceditor.loader ) {
	...
	kceditor.loader = ( function() {
		...
		timestamp = ( kceditor && kceditor.timestamp ) || ( new Date() ).valueOf();		// %REMOVE_LINE%

		var getUrl = function( resource ) {
				if ( kceditor && kceditor.getUrl )
					return kceditor.getUrl( resource );

				return kceditor.basePath + resource + ( resource.indexOf( '?' ) >= 0 ? '&' : '?' ) + 't=' + timestamp;
			};
		...
		return {
			 ...
			 *		alert( kceditor.loader.loadedScripts );
			 */
			loadedScripts: [],
			/**
			 * Table of script names and their dependencies.
			...
			loadPending: function() {
				...
				function onScriptLoaded() {
					...
					kceditor.loader.loadedScripts.push( scriptName );

					// Load the next.
					kceditor.loader.loadPending();
				}

				// We must guarantee the execution order of the scripts, so we
				// need to load them one by one. (https://dev.kceditor.com/ticket/4145)
				...
				if ( typeof script.onreadystatechange !== 'undefined' ) {
					...
					script.onload = function() {
						...
						// immediately. Which will break the loading sequence. (https://dev.kceditor.com/ticket/3661)
						setTimeout( function() {
			 ...
			 *
			 *		kceditor.loader.load( 'dom/element' );
			 *
...
if ( kceditor._autoLoad ) {
	kceditor.loader.load( kceditor._autoLoad );
	delete kceditor._autoLoad;
}
