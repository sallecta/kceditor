 ...
 * Block style type.
 *
 * Read more in the {@link kceditor.style} class documentation.
 *
 * @readonly
 * @property {Number} [=1]
 * @member kceditor
 */
kceditor.STYLE_BLOCK = 1;
...
( function() {
	...
	var notBookmark = kceditor.dom.walker.bookmark( 0, 1 ),
		nonWhitespaces = kceditor.dom.walker.whitespaces( 1 );
	 ...
	 * which can be applied to and removed from a {@link kceditor.dom.selection selection} through
	 * {@link kceditor.editor editor} methods: {@link kceditor.editor#applyStyle} and {@link kceditor.editor#removeStyle},
	 * respectively.
	 *
	 * Three default style types are available: {@link kceditor#STYLE_BLOCK STYLE_BLOCK}, {@link kceditor#STYLE_INLINE STYLE_INLINE},
	 * and {@link kceditor#STYLE_OBJECT STYLE_OBJECT}. Based on its type, a style heavily changes its behavior.
	 ...
	 *		var style = new kceditor.style( { element: 'h1' } );
	 *
	 ...
	 *		var style = new kceditor.style( { element: 'img', attributes: { 'class': 'foo' } } );
	 *
	 *		// Considering the following selection:
	 ...
	 *
	 * ### API changes introduced in KCEditor 4.4.0
	 *
	 * Before KCEditor 4.4.0 all style instances had no access at all to the {@link kceditor.editor editor instance}
	 ...
	 * This design decision came from KCEditor 3; it started causing problems and became an unsolvable obstacle for
	 * the {@link kceditor.style.customHandlers.widget widget style handler} which we introduced in KCEditor 4.4.
	 *
	 ...
	 * implemented {@link kceditor.style.customHandlers.widget widget style handler}'s methods will not fail,
	 * although they will also not work by aborting at an early stage.
	 *
	 * Therefore, you can safely upgrade to KCEditor 4.4.0 even if you use style methods without providing
	 ...
	 * @param {kceditor.style.definition} styleDefinition
	 * @param variablesValues
	 */
	kceditor.style = function( styleDefinition, variablesValues ) {
		if ( typeof styleDefinition.type == 'string' )
			return new kceditor.style.customHandlers[ styleDefinition.type ]( styleDefinition );

		// Inline style text as attribute should be converted
		// to styles object.
		var attrs = styleDefinition.attributes;
		if ( attrs && attrs.style ) {
			styleDefinition.styles = kceditor.tools.extend( {},
				styleDefinition.styles, kceditor.tools.parseCssText( attrs.style ) );
			delete attrs.style;
		}

		if ( variablesValues ) {
			styleDefinition = kceditor.tools.clone( styleDefinition );
		...
		this.type = styleDefinition.type ||
			(
				blockElements[ element ] ? kceditor.STYLE_BLOCK :
				objectElements[ element ] ? kceditor.STYLE_OBJECT :
				kceditor.STYLE_INLINE
			);

		// If the 'element' property is an object with a set of possible element, it will be applied like an object style: only to existing elements
		if ( typeof this.element == 'object' )
			this.type = kceditor.STYLE_OBJECT;
	...
	kceditor.style.prototype = {
		 ...
		 * {@link kceditor.editor#applyStyle} method, which is a shorthand for this method.
		 *
		 * @param {kceditor.editor/kceditor.dom.document} editor The editor instance in which
		 * the style will be applied.
		 * A {@link kceditor.dom.document} instance is accepted for backward compatibility
		 * reasons, although since KCEditor 4.4.0 this type of argument is deprecated. Read more about
		 * the signature change in the {@link kceditor.style} documentation.
		 */
		apply: function( editor ) {
			// Backward compatibility.
			if ( editor instanceof kceditor.dom.document )
		 ...
		 * {@link kceditor.editor#removeStyle} method, which is a shorthand for this method.
		 *
		 * @param {kceditor.editor/kceditor.dom.document} editor The editor instance in which
		...
		remove: function( editor ) {
			...
			if ( this.checkApplicable( editor.elementPath(), editor ) ) {
				...
				// Before KCEditor 4.4.0 style knew nothing about editor, so in order to provide enterMode
				// which should be used developers were forced to hack the style object (see https://dev.kceditor.com/ticket/10190).
				// Since KCEditor 4.4.0 style knows about editor (at least when it's being applied/removed), but we
		 ...
		 * you probably want to use {@link kceditor.editor#applyStyle}.
		 *
		 * @param {kceditor.dom.range} range
		 * @param {kceditor.editor} editor The editor instance. Required argument since
		 * KCEditor 4.4. The style system will work without it, but it is highly
		 * recommended to provide it for integration with all features.  Read more about
		 * the signature change in the {@link kceditor.style} documentation.
		 */
		applyToRange: function( range ) {
			this.applyToRange =
				this.type == kceditor.STYLE_INLINE ? applyInlineStyle :
				this.type == kceditor.STYLE_BLOCK ? applyBlockStyle :
				this.type == kceditor.STYLE_OBJECT ? applyObjectStyle :
				null;
		 ...
		 * you probably want to use {@link kceditor.editor#removeStyle}.
		 *
		 * @param {kceditor.dom.range} range
		 * @param {kceditor.editor} editor The editor instance. Required argument since
		 * KCEditor 4.4. The style system will work without it, but it is highly
		...
		removeFromRange: function( range ) {
			this.removeFromRange =
				this.type == kceditor.STYLE_INLINE ? removeInlineStyle :
				this.type == kceditor.STYLE_BLOCK ? removeBlockStyle :
				this.type == kceditor.STYLE_OBJECT ? removeObjectStyle :
				null;
		 ...
		 *
		 * See {@link kceditor.editor#applyStyle}.
		 *
		 ...
		 *
		 * @param {kceditor.dom.elementPath} elementPath
		 * @param {kceditor.editor} editor The editor instance. Required argument since
		 * KCEditor 4.4. The style system will work without it, but it is highly
		 * recommended to provide it for integration with all features. Read more about
		 * the signature change in the {@link kceditor.style} documentation.
			...
			switch ( this.type ) {
				...
				case kceditor.STYLE_INLINE:
					...
					for ( var i = 0, element; i < elements.length; i++ ) {
						...
						if ( this.type == kceditor.STYLE_INLINE && ( element == elementPath.block || element == elementPath.blockLimit ) )
							continue;

						if ( this.type == kceditor.STYLE_OBJECT ) {
		 ...
		 *
		 * @param {kceditor.dom.elementPath} elementPath The elements path to
		 * check the style against.
		 * @param {kceditor.editor} editor The editor instance. Required argument since
		 * KCEditor 4.4. The style system will work without it, but it is highly
		 * recommended to provide it for integration with all features. Read more about
		 * the signature change in the {@link kceditor.style} documentation.
		 * @param {kceditor.filter} [filter] If defined, the style will be
		...
		checkApplicable: function( elementPath, editor, filter ) {
			// Backward compatibility.
			if ( editor && editor instanceof kceditor.filter )
				filter = editor;
		...
		buildPreview: function( label ) {
			...
			var cssStyle = kceditor.style.getStyleText( styleDefinition );
			if ( cssStyle )
		...
		/**
		 * If defined (for example by {@link kceditor.style#addCustomHandler custom style handler}), it returns
		 * the {@link kceditor.filter.allowedContentRules allowed content rules} which should be added to the
		 * {@link kceditor.filter} when enabling this style.
		 *
		 * **Note:** This method is not defined in the {@link kceditor.style} class.
		 *
		 * @since 4.4.0
		 * @method toAllowedContentRules
		 * @param {kceditor.editor} [editor] The editor instance.
		 * @returns {kceditor.filter.allowedContentRules} The rules that should represent this style in the {@link kceditor.filter}.
		 */
	};
	 ...
	 */
	kceditor.style.getStyleText = function( styleDefinition ) {
		...
		for ( var style in stylesDef ) {
			...
			// Some browsers don't support 'inherit' property value, leave them intact. (https://dev.kceditor.com/ticket/5242)
			if ( styleVal == 'inherit' )
		...
		if ( stylesText.length )
			stylesText = kceditor.tools.normalizeCssText( stylesText, true );
	...
	/**
	 * Namespace containing custom style handlers added with {@link kceditor.style#addCustomHandler}.
	 *
	 * @since 4.4.0
	 * @class
	 * @singleton
	 */
	kceditor.style.customHandlers = {};
	 ...
	 * @member kceditor.style
	*/
	kceditor.style.unstylableElements = [];

	/**
	 * Creates a {@link kceditor.style} subclass and registers it in the style system.
	 * Registered class will be used as a handler for a style of this type. This allows
	 * to extend the styles system, which by default uses only the {@link kceditor.style}, with
	 * new functionality. Registered classes are accessible in the {@link kceditor.style.customHandlers}.
	 *
	 * ### The Style Class Definition
	 *
	 * The definition object is used to override properties in a prototype inherited
	 * from the {@link kceditor.style} class. It must contain a `type` property which is
	 * a name of the new type and therefore it must be unique. The default style types
	 * ({@link kceditor#STYLE_BLOCK STYLE_BLOCK}, {@link kceditor#STYLE_INLINE STYLE_INLINE},
	 * and {@link kceditor#STYLE_OBJECT STYLE_OBJECT}) are integers, but for easier identification
	 ...
	 * custom style should be assigned. It defaults to the {@link kceditor#STYLE_OBJECT}.
	 *
	 * Other properties of the definition object will just be used to extend the prototype inherited
	 * from the {@link kceditor.style} class. So if the definition contains an `apply` method, it will
	 * override the {@link kceditor.style#apply} method.
	 *
	 * ### Usage
	 *
	 * Registering a basic handler:
	 *
	 *		var styleClass = kceditor.style.addCustomHandler( {
	 *			type: 'custom'
	 *		} );
	 *
	 *		var style = new styleClass( { ... } );
	 *		style instanceof styleClass; // -> true
	 *		style instanceof kceditor.style; // -> true
	 *		style.type; // -> 'custom'
	 *
	 * The {@link kceditor.style} constructor used as a factory:
	 *
	 *		var styleClass = kceditor.style.addCustomHandler( {
	 *			type: 'custom'
	 *		} );
	 ...
	 *		var style = new kceditor.style( { type: 'custom', attributes: ... } );
	 *		style instanceof styleClass; // -> true
	 *
	 * Thanks to that, integration code using styles does not need to know
	 * which style handler it should use. It is determined by the {@link kceditor.style} constructor.
	 *
	 * Overriding existing {@link kceditor.style} methods:
	 *
	 *		var styleClass = kceditor.style.addCustomHandler( {
	 *			type: 'custom',
	 *			apply: function( editor ) {
	 ...
	 *
	 *		style = new kceditor.style( { element: 'img', attributes: { 'class': 'foo' } } );
	 ...
	 * about DOM and especially {@link kceditor.dom.range ranges} and {@link kceditor.dom.walker DOM walker} it is impossible
	 ...
	 * be found in the [widget plugin](https://github.com/kceditor/kceditor4/blob/master/plugins/widget/plugin.js).
	 *
	 * When implementing a style handler from scratch at least the following methods must be defined:
	 *
	 * * {@link kceditor.style#apply apply} and {@link kceditor.style#remove remove},
	 * * {@link kceditor.style#checkElementRemovable checkElementRemovable} and
	 * {@link kceditor.style#checkElementMatch checkElementMatch} &ndash; Note that both methods reuse the same logic,
	 * * {@link kceditor.style#checkActive checkActive} &ndash; Reuses
	 * {@link kceditor.style#checkElementMatch checkElementMatch},
	 * * {@link kceditor.style#toAllowedContentRules toAllowedContentRules} &ndash; Not required, but very useful in
	 * case of a custom style that has to notify the {@link kceditor.filter} which rules it allows when registered.
	 *
	 * @since 4.4.0
	 * @static
	 * @member kceditor.style
	 * @param definition The style class definition.
	 * @returns {kceditor.style} The new style class created for the provided definition.
	 */
	kceditor.style.addCustomHandler = function( definition ) {
		...
		styleClass.prototype = kceditor.tools.extend(
			// Prototype of kceditor.style.
			kceditor.tools.prototypedCopy( kceditor.style.prototype ),
			// Defaults.
			{
				assignedTo: kceditor.STYLE_OBJECT
			},
	...
	var posPrecedingIdenticalContained =
			kceditor.POSITION_PRECEDING | kceditor.POSITION_IDENTICAL | kceditor.POSITION_IS_CONTAINED,
		posFollowingIdenticalContained =
			kceditor.POSITION_FOLLOWING | kceditor.POSITION_IDENTICAL | kceditor.POSITION_IS_CONTAINED;
	...
	function checkIfStyleCanBeChildOf( def, currentParent, elementName, isUnknownElement ) {
		return currentParent &&
			( ( currentParent.getDtd() || kceditor.dtd.span )[ elementName ] || isUnknownElement ) &&
			( !def.parentRule || def.parentRule( currentParent ) );
	}

	function checkIfStartsRange( nodeName, currentNode, lastNode ) {
		return (
			!nodeName || !kceditor.dtd.$removeEmpty[ nodeName ] ||
	...
	function checkIfTextOrReadonlyOrEmptyElement( currentNode, nodeIsReadonly ) {
		...
		return nodeType == kceditor.NODE_TEXT || nodeIsReadonly || ( nodeType == kceditor.NODE_ELEMENT && !currentNode.getChildCount() );
	}
	...
	function applyInlineStyle( range ) {
		...
		if ( range.collapsed ) {
			...
			range.moveToPosition( collapsedElement, kceditor.POSITION_BEFORE_END );
		...
		var dtd = kceditor.dtd[ elementName ];
		if ( !dtd ) {
			isUnknownElement = true;
			dtd = kceditor.dtd.span;
		}
		...
		if ( currentNode.getPosition( lastNode ) == kceditor.POSITION_FOLLOWING )
			currentNode = 0;
		...
		while ( currentNode ) {
			...
			if ( currentNode.equals( lastNode ) ) {
				...
				var nodeName = currentNode.type == kceditor.NODE_ELEMENT ? currentNode.getName() : null,
					nodeIsReadonly = nodeName && ( currentNode.getAttribute( 'contentEditable' ) == 'false' ),
					nodeIsUnstylable = nodeName &&
						kceditor.tools.array.indexOf( kceditor.style.unstylableElements, nodeName ) !== -1,
					nodeIsNoStyle = nodeName && ( currentNode.getAttribute( 'data-nostyle' ) || nodeIsUnstylable );

				// Skip bookmarks or comments.
				if ( ( nodeName && currentNode.data( 'cke-bookmark' ) ) || currentNode.type === kceditor.NODE_COMMENT ) {
					currentNode = currentNode.getNextSourceNode( true );
					continue;
				}

				// Find all nested editables of a non-editable block and apply this style inside them.
				if ( nodeIsReadonly && includeReadonly && kceditor.dtd.$block[ nodeName ] )
			...
			if ( applyStyle && styleRange && !styleRange.collapsed ) {
				...
				// Style already inherit from parents, left just to clear up any internal overrides. (https://dev.kceditor.com/ticket/5931)
				else {
					styleNode = new kceditor.dom.element( 'span' );
					styleRange.extractContents().appendTo( styleNode );
					styleRange.insertNode( styleNode );
					removeFromInsideElement.call( this, styleNode );
		...
		// Minimize the result range to exclude empty text nodes. (https://dev.kceditor.com/ticket/5374)
	...
	function removeInlineStyle( range ) {
		...
		if ( range.collapsed ) {
			var startPath = new kceditor.dom.elementPath( startNode.getParent(), range.root ),
			...
			for ( var i = 0, element; i < startPath.elements.length && ( element = startPath.elements[ i ] ); i++ ) {
				...
				//  also make sure other inner styles were well preserved.(https://dev.kceditor.com/ticket/3309)
				//
				// 3. Force removing the element even if it's an boundary element when alwaysRemoveElement is true.
				// Without it, the links won't be unlinked if the cursor is placed right before/after it. (https://dev.kceditor.com/ticket/13062)
				...
				if ( this.checkElementRemovable( element ) ) {
					...
					if ( !alwaysRemoveElement && range.collapsed && ( range.checkBoundaryOfElement( element, kceditor.END ) || ( isStart = range.checkBoundaryOfElement( element, kceditor.START ) ) ) ) {
			...
			while ( !currentNode.equals( endNode ) ) {
				...
				if ( currentNode.type == kceditor.NODE_ELEMENT && this.checkElementRemovable( currentNode ) ) {
					...
					if ( nextNode.type == kceditor.NODE_ELEMENT && nextNode.contains( startNode ) ) {
						breakNodes();
		...
		// and endNode.
		function breakNodes() {
			var startPath = new kceditor.dom.elementPath( startNode.getParent() ),
				endPath = new kceditor.dom.elementPath( endNode.getParent() ),
				breakStart = null,
				breakEnd = null;
	...
	// @param {kceditor.dom.element} editablesContainer
	// @context kceditor.style
	function applyStyleOnNestedEditables( editablesContainer ) {
		var editables = findNestedEditables( editablesContainer ),
			editable,
			l = editables.length,
			i = 0,
			range = l && new kceditor.dom.range( editablesContainer.getDocument() );
	...
	function checkIfAllowedInEditable( editable, style ) {
		var filter = kceditor.filter.instances[ editable.data( 'cke-filter' ) ];
	...
	function applyObjectStyle( range ) {
		// Selected or parent element. (https://dev.kceditor.com/ticket/9651)
		var start = range.getEnclosedNode() || range.getCommonAncestor( false, true ),
			element = new kceditor.dom.elementPath( start, range.root ).contains( this.element, 1 );
	...
	function removeObjectStyle( range ) {
		var parent = range.getCommonAncestor( true, true ),
			element = new kceditor.dom.elementPath( parent, range.root ).contains( this.element, 1 );
	...
	function applyBlockStyle( range ) {
		...
		// make recognize <br /> tag as a separator in ENTER_BR mode (https://dev.kceditor.com/ticket/5121)
		if ( this._.enterMode )
			iterator.enlargeBr = ( this._.enterMode != kceditor.ENTER_BR );
	...
	function removeBlockStyle( range ) {
		...
		while ( ( block = iterator.getNextParagraph() ) ) {
			if ( this.checkElementRemovable( block ) ) {
				...
				if ( block.is( 'pre' ) ) {
					newBlock = this._.enterMode == kceditor.ENTER_BR ? null :
							range.document.createElement( this._.enterMode == kceditor.ENTER_P ? 'p' : 'div' );
	...
	// when necessary. (https://dev.kceditor.com/ticket/3188)
	...
	function mergePre( preBlock ) {
		var previousBlock;
		if ( !( ( previousBlock = preBlock.getPrevious( nonWhitespaces ) ) && previousBlock.type == kceditor.NODE_ELEMENT && previousBlock.is( 'pre' ) ) )
			return;
	...
	function fromPres( preHtmls, newBlock ) {
		...
		if ( preHtmls.length > 1 )
			docFrag = new kceditor.dom.documentFragment( newBlock.getDocument() );
		...
		for ( var i = 0; i < preHtmls.length; i++ ) {
			...
			blockHtml = replace( blockHtml, /^[ \t]+|[ \t]+$/g, function( match, offset ) {
				if ( match.length == 1 ) // one space, preserve it
					...
					return kceditor.tools.repeat( '&nbsp;', match.length - 1 ) + ' ';
				else // end of block
					return ' ' + kceditor.tools.repeat( '&nbsp;', match.length - 1 );
			} );
	...
	function removeFromElement( element, keepDataAttrs ) {
		var def = this._.definition,
			...
			removeEmpty = kceditor.tools.isEmpty( attributes ) && kceditor.tools.isEmpty( styles );

		// Remove definition attributes/style from the elemnt.
		for ( var attName in attributes ) {
			// The 'class' element value must match (https://dev.kceditor.com/ticket/1318).
			if ( ( attName == 'class' || this._.definition.fullMatch ) && element.getAttribute( attName ) != normalizeProperty( attName, attributes[ attName ] ) )
				continue;

			// Do not touch data-* attributes (https://dev.kceditor.com/ticket/11011) (https://dev.kceditor.com/ticket/11258).
		...
		for ( var styleName in styles ) {
			// Full match style insist on having fully equivalence. (https://dev.kceditor.com/ticket/5018)
		...
		if ( removeEmpty ) {
			if ( this._.definition.alwaysRemoveElement )
				...
				if ( !kceditor.dtd.$block[ element.getName() ] || this._.enterMode == kceditor.ENTER_BR && !element.hasAttributes() )
					removeNoAttribsElement( element );
				else
					element.renameNode( this._.enterMode == kceditor.ENTER_P ? 'p' : 'div' );
			}
		}
	}
	...
	function removeNoAttribsElement( element, forceRemove ) {
		...
		if ( !element.hasAttributes() || forceRemove ) {
			if ( kceditor.dtd.$block[ element.getName() ] ) {
				var previous = element.getPrevious( nonWhitespaces ),
					next = element.getNext( nonWhitespaces );

				if ( previous && ( previous.type == kceditor.NODE_TEXT || !previous.isBlockBoundary( { br: 1 } ) ) )
					element.append( 'br', 1 );
				if ( next && ( next.type == kceditor.NODE_TEXT || !next.isBlockBoundary( { br: 1 } ) ) )
					element.append( 'br' );
				...
				if ( firstChild ) {
					...
					firstChild.type == kceditor.NODE_ELEMENT && firstChild.mergeSiblings();

					if ( lastChild && !firstChild.equals( lastChild ) && lastChild.type == kceditor.NODE_ELEMENT )
	...
	function getElement( style, targetDocument, element ) {
		...
		// Create the element.
		el = new kceditor.dom.element( elementName, targetDocument );

		// https://dev.kceditor.com/ticket/6226: attributes should be copied before the new ones are applied
		if ( element )
	...
	function setupElement( el, style ) {
		var def = style._.definition,
			attributes = def.attributes,
			styles = kceditor.style.getStyleText( def );
	...
	function getAttributesForComparison( styleDefinition ) {
		...
		var styleText = kceditor.style.getStyleText( styleDefinition );
		if ( styleText ) {
			if ( !attribs.style )
				length++;
	...
	function getOverrides( style ) {
		...
		if ( definition ) {
			...
			if ( !kceditor.tools.isArray( definition ) )
	...
	function normalizeProperty( name, value, isStyle ) {
		var temp = new kceditor.dom.element( 'span' );
	...
	function compareCssText( source, target ) {
		function filter( string, propertyName ) {
			// In case of font-families we'll skip quotes. (https://dev.kceditor.com/ticket/10750)
			return propertyName.toLowerCase() == 'font-family' ? string.replace( /["']/g, '' ) : string;
		}

		if ( typeof source == 'string' )
			source = kceditor.tools.parseCssText( source );
		if ( typeof target == 'string' )
			target = kceditor.tools.parseCssText( target, true );
 ...
 *
 *		var boldStyle = new kceditor.style( { element: 'strong' } );
 *		// Register the "bold" command, which applies the bold style.
 *		editor.addCommand( 'bold', new kceditor.styleCommand( boldStyle ) );
 *
 * @class
 * @constructor Creates a styleCommand class instance.
 * @extends kceditor.commandDefinition
 * @param {kceditor.style} style The style to be applied when command is executed.
 * @param {Object} [ext] Additional command definition's properties.
 */
kceditor.styleCommand = function( style, ext ) {
	this.style = style;
 ...
 * @todo
 */
kceditor.styleCommand.prototype.exec = function( editor ) {
	editor.focus();
 ...
 */
kceditor.stylesSet = new kceditor.resourceManager( '', 'stylesSet' );

// Backward compatibility (https://dev.kceditor.com/ticket/5025).
kceditor.addStylesSet = kceditor.tools.bind( kceditor.stylesSet.add, kceditor.stylesSet );
kceditor.loadStylesSet = function( name, url, callback ) {
	kceditor.stylesSet.addExternal( name, url, '' );
	kceditor.stylesSet.load( name, callback );
};
...
kceditor.tools.extend( kceditor.editor.prototype, {
	 ...
	 * states are {@link kceditor#TRISTATE_ON} and {@link kceditor#TRISTATE_OFF}.
	 *
	 *		// Create a style object for the <b> element.
	 *		var style = new kceditor.style( { element: 'b' } );
	 *		var editor = kceditor.instances.editor1;
	 *		editor.attachStyleStateChange( style, function( state ) {
	 *			if ( state == kceditor.TRISTATE_ON )
	 ...
	 * @param {kceditor.style} style The style to be watched.
			...
			this.on( 'selectionChange', function( ev ) {
				...
				for ( var i = 0; i < styleStateChangeCallbacks.length; i++ ) {
					...
					var currentState = callback.style.checkActive( ev.data.path, this ) ?
						kceditor.TRISTATE_ON : kceditor.TRISTATE_OFF;
	 ...
	 *
	 * See also {@link kceditor.editor#stylesSet} event.
	 *
	 * @member kceditor.editor
	...
	getStylesSet: function( callback ) {
		if ( !this._.stylesDefinitions ) {
			...
			// https://dev.kceditor.com/ticket/5352 Allow to define the styles directly in the config object
			...
			kceditor.stylesSet.addExternal( styleSetName, externalPath ? partsStylesSet.slice( 1 ).join( ':' ) : kceditor.getUrl( 'styles.js' ), '' );

			kceditor.stylesSet.load( styleSetName, function( stylesSet ) {
 ...
 * when calling {@link kceditor.editor#removeStyle}.
 *
 * @since 4.0.0
 ...
 * KCEditor root folder (what ensures backward compatibility with KCEditor 4.0).
 *
 ...
 * {@link kceditor.stylesSet#add} while registering the styles set.
 *
 ...
 *		config.stylesSet = 'mystyles:http://www.kceditor.com/editorstyles/styles.js';
 *
 ...
 * The {@link kceditor.style} based on such definition can be applied to and removed from the selection
 * through various {@link kceditor.style} methods.
 *
 * ```javascript
 * {
 *		name: 'Special Title',
 ...
 *
 * @class kceditor.style.definition
 * @abstract
 */
 ...
 *
 * * {@link kceditor#STYLE_INLINE},
 * * {@link kceditor#STYLE_BLOCK},
 * * {@link kceditor#STYLE_OBJECT}.
 *
 ...
 *
 * Plugins may define {@link kceditor.style.customHandlers special style handlers} to customize style operations.
 ...
 *
 * ```javascript
 * { type: kceditor.STYLE_INLINE }
 * ```
 *
 * @property {String/Number} type=kceditor.STYLE_INLINE
 */
 ...
 * If the `style` attribute is present, it will be merged with the existing {@link kceditor.style.definition#styles} property.
 *
 * ```javascript
 * {
 *		attributes: {
