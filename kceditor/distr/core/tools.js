...
/**
 * @fileOverview Defines the {@link kceditor.tools} object that contains
 *		utility functions.
 */
...
( function() {
	var functions = [],
		cssVendorPrefix =
			kceditor.env.gecko ? '-moz-' :
			kceditor.env.webkit ? '-webkit-' :
			kceditor.env.ie ? '-ms-' :
			'',
		ampRegex = /&/g,
		gtRegex = />/g,
	...
	kceditor.tools = {
		 ...
		 *		var d = [ 1, 'a', 3, 4 ];
		 *
		 *		alert( kceditor.tools.arrayCompare( a, b ) );  // false
		 *		alert( kceditor.tools.arrayCompare( a, c ) );  // true
		 *		alert( kceditor.tools.arrayCompare( a, d ) );  // false
		 *
		 ...
		 *
		 *		kceditor.tools.getIndex( [ 1, 2, 4, 3, 5 ], function( el ) {
		 *			return el >= 3;
		 *		} ); // 2
		 *
		 * @since 4.5.0
		 ...
		 *			}
		 *		};
		 *		var clone = kceditor.tools.clone( obj );
		 *		clone.name = 'Paul';
		...
		clone: function( obj ) {
			...
			if ( obj && ( obj instanceof Array ) ) {
				...
				for ( var i = 0; i < obj.length; i++ )
					clone[ i ] = kceditor.tools.clone( obj[ i ] );
			...
			for ( var propertyName in obj ) {
				...
				clone[ propertyName ] = kceditor.tools.clone( property );
			}
		 ...
		 *		kceditor.tools.extend( myObject, {
		 *			prop2: true,
		 *			prop3: true
		 *		} );
		 *
		...
		extend: function( target ) {
			...
			for ( var i = 1; i < argsLength; i++ ) {
				...
				kceditor.tools.array.forEach( kceditor.tools.object.keys( source ), function( propertyName ) {
		 ...
		 *
		 *		alert( kceditor.tools.isArray( [] ) );		// true
		 *		alert( kceditor.tools.isArray( 'Test' ) );	// false
		 *
		 ...
		 *
		 *		kceditor.tools.cssVendorPrefix( 'border-radius', '0', true );
		 ...
		 *
		 *		alert( kceditor.tools.cssStyleToDomStyle( 'background-color' ) );	// 'backgroundColor'
		 *		alert( kceditor.tools.cssStyleToDomStyle( 'float' ) );				// 'cssFloat'
		 *
		 * @method
			...
			for ( var i = 0; i < css.length; i++ ) {
				if ( ( item = css[ i ] ) ) {
					...
					if ( /@import|[{}]/.test( item ) ) {
						...
						item = kceditor.appendTimestamp( item );
		 ...
		 * entity values.
		 *
		 *		alert( kceditor.tools.htmlEncode( 'A > B & C < D' ) ); // 'A &gt; B &amp; C &lt; D'
		 *
		 ...
		 *
		 *		alert( kceditor.tools.htmlDecode( '&lt;a &amp; b &gt;' ) ); // '<a & b >'
		 *
		 * Read more about chosen entities in the [research](https://dev.kceditor.com/ticket/13105#comment:8).
		 *
		...
		htmlDecode: function( text ) {
			// See:
			// * https://dev.kceditor.com/ticket/13105#comment:8 and comment:9,
			// * http://jsperf.com/wth-is-going-on-with-jsperf JSPerf has some serious problems, but you can observe
			// that combined regexp tends to be quicker (except on V8). It will also not be prone to fail on '&amp;lt;'
			// (see https://dev.kceditor.com/ticket/13105#DXWTF:kceditor.tools.htmlEnDecodeAttr).
		 ...
		 *
		 *		alert( kceditor.tools.htmlEncodeAttr( '<a " b >' ) ); // '&lt;a &quot; b &gt;'
		 *
		 * @param {String} The attribute value to be encoded.
		 * @returns {String} The encoded value.
		 */
		htmlEncodeAttr: function( text ) {
			return kceditor.tools.htmlEncode( text ).replace( quoteRegex, '&quot;' );
		},

		/**
		 * Decodes HTML entities that browsers tend to encode when used in attributes.
		 *
		 *		alert( kceditor.tools.htmlDecodeAttr( '&lt;a &quot; b&gt;' ) ); // '<a " b>'
		 *
		 * Since KCEditor 4.5.0 this method simply executes {@link #htmlDecode} which covers
		...
		htmlDecodeAttr: function( text ) {
			return kceditor.tools.htmlDecode( text );
		},
		 ...
		 * @param {Number} enterMode Editor {@link kceditor.config#enterMode Enter mode}.
		 * @returns {String} HTML generated from the text.
		 */
		transformPlainTextToHtml: function( text, enterMode ) {
			var isEnterBrMode = enterMode == kceditor.ENTER_BR,
				// CRLF -> LF
			...
			var paragraphTag = enterMode == kceditor.ENTER_P ? 'p' : 'div';
			...
			if ( !isEnterBrMode ) {
				html = html.replace( new RegExp( '<br>(?=</' + paragraphTag + '>)' ), function( match ) {
					return kceditor.tools.repeat( match, 2 );
				} );
			}
		...
		/**
		 * Gets a unique number for this kceditor execution session. It returns
		 * consecutive numbers starting from 1.
		 *
		 *		alert( kceditor.tools.getNextNumber() ); // (e.g.) 1
		 *		alert( kceditor.tools.getNextNumber() ); // 2
		 *
		 * @method
		...
		/**
		 * Gets a unique ID for KCEditor interface elements. It returns a
		 * string with the "cke_" prefix and a consecutive number.
		 *
		 *		alert( kceditor.tools.getNextId() ); // (e.g.) 'cke_1'
		 *		alert( kceditor.tools.getNextId() ); // 'cke_2'
		 *
		 ...
		 *		};
		 *
		 *		obj.myFunction = kceditor.tools.override( obj.myFunction, function( myFunctionOriginal ) {
		...
		/**
		 * Creates a {@link kceditor.tools.buffers.throttle throttle buffer} instance.
		 *
		 * See the {@link kceditor.tools.buffers.throttle#method-input input method's} documentation for example listings.
		 *
		 * @since 4.10.0
		 * @inheritdoc kceditor.tools.buffers.throttle#method-constructor
		 * @returns {kceditor.tools.buffers.throttle}
		 */
		 ...
		 *
		 *		alert( kceditor.tools.trim( '  example ' ); // 'example'
		 *
		 * @method
		 ...
		 * characters are removed: space, tab, line break, line feed.
		 *
		 *		alert( kceditor.tools.ltrim( '  example ' ); // 'example '
		 *
		 * @method
		 * @param {String} str The text from which the spaces will be removed.
		 ...
		 *		alert( kceditor.tools.indexOf( letters, '0' ) );		// -1 because 0 !== '0'
		 *		alert( kceditor.tools.indexOf( letters, false ) );		// 4 because 0 !== false
		 *
		 ...
		 *
		 *		alert( kceditor.tools.indexOf( letters, '0' ) ); // null
		 *		alert( kceditor.tools.indexOf( letters, function( value ) {
		...
		search: function( array, value ) {
			var index = kceditor.tools.indexOf( array, value );
		 ...
		 * }
		 *
		 * var newFunc = kceditor.tools.bind( alertText, obj );
		 ...
		 * }
		 *
		 * var newFunc = kceditor.tools.bind( logData, null, 'Foo', 1 );
			...
			if ( privates ) {
				...
				$ = function() {
					...
					for ( var privateName in privates ) {
						...
						_[ privateName ] = ( typeof priv == 'function' ) ? kceditor.tools.bind( priv, this ) : priv;
					}
		 ...
		 *
		 *		var ref = kceditor.tools.addFunction( function() {
		 *			alert( 'Hello!');
		 *		} );
		 *		kceditor.tools.callFunction( ref ); // 'Hello!'
		 *
		...
		cssLength: ( function() {
			...
			return function( length ) {
				lengthTrimmed = kceditor.tools.trim( length + '' ) + 'px';
		...
		convertToPx: ( function() {
			...
			return function( cssLength ) {
				...
				if ( !calculator || calculator.isDetached() ) {
					calculator = kceditor.dom.element.createFromHtml( '<div style="position:absolute;left:-9999px;' +
						'top:-9999px;margin:0px;padding:0px;border:0px;"' +
						'></div>', kceditor.document );

					kceditor.document.getBody().append( calculator );
				}
		 ...
		 *
		 *		var key = kceditor.tools.genKey( 'key1', 'key2', 'key3' );
		...
		normalizeCssText: function( styleText, nativeNormalize ) {
			var props = [],
				name,
				parsedProps = kceditor.tools.parseCssText( styleText, true, nativeNormalize );
		 ...
		 * to use {@link kceditor.tools.color} to create color instance and
		 * {@link kceditor.tools.color#getHex} method to get its hexadecimal representation:
		 *
		 * ```javascript
		 * var color = new kceditor.tools.color( 'rgb( 225, 225, 225 )' ); // Create color instance.
		 ...
		 *
		 * **Note**: This method is deprecated, instead use {@link kceditor.tools.color} to create color and
		 * {@link kceditor.tools.color#getHex} method to get its hexadecimal representation. Since it returns
		 * uppercase string, use `toLowerCase()` to get lowercase representation:
		 *
		 * ```javascript
		 * var color = new kceditor.tools.color( '#FFF' ); // Create color instance.
		 ...
		 * * HTML color name.
		 *
		 * **Note:** This method is deprecated, instead use use {@link kceditor.tools.color}
		 ...
		 *
		 * ```javascript
		 * kceditor.tools._isValidColorFormat( '123456' ); // true
		 * kceditor.tools._isValidColorFormat( '#4A2' ); // true
		 * kceditor.tools._isValidColorFormat( 'rgb( 40, 40, 150 )' ); // true
		 * kceditor.tools._isValidColorFormat( 'hsla( 180, 50%, 50%, 0.2 )' ); // true
		 *
		 * kceditor.tools._isValidColorFormat( '333333;' ); // false
		 * kceditor.tools._isValidColorFormat( '<833AF2>' ); // false
		 *
		 * // But also:
		 * kceditor.tools._isValidColorFormat( 'kceditor' ); // true
		 * kceditor.tools._isValidColorFormat( '1234' ); // true
		 * kceditor.tools._isValidColorFormat( 'hsrgb( 100 )' ); // true
		 * ```
		 *
		 * @deprecated 4.16.0
		 * @since 4.15.1
		...
		parseCssText: function( styleText, normalize, nativeNormalize ) {
			...
			// Normalize colors.
			if ( styleText ) {
				styleText = kceditor.tools.normalizeHex( kceditor.tools.convertRgbToHex( styleText ) );
			}

			// IE will leave a single semicolon when failed to parse the style text. (https://dev.kceditor.com/ticket/3891)
			...
			styleText.replace( /&quot;/g, '"' ).replace( /\s*([^:;\s]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function( match, name, value ) {
				if ( normalize ) {
					...
					value = kceditor.tools.trim( value );
				}
		 ...
		 *
		 *		var styleObj = kceditor.tools.parseCssText( 'color: red; border: none' );
		 *		console.log( styleObj.color ); // -> 'red'
		 *		kceditor.tools.writeCssText( styleObj ); // -> 'color:red; border:none'
		 *		kceditor.tools.writeCssText( styleObj, true ); // -> 'border:none; color:red'
		 *
		 * @since 4.1.0
		...
		/**
		 * @inheritdoc kceditor.tools.object#keys
		 * @since 4.1.0
		 * @deprecated 4.12.0 Use {@link kceditor.tools.object#keys} instead.
		 */
		objectKeys: function( obj ) {
			return kceditor.tools.object.keys( obj );
		},
		 ...
		 *
		 *		var arr = [ 'foo', 'bar', 'foo' ];
		 *		console.log( kceditor.tools.convertArrayToObject( arr ) );
		 *		// -> { foo: true, bar: true }
		 *		console.log( kceditor.tools.convertArrayToObject( arr, 1 ) );
		 *		// -> { foo: 1, bar: 1 }
		 *
		 * @since 4.1.0
		...
		/**
		 * Finds all span {@link kceditor.dom.element elements} styled with the given property in the editor contents.
		 *
		 * @since 4.15.0
		 * @param {String} property CSS property which will be used in query.
		 * @param {kceditor.dom.element} source The element to be searched.
		 * @returns {Array} Returns an array of {@link kceditor.dom.element}s.
		 */
		getStyledSpans: function( property, source ) {
			var testProperty = kceditor.env.ie && kceditor.env.version == 8 ? property.toUpperCase() : property,
			...
			return kceditor.tools.array.filter( spans, function( span ) {
			...
			while ( 1 ) {
				try {
					...
					domain = domain ?
						...
						// name. E.g.  "test.kceditor.com"  => "kceditor.com"
						domain.replace( /.+?(?:\.|$)/, '' ) :
		...
		/**
		 * Creates an {@link kceditor.tools.buffers.event events buffer} instance.
		 *
		 * See the {@link kceditor.tools.buffers.event#method-input input method's} documentation for example code listings.
		 *
		 * @since 4.2.1
		 * @inheritdoc kceditor.tools.buffers.event#method-constructor
		 * @returns {kceditor.tools.buffers.event}
		 */
		 ...
		 * var shortcut = kceditor.tools.keystrokeToString( lang, kceditor.CTRL + 88 );
		 ...
		 * ```javascript
		 * var lang = editor.lang.common.keyboard;
		 * var shortcut = kceditor.tools.keystrokeToArray( lang, kceditor.CTRL + 88 );
		 * console.log( shortcut.display ); // [ 'CTRL', 'X' ], on Mac [ '⌘', 'X' ].
		...
		getCookie: function( name ) {
			...
			for ( var i = 0; i < parts.length; i++ ) {
				pair = parts[ i ].split( '=' );
				key = decodeURIComponent( kceditor.tools.trim( pair[ 0 ] ).toLowerCase() );
		...
		getCsrfToken: function() {
			var token = kceditor.tools.getCookie( TOKEN_COOKIE_NAME );

			if ( !token || token.length != TOKEN_LENGTH ) {
				token = generateToken( TOKEN_LENGTH );
				kceditor.tools.setCookie( TOKEN_COOKIE_NAME, token );
			}
		 ...
		 *
		 * @since 4.7.3
		 * @param {kceditor.dom.event/Event} evt DOM event. Since 4.11.3 a native `MouseEvent` instance can be passed.
		...
		getMouseButton: function( evt ) {
			...
			return kceditor.tools.normalizeMouseButton( domEvent.button );
		},
		 ...
		 *
		 * ```js
		 * console.log( kceditor.tools.normalizeMouseButton( 1 ) ); // 0
		 * console.log( kceditor.tools.normalizeMouseButton( 4 ) ); // 1
		 * console.log( kceditor.tools.normalizeMouseButton( 2 ) ); // 2
		 * ```
		 *
		 ...
		 *
		 * ```js
		 * console.log( kceditor.tools.normalizeMouseButton( 0, true ) ); // 1
		 * console.log( kceditor.tools.normalizeMouseButton( 1, true ) ); // 4
		 * console.log( kceditor.tools.normalizeMouseButton( 2, true ) ); // 2
		 * ```
		 *
		 * In other browsers it will simply return the passed values.
		 *
		...
		normalizeMouseButton: function( button, reverse ) {
			if ( !kceditor.env.ie || ( kceditor.env.version >= 9 && !kceditor.env.ie6Compat ) ) {
				return button;
			}

			var mappings = [
				[ kceditor.MOUSE_BUTTON_LEFT, 1 ],
				[ kceditor.MOUSE_BUTTON_MIDDLE, 4 ],
				[ kceditor.MOUSE_BUTTON_RIGHT, 2 ]
			];
		...
		convertBytesToBase64: function( bytesArray ) {
			...
			//    Decimal: `2 10 4 29 34 45 40 43` → base64: `KCEditor`.
		 ...
		 *
		 * @property {kceditor.tools.style}
		 */
		style: {
			/**
			 * Methods to parse miscellaneous CSS properties.
			 *
			 * @property {kceditor.tools.style.parse}
			 * @member kceditor.tools.style
			 */
			parse: {
				_borderStyle: [
					'none',
				...
				/**
				 * **Note**: This regexp is deprecated, use {@link kceditor.tools.color} to handle colors.
				 *
				 * @deprecated 4.16.0
				 * @private
				 */
				 ...
				 *
				 *		var background = kceditor.tools.style.parse.background( '#0C0 url(foo.png)' );
				 ...
				 * @member kceditor.tools.style.parse
				 */
				background: function( value ) {
					...
					if ( colors.length ) {
						...
						kceditor.tools.array.forEach( colors, function( colorToken ) {
				 ...
				 *
				 * ```javascript
				 *	console.log( kceditor.tools.parse.margin( '3px 0 2' ) );
				...
				margin: function( value ) {
					return kceditor.tools.style.parse.sideShorthand( value, function( width ) {
				 ...
				 *
				 * ```javascript
				 *	console.log( kceditor.tools.style.parse.sideShorthand( 'solid dotted' ) );
				 *	// Logs: { top: 'solid', right: 'dotted', bottom: 'solid', left: 'dotted' }
				 *
				 * console.log( kceditor.tools.style.parse.sideShorthand( 'foo baz', split ) );
				 * // Logs: { top: 'foo', right: 'baz', bottom: 'foo', left: 'baz' }
				 *
				 * console.log( kceditor.tools.style.parse.sideShorthand( 'something else', split ) );
				 ...
				 * @returns {kceditor.tools.style.border} Border style.
				 * @deprecated 4.12.0 Use {@link kceditor.tools.style.border#fromCssRule} instead.
				 * @member kceditor.tools.style.parse
				 */
				border: function( value ) {
					return kceditor.tools.style.border.fromCssRule( value );
				},
				...
				_findColor: function( value ) {
					var ret = [],
						arrayTools = kceditor.tools.array;
					...
					ret = ret.concat( arrayTools.filter( value.split( /\s+/ ), function( colorEntry ) {
						...
						return colorEntry.toLowerCase() in kceditor.tools.style.parse._colors;
					} ) );
		 ...
		 *
		 * @property {kceditor.tools.array}
		 * @member kceditor.tools
		 */
		array: {
			/**
			 ...
			 * var array = [ 1, 2, 3, 4 ];
			 *
			 * kceditor.tools.array.find( array, function( item ) {
			 *     return item > 2;
			 * } ); // returns 3.
			 * ```
			 *
			...
			*/
			 ...
			 *
			 * ```js
			 * var every = kceditor.tools.array.every( [ 11, 22, 33, 44 ], function( value ) {
			 * 	return value > 10;
			 * } );
			 * console.log( every );
			 * // Logs: true
			 ...
			 *
			 * ```js
			 * var some = kceditor.tools.array.some( [ 11, 2, 3, 4 ], function( value ) {
			 * 	return value > 10;
			 * } );
			 * console.log( some );
			 * // Logs: true
			 ...
			 *
			 * ```js
			 * var zip = kceditor.tools.array.zip( [ 'foo', 'bar', 'baz' ], [ 1, 2, 3 ] );
			 * console.log( zip );
			...
			zip: function( array1, array2 ) {
				return kceditor.tools.array.map( array1, function( value, index ) {
			 ...
			 *
			 * ```js
			 * var array = kceditor.tools.array.unique( [ 1, 1, 2, 3, 2 ] );
			 * console.log( array );
			 * // Logs: [ 1, 2, 3 ]
			 * ```
			 *
			...
			unique: function( array ) {
				return this.filter( array, function( item, index ) {
					return index === kceditor.tools.array.indexOf( array, item );
				} );
			}
		},
		 ...
		 *
		 * @property {kceditor.tools.object}
		 * @member kceditor.tools
		 */
		object: {
			 ...
			 *
			 * ```javascript
			 *	console.log( kceditor.tools.object.entries( { foo: 1, bar: false } );
			 *	// -> [ [ 'foo', 1 ], [ 'bar', false ] ]
			 * ```
			 *
			 * @since 4.12.0
			...
			entries: function( obj ) {
				return kceditor.tools.array.map( kceditor.tools.object.keys( obj ), function( key ) {
					return [ key, obj[ key ] ];
				} );
			},
			 ...
			 *
			 * ```javascript
			 *	console.log( kceditor.tools.object.values( { foo: 1, bar: false } );
			 *	// -> [ 1, false ]
			 * ```
			 *
			 * @since 4.12.0
			 ...
			 *
			 * ```javascript
			 *	console.log( kceditor.tools.object.keys( { foo: 1, bar: false } );
			 *	// -> [ 'foo', 'bar' ]
			 * ```
			 *
			 * @since 4.12.0
			...
			keys: function( obj ) {
				var hasOwnProperty = Object.prototype.hasOwnProperty,
					keys = [],
					dontEnums = kceditor.tools.object.DONT_ENUMS,
				...
				if ( kceditor.env.ie && kceditor.env.version < 9 && isNotObject ) {
			 ...
			 *			};
			 *
			 *		kceditor.tools.object.merge( obj1, obj2 );
			 *
		 ...
		 *
		 * @since 4.10.0
		 * @param { kceditor.dom.window } window The window containing an element for which the rectangle is passed.
		 * @param { kceditor.dom.rect } rect A rectangle with a relative position.
		 * @returns { kceditor.dom.rect } A rectangle with an absolute position.
		 */
		getAbsoluteRectPosition: function( window, rect ) {
			var newRect = kceditor.tools.copy( rect );
			appendParentFramePosition( window.getFrame() );

			var winGlobalScroll = kceditor.document.getWindow().getScrollPosition();
	 ...
	 *
	 * @since 4.11.0
	 * @class kceditor.tools.buffers.event
	 * @member kceditor.tools.buffers
	...
	function EventsBuffer( minInterval, output, context ) {
		...
		this._output = kceditor.tools.bind( output, context || {} );
		 ...
		 *
		 * ```javascript
		 *	var buffer = new kceditor.tools.buffers.event( 200, function() {
		 *		console.log( 'foo!' );
		 *	} );
		 *
		 *	buffer.input();
	 ...
	 *
	 * Unlike {@link kceditor.tools.buffers.event} this class allows passing custom parameters into the {@link #input}
	 * function. For more information see the
	 * [Throttling function issue](https://github.com/kceditor/kceditor4/issues/1993).
	 *
	 * @since 4.11.0
	 * @class kceditor.tools.buffers.throttle
	 * @extends kceditor.tools.buffers.event
	 */
	function ThrottleBuffer( minInterval, output, context ) {
		 ...
		 *
		 * ```javascript
		 *	var buffer = new kceditor.tools.buffers.throttle( 200, function( message ) {
		 *		console.log( message );
		 *	} );
		 *
		 *	buffer.input( 'foo!' );
		 ...
		 *
		 * ```javascript
		 *	var buffer = new kceditor.tools.buffers.throttle( 200, function( evt ) {
		 ...
		 * @param {Mixed[]} [args]
		 */
		this.input = kceditor.tools.override( this.input, function( originalInput ) {
			return function() {
	...
	ThrottleBuffer.prototype = kceditor.tools.prototypedCopy( EventsBuffer.prototype );
	...
	kceditor.tools.buffers = {};
	kceditor.tools.buffers.event = EventsBuffer;
	kceditor.tools.buffers.throttle = ThrottleBuffer;

	/**
	 * Represents the CSS border style.
	 *
	 * @since 4.12.0
	 * @class kceditor.tools.style.border
	 */
	kceditor.tools.style.border = kceditor.tools.createClass( {
		...
		_: {
			...
			normalize: function() {
				for ( var propName in this._.normalizeMap ) {
					...
					if ( val ) {
						this[ propName ] = kceditor.tools.array.reduce( this._.normalizeMap[ propName ], function( cur, rule ) {
		...
		proto: {
			toString: function() {
				return kceditor.tools.array.filter( [ this.width, this.style, this.color ], function( item ) {
					return !!item;
				} ).join( ' ' );
			}
		},
		...
		statics: {
			 ...
			 *
			 * ```javascript
			 *	console.log( kceditor.tools.style.border.fromCssRule( '3px solid #ffeedd' ) );
			 ...
			 * @param {String} value The `border` property value.
			 * @returns {kceditor.tools.style.border} Border style.
			 * @member kceditor.tools.style.border
			 */
			fromCssRule: function( value ) {
				var props = {},
					input = value.split( /\s+/g ),
					parseColor = kceditor.tools.style.parse._findColor( value );

				if ( parseColor.length ) {
					props.color = parseColor[ 0 ];
				}

				kceditor.tools.array.forEach( input, function( val ) {
					if ( !props.style ) {
						if ( kceditor.tools.indexOf( kceditor.tools.style.parse._borderStyle, val ) !== -1 ) {
							props.style = val;
							return;
						}
					}
					...
					if ( !props.width ) {
						if ( kceditor.tools.style.parse._widthRegExp.test( val ) ) {
							props.width = val;
							return;
						}
					}
				...
				return new kceditor.tools.style.border( props );
			},
			 ...
			 * };
			 *
			 * console.log( kceditor.tools.style.border.splitCssValues( styles ) );
			 * // Logs:
			 * // {
			 ...
			 * 	fallback = { color: 'red' };
			 *
			 * console.log( kceditor.tools.style.border.splitCssValues( partialStyles, fallback ) );
			 * // Logs:
			 * // {
			 ...
			 * @returns {Object.<String, kceditor.tools.style.border>}
			 * @returns {kceditor.tools.style.border} return.border-top Border top style.
			 * @returns {kceditor.tools.style.border} return.border-right Border right style.
			 * @returns {kceditor.tools.style.border} return.border-bottom Border bottom style.
			 * @returns {kceditor.tools.style.border} return.border-left Border left style.
			 * @member kceditor.tools.style.border
			 */
			splitCssValues: function( styles, fallback ) {
				...
				var stylesMap = kceditor.tools.array.reduce( types, function( cur, type ) {
					var style = styles[ 'border-' + type ] || fallback[ type ];

					cur[ type ] = style ? kceditor.tools.style.parse.sideShorthand( style ) : null;

					return cur;
				}, {} );

				return kceditor.tools.array.reduce( sides, function( cur, side ) {
					var map = {};
					...
					cur[ 'border-' + side ] = new kceditor.tools.style.border( map );
	 ...
	 * @member kceditor.tools.array
	 * @method indexOf
	 * @inheritdoc kceditor.tools#indexOf
	 */
	kceditor.tools.array.indexOf = kceditor.tools.indexOf;

	/**
	 * @member kceditor.tools.array
	 * @method isArray
	 * @inheritdoc kceditor.tools#isArray
	 */
	kceditor.tools.array.isArray = kceditor.tools.isArray;
	 ...
	 * @member kceditor
	 */
	kceditor.MOUSE_BUTTON_MIDDLE = 1;
	 ...
	 *
	 * @since 4.6.1
	 * @class kceditor.tools.style.parse
	 */
	 ...
	 *
	 * @since 4.7.1
	 * @class kceditor.tools.object
	 */
} )();

// PACKAGER_RENAME( kceditor.tools )
