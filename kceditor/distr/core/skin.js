...
/**
 * @fileOverview Defines the {@link kceditor.skin} class that is used to manage skin parts.
 */
...
( function() {
	...
	function getName() {
		return kceditor.skinName.split( ',' )[ 0 ];
	}

	function getConfigPath() {
		return kceditor.getUrl( kceditor.skinName.split( ',' )[ 1 ] || ( 'skins/' + getName() + '/' ) );
	}
	...
	kceditor.skin = {
		...
		loadPart: function( part, fn ) {
			if ( kceditor.skin.name != getName() ) {
				kceditor.scriptLoader.load( kceditor.getUrl( getConfigPath() + 'skin.js' ), function() {
					loadCss( part, fn );
				} );
			} else {
				loadCss( part, fn );
		...
		getPath: function( part ) {
			return kceditor.getUrl( getCssPath( part ) );
		},
		...
		getIconStyle: function( name, rtl, overridePath, overrideOffset, overrideBgsize ) {
			...
			// If we use apostrophes in background-image, we must escape apostrophes in path (just to be sure). (https://dev.kceditor.com/ticket/13361)
			if ( path )
				path = path.replace( /'/g, '\\\'' );

			return path &&
				( 'background-image:url(\'' + kceditor.getUrl( path ) + '\');background-position:0 ' + offset + 'px;background-size:' + bgsize + ';' );
		}
	};

	function getCssPath( part ) {
		// Check for ua-specific version of skin part.
		var uas = kceditor.skin[ 'ua_' + part ], env = kceditor.env;
		if ( uas ) {
		...
		return kceditor.getUrl( getConfigPath() + part + '.css' );
	}

	function loadCss( part, callback ) {
		// Avoid reload.
		if ( !cssLoaded[ part ] ) {
			kceditor.document.appendStyleSheet( getCssPath( part ) );
			cssLoaded[ part ] = 1;
		}
	...
	kceditor.tools.extend( kceditor.editor.prototype, {
		 ...
		 *
		 *		kceditor.instances.editor1.getUiColor();
		 *
		 * @method
		 ...
		 *
		 * 		kceditor.instances.editor1.setUiColor( '#ff00ff' );
		 *
		 * @method
		 * @member kceditor.editor
		...
		setUiColor: function( color ) {
			var uiStyle = getStylesheet( kceditor.document );

			return ( this.setUiColor = function( color ) {
				this.uiColor = color;

				var chameleon = kceditor.skin.chameleon,
					editorStyleContent = '',
					panelStyleContent = '';
	...
	function updateStylesheets( styleNodes, styleContent, replace ) {
		...
		for ( var id = 0; id < styleNodes.length; id++ ) {
			if ( kceditor.env.webkit ) {
				...
				if ( kceditor.env.ie && kceditor.env.version < 11 )
	...
	kceditor.on( 'instanceLoaded', function( evt ) {
		...
		if ( kceditor.env.ie && kceditor.env.quirks ) {
			return;
		}
			...
			showCallback = function( event ) {
				...
				if ( !iframe.getById( 'cke_ui_color' ) ) {
					...
					editor.on( 'destroy', function() {
						uiColorMenus = kceditor.tools.array.filter( uiColorMenus, function( storedNode ) {
					...
					if ( color ) {
						updateStylesheets( [ node ], kceditor.skin.chameleon( editor, 'panel' ), [ [ uiColorRegexp, color ] ] );
					}
				}
			};
 ...
 * {@link kceditor.env}. This is used to load the skin part file in addition
 ...
 *
 * This is a shortcut to {@link kceditor#skinName}.
 *
 ...
 * must match the {@link kceditor.env} properties. You can find more information about browser "hacks" in the
 * {@glink guide/skin_sdk_browser_hacks Dedicated Browser Hacks} guide.
 *
 *		kceditor.skin.ua_editor = 'ie,iequirks,ie8,gecko';
 *
 ...
 *
 *		kceditor.skin.ua_dialog = 'ie,iequirks,ie8,gecko';
 *
