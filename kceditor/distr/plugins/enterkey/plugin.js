...
( function() {
	...
	kceditor.plugins.enterkey = {
		enterBlock: function( editor, mode, range, forceMode ) {
			...
			// which have root property set to closest editable, to make auto paragraphing work. (https://dev.kceditor.com/ticket/12162)
			...
			var atBlockStart = range.checkStartOfBlock(),
				...
				blockTag = ( mode == kceditor.ENTER_DIV ? 'div' : 'p' ),
			...
			if ( block && atBlockStart && atBlockEnd ) {
				...
				if ( blockParent.is( 'li' ) && blockParent.getChildCount() > 1 ) {
					var placeholder = new kceditor.dom.element( 'li' ),
				...
				// Exit the list when we're inside an empty list item block. (https://dev.kceditor.com/ticket/5376).
					...
					var blockGrandParent = blockParent.getParent(),
						...
						needsBlock = enterMode != kceditor.ENTER_BR || dirLoose || style || className,
					...
					if ( blockGrandParent.is( 'li' ) ) {
						...
						if ( path.block.is( 'li' ) ) {
							...
							newBlock = doc.createElement( mode == kceditor.ENTER_P ? 'p' : 'div' );
				...
				if ( block && block.getParent().is( 'blockquote' ) ) {
					...
					if ( !block.getPrevious().getFirst( kceditor.dom.walker.invisible( 1 ) ) )
						block.getPrevious().remove();

					// If we were at the end of <blockquote>, there will be an empty element after it now.
					if ( !block.getNext().getFirst( kceditor.dom.walker.invisible( 1 ) ) )
			...
			// If this is a block under a list item, split it as well. (https://dev.kceditor.com/ticket/1647)
			if ( nextBlock ) {
			...
			if ( !isStartOfBlock && !isEndOfBlock ) {
				...
				// wouldn't be editable. (https://dev.kceditor.com/ticket/1420)
				if ( nextBlock.is( 'li' ) ) {
					var walkerRange = range.clone();
					walkerRange.selectNodeContents( nextBlock );
					var walker = new kceditor.dom.walker( walkerRange );
					walker.evaluator = function( node ) {
						return !( bookmark( node ) || whitespaces( node ) || node.type == kceditor.NODE_ELEMENT && node.getName() in kceditor.dtd.$inline && !( node.getName() in kceditor.dtd.$empty ) );
					};

					node = walker.next();
					if ( node && node.type == kceditor.NODE_ELEMENT && node.is( 'ul', 'ol' ) )
						( kceditor.env.needsBrFiller ? doc.createElement( 'br' ) : doc.createText( '\xa0' ) ).insertBefore( node );
				}
			...
			// We don't want to add new block, because it was created with range.splitBlock().
				...
				if ( previousBlock ) {
					...
					// a Shift+Enter (https://dev.kceditor.com/ticket/77). Create a new block element instead
					// (later in the code).
				...
				if ( !newBlock ) {
					// We have already created a new list item. (https://dev.kceditor.com/ticket/6849)
					if ( node && node.is( 'li' ) )
						newBlock = node;
					else {
				...
				if ( elementPath ) {
					for ( var i = 0, len = elementPath.elements.length; i < len; i++ ) {
						...
						if ( kceditor.dtd.$removeEmpty[ element.getName() ] ) {
				...
				// list item start number should not be duplicated (https://dev.kceditor.com/ticket/7330), but we need
				// to remove the attribute after it's onto the DOM tree because of old IEs (https://dev.kceditor.com/ticket/7581).
				...
				// empty table cell).
				if ( kceditor.env.ie && isStartOfBlock && ( !isEndOfBlock || !previousBlock.getChildCount() ) ) {
			...
			function preventExtraLineInsideTable( mode ) {
				...
				if ( mode === kceditor.ENTER_BR ) {
					return false;
				}

				if ( kceditor.tools.indexOf( [ 'td', 'th' ], path.lastElement.getName() ) === -1 ) {
					return false;
				}
		...
		enterBr: function( editor, mode, range, forceMode ) {
			...
			var elementPath = new kceditor.dom.elementPath( editor.getSelection().getStartElement() );
			...
			if ( !forceMode && isEndOfBlock && headerTagRegex.test( startBlockTag ) ) {
				...
				if ( ( newBlockDir = startBlock.getDirection() ) ) {
					...
					range.setStartAt( startBlock.getNext(), kceditor.env.ie ? kceditor.POSITION_BEFORE_START : kceditor.POSITION_AFTER_START );
				}
			} else {
				var lineBreak;

				// IE<8 prefers text node as line-break inside of <pre> (https://dev.kceditor.com/ticket/4711).
				if ( startBlockTag == 'pre' && kceditor.env.ie && kceditor.env.version < 8 )
				...
				if ( !kceditor.env.needsBrFiller )
					range.setStartAt( lineBreak, kceditor.POSITION_AFTER_END );
				else {
					// A text node is required by Gecko only to make the cursor blink.
					...
					range.setStartAt( lineBreak.getNext(), kceditor.POSITION_AFTER_START );
	...
	plugin = kceditor.plugins.enterkey;
	...
	function shiftEnter( editor ) {
		...
		// of cloning the current block. (https://dev.kceditor.com/ticket/77)
	...
	function replaceRangeWithClosestEditableRoot( range ) {
		var closestEditable = range.startContainer.getAscendant( function( node ) {
			return node.type == kceditor.NODE_ELEMENT && node.getAttribute( 'contenteditable' ) == 'true';
		}, true );

		if ( range.root.equals( closestEditable ) ) {
			return range;
		} else {
			var newRange = new kceditor.dom.range( closestEditable );
