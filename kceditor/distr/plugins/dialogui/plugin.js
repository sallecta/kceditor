...
kceditor.plugins.add( 'dialogui', {
	onLoad: function() {
		...
		var initPrivateObject = function( elementDefinition ) {
				...
				args.push( true );
				kceditor.tools.extend.apply( kceditor.tools, args );
				return this._;
			},
			textBuilder = {
				build: function( dialog, elementDefinition, output ) {
					return new kceditor.ui.dialog.textInput( dialog, elementDefinition, output );
				}
			},
			commonBuilder = {
				build: function( dialog, elementDefinition, output ) {
					return new kceditor.ui.dialog[ elementDefinition.type ]( dialog, elementDefinition, output );
				}
			},
			containerBuilder = {
				build: function( dialog, elementDefinition, output ) {
			...
			commonEventProcessors = kceditor.tools.extend( {}, kceditor.ui.dialog.uiElement.prototype.eventProcessors, {
				onChange: function( dialog, func ) {
					if ( !this._.domOnChangeRegistered ) {
						dialog.on( 'load', function() {
							this.getInputElement().on( 'change', function() {
								// Make sure 'onchange' doesn't get fired after dialog closed. (https://dev.kceditor.com/ticket/5719)
			...
			// @context {kceditor.dialog.uiElement} UI element (textarea or textInput)
			// @param {kceditor.dom.event} evt
			toggleBidiKeyUpHandler = function( evt ) {
				var keystroke = evt.data.getKeystroke();

				// ALT + SHIFT + Home for LTR direction.
				if ( keystroke == kceditor.SHIFT + kceditor.ALT + 36 )
					this.setDirectionMarker( 'ltr' );

				// ALT + SHIFT + End for RTL direction.
				else if ( keystroke == kceditor.SHIFT + kceditor.ALT + 35 )
					this.setDirectionMarker( 'rtl' );
			};

		kceditor.tools.extend( kceditor.ui.dialog, {
			/**
			 * Base class for all dialog window elements with a textual label on the left.
			 *
			 * @class kceditor.ui.dialog.labeledElement
			 * @extends kceditor.ui.dialog.uiElement
			 * @constructor Creates a labeledElement class instance.
			 * @param {kceditor.dialog} dialog Parent dialog window object.
			...
			labeledElement: function( dialog, elementDefinition, htmlList, contentHtml ) {
				...
				_.labelId = kceditor.tools.getNextId() + '_label';
				this._.children = [];
				...
				var innerHTML = function() {
						...
						if ( elementDefinition.labelLayout != 'horizontal' ) {
							...
							var hboxDefinition = {
								...
								children: [ {
									...
									html: '<label class="cke_dialog_ui_labeled_label' + requiredClass + '"' +
										...
										' for="' + _.inputId + '"' +
										( elementDefinition.labelStyle ? ' style="' + elementDefinition.labelStyle + '"' : '' ) + '>' +
											kceditor.tools.htmlEncode( elementDefinition.label ) +
										'</label>'
								},
								{
									type: 'html',
							...
							kceditor.dialog._.uiElementBuilders.hbox.build( dialog, hboxDefinition, html );
						}
						return html.join( '' );
					};
				...
				kceditor.ui.dialog.uiElement.call( this, dialog, elementDefinition, htmlList, 'div', null, attributes, innerHTML );
			},
			 ...
			 *
			 * @class kceditor.ui.dialog.textInput
			 * @extends kceditor.ui.dialog.labeledElement
			...
			textInput: function( dialog, elementDefinition, htmlList ) {
				...
				var domId = this._.inputId = kceditor.tools.getNextId() + '_textInput',
				...
				dialog.on( 'load', function() {
					...
					// Lower the priority this 'keyup' since 'ok' will close the dialog.(https://dev.kceditor.com/ticket/3749)
				...
				kceditor.ui.dialog.labeledElement.call( this, dialog, elementDefinition, htmlList, innerHTML );
			},

			/**
			 * A text area with a label at the top or on the left.
			 *
			 * @class kceditor.ui.dialog.textarea
			 * @extends kceditor.ui.dialog.labeledElement
			...
			textarea: function( dialog, elementDefinition, htmlList ) {
				...
				initPrivateObject.call( this, elementDefinition );
				var me = this,
					domId = this._.inputId = kceditor.tools.getNextId() + '_textarea',
					attributes = {};
				...
				var innerHTML = function() {
						...
						html.push( '>', kceditor.tools.htmlEncode( me._[ 'default' ] ), '</textarea></div>' );
						return html.join( '' );
					};
				kceditor.ui.dialog.labeledElement.call( this, dialog, elementDefinition, htmlList, innerHTML );
			},
			 ...
			 *
			 * @class kceditor.ui.dialog.checkbox
			 * @extends kceditor.ui.dialog.uiElement
			...
			checkbox: function( dialog, elementDefinition, htmlList ) {
				...
				var innerHTML = function() {
						var myDefinition = kceditor.tools.extend(
								{},
								elementDefinition,
								{
									id: elementDefinition.id ? elementDefinition.id + '_checkbox' : kceditor.tools.getNextId() + '_checkbox'
								},
								true
							),
							html = [];

						var labelId = kceditor.tools.getNextId() + '_label';
						...
						_.checkbox = new kceditor.ui.dialog.uiElement( dialog, myDefinition, html, 'input', null, attributes );
						html.push(
							' <label id="',
							labelId,
							'" for="',
				...
				kceditor.ui.dialog.uiElement.call( this, dialog, elementDefinition, htmlList, 'span', null, null, innerHTML );
			},

			/**
			 * A group of radio buttons.
			 *
			 * @class kceditor.ui.dialog.radio
			 * @extends kceditor.ui.dialog.labeledElement
			...
			radio: function( dialog, elementDefinition, htmlList ) {
				...
				var innerHTML = function() {
					var inputHtmlList = [],
						html = [],
						commonName = ( elementDefinition.id ? elementDefinition.id : kceditor.tools.getNextId() ) + '_radio';

					for ( var i = 0; i < elementDefinition.items.length; i++ ) {
						var item = elementDefinition.items[ i ],
							title = item[ 2 ] !== undefined ? item[ 2 ] : item[ 0 ],
							value = item[ 1 ] !== undefined ? item[ 1 ] : item[ 0 ],
							inputId = kceditor.tools.getNextId() + '_radio_input',
							labelId = inputId + '_label',

							inputDefinition = kceditor.tools.extend( {}, elementDefinition, {
								id: inputId,
								title: null,
								type: null
							}, true ),

							labelDefinition = kceditor.tools.extend( {}, inputDefinition, {
								title: title
							}, true ),
						...
						var radioElement = new kceditor.ui.dialog.uiElement( dialog, inputDefinition, inputHtml, 'input', null, inputAttributes );
						...
						new kceditor.ui.dialog.uiElement( dialog, labelDefinition, inputHtml, 'label', null, {
							id: labelId,
					...
					new kceditor.ui.dialog.hbox( dialog, children, inputHtmlList, html );
			 ...
			 *
			 * @class kceditor.ui.dialog.button
			 * @extends kceditor.ui.dialog.uiElement
			...
			button: function( dialog, elementDefinition, htmlList ) {
				...
				dialog.on( 'load', function() {
					...
					( function() {
						element.on( 'click', function( evt ) {
							me.click();
							// https://dev.kceditor.com/ticket/9958
				...
				var outerDefinition = kceditor.tools.extend( {}, elementDefinition );
				delete outerDefinition.style;

				var labelId = kceditor.tools.getNextId() + '_label';
				kceditor.ui.dialog.uiElement.call( this, dialog, outerDefinition, htmlList, 'a', null, {
			 ...
			 * A select box.
			 *
			 * @class kceditor.ui.dialog.select
			 * @extends kceditor.ui.dialog.uiElement
			 * @constructor Creates a button class instance.
			 * @param {kceditor.dialog} dialog Parent dialog window object.
			 * @param {kceditor.dialog.definition.uiElement} elementDefinition
			...
			select: function( dialog, elementDefinition, htmlList ) {
				...
				_.inputId = kceditor.tools.getNextId() + '_select';
				...
				var innerHTML = function() {
						var myDefinition = kceditor.tools.extend(
								...
								elementDefinition,
								{
									id: ( elementDefinition.id ? elementDefinition.id + '_select' : kceditor.tools.getNextId() + '_select' )
								},
								true
							),
							html = [],
						...
						for ( var i = 0, item; i < elementDefinition.items.length && ( item = elementDefinition.items[ i ] ); i++ ) {
							innerHTML.push( '<option value="', kceditor.tools.htmlEncode( item[ 1 ] !== undefined ? item[ 1 ] : item[ 0 ] ).replace( /"/g, '&quot;' ), '" /> ', kceditor.tools.htmlEncode( item[ 0 ] ) );
						}

						if ( typeof myDefinition.inputStyle != 'undefined' )
							myDefinition.style = myDefinition.inputStyle;

						_.select = new kceditor.ui.dialog.uiElement( dialog, myDefinition, html, 'select', null, attributes, innerHTML.join( '' ) );
			...
			file: function( dialog, elementDefinition, htmlList ) {
				...
				var _ = kceditor.tools.extend( initPrivateObject.call( this, elementDefinition ), { definition: elementDefinition, buttons: [] } );
				...
				/** @ignore */
				var innerHTML = function() {
					_.frameId = kceditor.tools.getNextId() + '_fileInput';
					...
					// Support for custom document.domain on IE. (https://dev.kceditor.com/ticket/10165)
					html.push( kceditor.env.ie ?
				...
				dialog.on( 'load', function() {
					var iframe = kceditor.document.getById( _.frameId ),
			 ...
			 *
			 * @class kceditor.ui.dialog.fileButton
			 * @extends kceditor.ui.dialog.button
			...
			fileButton: function( dialog, elementDefinition, htmlList ) {
				...
				var myDefinition = kceditor.tools.extend( {}, elementDefinition );
				...
				kceditor.ui.dialog.button.call( this, dialog, myDefinition, htmlList );
			},
			...
			html: ( function() {
				 ...
				 *
				 * @class kceditor.ui.dialog.html
				 * @extends kceditor.ui.dialog.uiElement
				 * @constructor Creates a html class instance.
				 * @param {kceditor.dialog} dialog Parent dialog window object.
				 * @param {kceditor.dialog.definition.uiElement} elementDefinition Element definition.
				 * Accepted fields:
				 *
				...
				return function( dialog, elementDefinition, htmlList ) {
					...
					kceditor.ui.dialog.uiElement.call( this, dialog, elementDefinition, myHtmlList, 'span', null, null, '' );
			 ...
			 *
			 * @class kceditor.ui.dialog.fieldset
			 * @extends kceditor.ui.dialog.uiElement
			...
			fieldset: function( dialog, childObjList, childHtmlList, htmlList, elementDefinition ) {
				...
				kceditor.ui.dialog.uiElement.call( this, dialog, elementDefinition, htmlList, 'fieldset', null, null, innerHTML );
			}

		}, true );

		kceditor.ui.dialog.html.prototype = new kceditor.ui.dialog.uiElement();

		/** @class kceditor.ui.dialog.labeledElement */
		kceditor.ui.dialog.labeledElement.prototype = kceditor.tools.extend( new kceditor.ui.dialog.uiElement(), {
			/**
			 * Sets the label text of the element.
			 *
			 * @param {String} label The new label text.
			 * @returns {kceditor.ui.dialog.labeledElement} The current labeled element.
			 */
			setLabel: function( label ) {
				var node = kceditor.document.getById( this._.labelId );
				if ( node.getChildCount() < 1 )
				( new kceditor.dom.text( label, kceditor.document ) ).appendTo( node );
				else
		...
		/** @class kceditor.ui.dialog.button */
		kceditor.ui.dialog.button.prototype = kceditor.tools.extend( new kceditor.ui.dialog.uiElement(), {
			/**
			 ...
			 * @property {Object}
			 */
			eventProcessors: kceditor.tools.extend( {}, kceditor.ui.dialog.uiElement.prototype.eventProcessors, {
		...
		/** @class kceditor.ui.dialog.textInput */
		kceditor.ui.dialog.textInput.prototype = kceditor.tools.extend( new kceditor.ui.dialog.labeledElement(), {
			/**
			 * Gets the text input DOM element under this UI object.
			 *
			 * @returns {kceditor.dom.element} The DOM element of the text input.
			 */
			getInputElement: function() {
				return kceditor.document.getById( this._.inputId );
			},
			 ...
			 * @returns {kceditor.ui.dialog.textInput} The current UI element.
			 */
			setValue: function( value ) {
				...
				return kceditor.ui.dialog.uiElement.prototype.setValue.apply( this, arguments );
			},
			 ...
			 */
			getValue: function() {
				var value = kceditor.ui.dialog.uiElement.prototype.getValue.call( this );
		...
		kceditor.ui.dialog.textarea.prototype = new kceditor.ui.dialog.textInput();

		/** @class kceditor.ui.dialog.select */
		kceditor.ui.dialog.select.prototype = kceditor.tools.extend( new kceditor.ui.dialog.labeledElement(), {
			/**
			 * Gets the DOM element of the select box.
			 *
			 * @returns {kceditor.dom.element} The `<select>` element of this UI element.
			 */
			getInputElement: function() {
			 ...
			 * @returns {kceditor.ui.dialog.select} The current select UI element.
			 */
			add: function( label, value, index ) {
				var option = new kceditor.dom.element( 'option', this.getDialog().getParentEditor().document ),
		...
		/** @class kceditor.ui.dialog.checkbox */
		kceditor.ui.dialog.checkbox.prototype = kceditor.tools.extend( new kceditor.ui.dialog.uiElement(), {
			/**
			 * Gets the checkbox DOM element.
			 *
			 * @returns {kceditor.dom.element} The DOM element of the checkbox.
			 */
			getInputElement: function() {
			...
			eventProcessors: {
				onChange: function( dialog, func ) {
					if ( !kceditor.env.ie || ( kceditor.env.version > 8 ) )
		...
		/** @class kceditor.ui.dialog.radio */
		kceditor.ui.dialog.radio.prototype = kceditor.tools.extend( new kceditor.ui.dialog.uiElement(), {
			focus: function() {
		...
		/** @class kceditor.ui.dialog.file */
		kceditor.ui.dialog.file.prototype = kceditor.tools.extend( new kceditor.ui.dialog.labeledElement(), commonPrototype, {
			/**
			 * Gets the `<input>` element of this file input.
			 *
			 * @returns {kceditor.dom.element} The file input element.
			 */
			getInputElement: function() {
				var frameDocument = kceditor.document.getById( this._.frameId ).getFrameDocument();
				return frameDocument.$.forms.length > 0 ? new kceditor.dom.element( frameDocument.$.forms[ 0 ].elements[ 0 ] ) : this.getElement();
			},

			/**
			 * Uploads the file in the file input.
			 *
			 * @returns {kceditor.ui.dialog.file} This object.
			 */
			submit: function() {
			...
			reset: function() {
				var _ = this._,
					frameElement = kceditor.document.getById( _.frameId ),
				...
				if ( !callNumber ) {
					callNumber = this.formLoadedNumber = kceditor.tools.addFunction( function() {
					...
					unloadNumber = this.formUnloadNumber = kceditor.tools.addFunction( function() {
						this.getInputElement().clearCustomData();
					}, this );

					this.getDialog()._.editor.on( 'destroy', function() {
						kceditor.tools.removeFunction( callNumber );
						kceditor.tools.removeFunction( unloadNumber );
					} );
				}
				...
				function generateFormField() {
					...
					var size = '';
					if ( elementDefinition.size )
						size = elementDefinition.size - ( kceditor.env.ie ? 7 : 0 ); // "Browse" button is bigger in IE.
					...
					frameDocument.$.write( [
						'<html dir="' + langDir + '" lang="' + langCode + '"><head><title></title></head><body style="margin: 0; overflow: hidden; background: transparent;">',
							'<form enctype="multipart/form-data" method="POST" dir="' + langDir + '" lang="' + langCode + '" action="',
								kceditor.tools.htmlEncode( elementDefinition.action ),
							'">',
								// Replicate the field label inside of iframe.
								'<label id="', _.labelId, '" for="', inputId, '" style="display:none">',
									kceditor.tools.htmlEncode( elementDefinition.label ),
								'</label>',
								// Set width to make sure that input is not clipped by the iframe (https://dev.kceditor.com/ticket/11253).
								'<input style="width:100%" id="', inputId, '" aria-labelledby="', _.labelId, '" type="file" name="',
									kceditor.tools.htmlEncode( elementDefinition.id || 'cke_upload' ),
									'" size="',
									kceditor.tools.htmlEncode( size > 0 ? size : '' ),
								'" />',
							'</form>',
						'</body></html>',
						'<script>',
							// Support for custom document.domain in IE.
							kceditor.env.ie ? '(' + kceditor.tools.fixDomain + ')();' : '',

							'window.parent.kceditor.tools.callFunction(' + callNumber + ');',
							'window.onbeforeunload = function() {window.parent.kceditor.tools.callFunction(' + unloadNumber + ')}',
						'</script>'
					].join( '' ) );
				...
				// https://dev.kceditor.com/ticket/3465: Wait for the browser to finish rendering the dialog first.
		...
		kceditor.ui.dialog.fileButton.prototype = new kceditor.ui.dialog.button();

		kceditor.ui.dialog.fieldset.prototype = kceditor.tools.clone( kceditor.ui.dialog.hbox.prototype );

		kceditor.dialog.addUIElement( 'text', textBuilder );
		kceditor.dialog.addUIElement( 'password', textBuilder );
		kceditor.dialog.addUIElement( 'tel', textBuilder );
		kceditor.dialog.addUIElement( 'textarea', commonBuilder );
		kceditor.dialog.addUIElement( 'checkbox', commonBuilder );
		kceditor.dialog.addUIElement( 'radio', commonBuilder );
		kceditor.dialog.addUIElement( 'button', commonBuilder );
		kceditor.dialog.addUIElement( 'select', commonBuilder );
		kceditor.dialog.addUIElement( 'file', commonBuilder );
		kceditor.dialog.addUIElement( 'fileButton', commonBuilder );
		kceditor.dialog.addUIElement( 'html', commonBuilder );
		kceditor.dialog.addUIElement( 'fieldset', containerBuilder );
	}
} );
 ...
 *
 * @event change
 * @member kceditor.ui.dialog.uiElement
 */
 ...
 *
 * @event formLoaded
 * @member kceditor.ui.dialog.fileButton
 */
