...
( function() {
	var win = kceditor.document.getWindow(),
		pixelate = kceditor.tools.cssLength;

	kceditor.plugins.add( 'floatingspace', {
	...
	function scrollOffset( side ) {
		...
		return ( pageOffset in win.$ ) ? win.$[ pageOffset ] : kceditor.document.$.documentElement[ docScrollOffset ];
	}
	...
	function attach( editor ) {
		var config = editor.config,
			...
			layout = ( function() {
				...
				return function( evt ) {
					// https://dev.kceditor.com/ticket/10112 Do not fail on editable-less editor.
					...
					// (https://dev.kceditor.com/ticket/9769) If viewport width is less than space width,
					...
					if ( spaceRect.width > viewRect.width ) {
						...
						// (https://dev.kceditor.com/ticket/9769) Finally, stick the space to the opposite side of
					...
					// (https://dev.kceditor.com/ticket/9903) For mode is "top" or "bottom", add opposite scroll-x for right-aligned space.
		...
		if ( topHtml ) {
			var floatSpaceTpl = new kceditor.template(
				'<div' +
					' id="cke_{name}"' +
					' class="cke {id} cke_reset_all cke_chrome cke_editor_{name} cke_float cke_{langDir} ' + kceditor.env.cssClass + '"' +
					' dir="{langDir}"' +
					' title="' + ( kceditor.env.gecko ? ' ' : '' ) + '"' +
					' lang="{langCode}"' +
					' role="application"' +
					' style="{style}"' +
					...
					'</div>' +
				'</div>' ),
				floatSpace = kceditor.document.getBody().append( kceditor.dom.element.createFromHtml( floatSpaceTpl.output( {
					content: topHtml,
					id: editor.id,
				...
				changeBuffer = kceditor.tools.eventsBuffer( 500, layout ),
				uiBuffer = kceditor.tools.eventsBuffer( 100, layout );
