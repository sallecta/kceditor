...
( function() {
	...
	var commands = {
		toolbarFocus: {
			...
			exec: function( editor ) {
				if ( editor.toolbox ) {
					// Make the first button focus accessible for IE. (https://dev.kceditor.com/ticket/3417)
					// Adobe AIR instead need while of delay.
					if ( kceditor.env.ie || kceditor.env.air ) {
						setTimeout( function() {
	...
	kceditor.plugins.add( 'toolbar', {
		...
		init: function( editor ) {
			...
			var itemKeystroke = function( item, keystroke ) {
					...
					switch ( keystroke ) {
						case 9: // TAB
						case kceditor.SHIFT + 9: // SHIFT + TAB
						...
						case 38: // UP-ARROW
							...
							if ( next )
								...
								endFlag = 1;
								// Send a SHIFT + TAB.
								itemKeystroke( item, kceditor.SHIFT + 9 );
								endFlag = 0;
							}
						...
						case 32: // SPACE
							item.execute();
							return false;
						case kceditor.ALT + 122: // ALT + F11 (#438).
			...
			editor.on( 'uiSpace', function( event ) {
				...
				if ( editor.config.toolbarCanCollapse && editor.elementMode != kceditor.ELEMENT_MODE_INLINE )
				...
				for ( var r = 0; r < toolbarLength; r++ ) {
					...
					// at all in IE. (https://dev.kceditor.com/ticket/3983)
					if ( !row )
						continue;
					...
					for ( var i = 0; i < items.length; i++ ) {
						...
						if ( item ) {
							if ( item.type == kceditor.UI_SEPARATOR ) {
							...
							if ( !toolbarObj ) {
								...
								toolbarId = kceditor.tools.getNextId();
				...
				if ( editor.config.toolbarCanCollapse && editor.elementMode != kceditor.ELEMENT_MODE_INLINE ) {
					var collapserFn = kceditor.tools.addFunction( function() {
						editor.execCommand( 'toolbarCollapse' );
					} );

					editor.on( 'destroy', function() {
						kceditor.tools.removeFunction( collapserFn );
					} );
					...
					editor.setKeystroke( kceditor.ALT + ( kceditor.env.ie || kceditor.env.webkit ? 189 : 109 ) /*-*/, 'toolbarCollapse' );
					...
					output.push( '" onclick="kceditor.tools.callFunction(' + collapserFn + ')">', '<span class="cke_arrow">&#9650;</span>', // BLACK UP-POINTING TRIANGLE
						'</a>' );
				}
				...
				if ( this.toolbox ) {
					...
					for ( ; index < toolbars.length; index++ ) {
						...
						for ( i = 0; i < items.length; i++ ) {
							instance = items[ i ];
							if ( instance.clickFn )
								kceditor.tools.removeFunction( instance.clickFn );
							if ( instance.keyDownFn )
								kceditor.tools.removeFunction( instance.keyDownFn );
						}
					}
				}
			} );
			...
			editor.setKeystroke( kceditor.ALT + 121 /*F10*/, 'toolbarFocus' );

			editor.ui.add( '-', kceditor.UI_SEPARATOR, {} );
			editor.ui.addHandler( kceditor.UI_SEPARATOR, {
				create: function() {
					return {
	...
	function getToolbarConfig( editor ) {
		...
		function buildToolbarConfig() {
			...
			var toolbar = kceditor.tools.clone( editor.config.toolbarGroups ) || getPrivateToolbarGroups( editor );
		...
		function fillGroup( toolbarGroup, uiItems ) {
			if ( uiItems.length ) {
				...
				while ( ( item = uiItems.shift() ) ) {
					...
					if ( !removeButtons || kceditor.tools.indexOf( removeButtons, name ) == -1 ) {
		...
		function populateToolbarConfig( config ) {
			...
			for ( i = 0; i < config.length; ++i ) {
				...
				if ( group == '/' )
					toolbar.push( group );
				else if ( kceditor.tools.isArray( group ) ) {
					fillGroup( newGroup, kceditor.tools.clone( group ) );
					toolbar.push( newGroup );
				}
				else if ( group.items ) {
					fillGroup( newGroup, kceditor.tools.clone( group.items ) );
	...
	/**
	 * Adds a toolbar group. See {@link kceditor.config#toolbarGroups} for more details.
	 *
	 * **Note:** This method will not modify toolbar groups set explicitly by
	 * {@link kceditor.config#toolbarGroups}. It will only extend the default setting.
	 *
	 ...
	 * @member kceditor.ui
	 */
	kceditor.ui.prototype.addToolbarGroup = function( name, previous, subgroupOf ) {
		...
		if ( subgroupOf ) {
			...
			subgroupOf = kceditor.tools.search( toolbarGroups, function( group ) {
			...
			if ( subgroupOf ) {
				...
				if ( previous ) {
					...
					previous = kceditor.tools.indexOf( subgroupOf.groups, previous );
					if ( previous >= 0 ) {
		...
		if ( previous ) {
			...
			previous = kceditor.tools.indexOf( toolbarGroups, function( group ) {
 ...
 * @member kceditor
 */
kceditor.UI_SEPARATOR = 'separator';
 ...
 */
kceditor.config.toolbarLocation = 'top';
 ...
 *
 * In KCEditor 4.5.0+ you can generate your toolbar customization code by using the {@glink features/toolbar visual
 ...
 *		// This second part of the setting name can be anything. You must use this name in the kceditor.config.toolbar setting
 ...
 * this setting is used to group all defined buttons (see {@link kceditor.ui#addButton}).
 ...
 * {@link kceditor.ui#addToolbarGroup}. This is only possible if the default setting was used.
 *
 *		// Default setting.
 ...
 * {@link kceditor.config#removePlugins} setting to remove features from the editor
 * or even better, [create a custom editor build](https://github.com/sallecta/kceditor/cke4/builder) with
 ...
 * {@link kceditor.config#toolbar} option if it is set or automatically
 * based on {@link kceditor.config#toolbarGroups}.
 *
 * @readonly
