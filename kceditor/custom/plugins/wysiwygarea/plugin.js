...
( function() {
	...
	kceditor.plugins.add( 'wysiwygarea', {
		init: function( editor ) {
			...
			editor.addMode( 'wysiwyg', function( callback ) {
				...
				// Microsoft Edge throws "Permission Denied" if treated like an IE (https://dev.kceditor.com/ticket/13441).
				if ( kceditor.env.air ) {
					src = 'javascript:void(0)'; // jshint ignore:line
				} else if ( kceditor.env.ie && !kceditor.env.edge ) {
					src = 'javascript:void(function(){' + encodeURIComponent( src ) + '}())'; // jshint ignore:line
				} else {
					src = '';
				}

				var iframe = kceditor.dom.element.createFromHtml( '<iframe src="' + src + '" frameBorder="0"></iframe>' );
				...
				var useOnloadEvent = ( kceditor.env.ie && !kceditor.env.edge ) || kceditor.env.gecko;
				if ( useOnloadEvent )
				...
				var frameLabel = editor.title,
					...
					isIE11 = kceditor.env.ie && kceditor.env.version === 11,
					isMutationObserverSupported = !!window.MutationObserver,
					mutationObserver;

				if ( frameLabel ) {
					if ( kceditor.env.ie && helpLabel )
				...
				if ( helpLabel ) {
					var labelId = kceditor.tools.getNextId(),
						desc = kceditor.dom.element.createFromHtml( '<span id="' + labelId + '" class="cke_voice_label">' + helpLabel + '</span>' );
	...
	/**
	 * Adds the path to a stylesheet file to the exisiting {@link kceditor.config#contentsCss} value.
	 *
	 ...
	 */
	kceditor.editor.prototype.addContentsCss = function( cssPath ) {
		var cfg = this.config,
			curContentsCss = cfg.contentsCss;

		// Convert current value into array.
		if ( !kceditor.tools.isArray( curContentsCss ) )
	...
	function onDomReady( win ) {
		...
		if ( kceditor.env.ie ) {
			...
			// taking the editing focus at startup. (https://dev.kceditor.com/ticket/141 / https://dev.kceditor.com/ticket/523)
			body.disabled = true;
		...
		doc = new kceditor.dom.document( doc );
		...
		// Without it IE8 has problem with removing selection in nested editable. (https://dev.kceditor.com/ticket/13785)
		if ( kceditor.env.ie && !kceditor.env.edge ) {
			doc.getDocumentElement().addClass( doc.$.compatMode );
		}

		// Prevent IE/Edge from leaving a new paragraph/div after deleting all contents in body (https://dev.kceditor.com/ticket/6966, https://dev.kceditor.com/ticket/13142).
		if ( kceditor.env.ie && !kceditor.env.edge && editor.enterMode != kceditor.ENTER_P ) {
			removeSuperfluousElement( 'p' );
		}
		// Starting from Edge 15 additional `div` is not added to the editor.
		else if ( kceditor.env.edge && kceditor.env.version < 15 && editor.enterMode != kceditor.ENTER_DIV  ) {
			removeSuperfluousElement( 'div' );
		}

		// Fix problem with cursor not appearing in Webkit and IE11+ when clicking below the body (https://dev.kceditor.com/ticket/10945, https://dev.kceditor.com/ticket/10906).
		// Fix for older IEs (8-10 and QM) is placed inside selection.js.
		if ( kceditor.env.webkit || ( kceditor.env.ie && kceditor.env.version > 10 ) ) {
		...
		if ( kceditor.env.gecko || kceditor.env.ie && editor.document.$.compatMode == 'CSS1Compat' ) {
			this.attachListener( this, 'keydown', function( evt ) {
				...
				if ( keyCode == 33 || keyCode == 34 ) {
					...
					// with standard doctype, manually fix it. (https://dev.kceditor.com/ticket/4736)
					if ( kceditor.env.ie ) {
						setTimeout( function() {
					...
					// when editor contents are not overflowed. (https://dev.kceditor.com/ticket/7955)
		...
		if ( kceditor.env.ie ) {
			...
			// for the dialog. (https://dev.kceditor.com/ticket/8157)
			// [IE<8 & Opera] Even worse For old IEs, the cursor will not vanish even if
			// the selection has been moved to another text input in some cases. (https://dev.kceditor.com/ticket/4716)
			//
			// Now the range restore is disabled, so we simply force IE to clean
			// up the selection before blur.
			this.attachListener( doc, 'blur', function() {
				// Error proof when the editor is not visible. (https://dev.kceditor.com/ticket/6375)
				try {
					doc.$.selection.empty();
				} catch ( er ) {}
			} );
		...
		if ( kceditor.env.iOS ) {
			...
			// event and body becomes the focused element (https://dev.kceditor.com/ticket/10714).
		...
		// document.title is malfunctioning on Chrome, so get value from the element (https://dev.kceditor.com/ticket/12402).
		...
		function removeSuperfluousElement( tagName ) {
			...
			// leak fake selection's content into editable due to removing hidden selection container (https://dev.kceditor.com/ticket/14831).
			editable.attachListener( editable, 'keyup', function() {
				var elements = doc.getElementsByTag( tagName );
				if ( lockRetain ) {
					if ( elements.count() == 1 && !elements.getItem( 0 ).getCustomData( 'retain' ) &&
						kceditor.tools.isEmpty( elements.getItem( 0 ).getAttributes() ) ) {
	...
	framedWysiwyg = kceditor.tools.createClass( {
		$: function() {
			this.base.apply( this, arguments );

			this._.frameLoadedHandler = kceditor.tools.addFunction( function( win ) {
				// Avoid opening design mode in a frame window thread,
				// which will cause host page scrolling.(https://dev.kceditor.com/ticket/4397)
				kceditor.tools.setTimeout( onDomReady, 0, this, win );
			}, this );
		...
		proto: {
			...
			setData: function( data, isSnapshot ) {
				...
				if ( isSnapshot ) {
					...
					// and because it makes sense. (https://dev.kceditor.com/ticket/10370)
					editor.fire( 'dataReady' );
				}
				else {
					...
					var headExtra = kceditor.tools.buildStyleHtml( iframeCssFixes() ).replace( /<style>/, '<style data-cke-temp="1">' );

					if ( !fullPage )
						headExtra += kceditor.tools.buildStyleHtml( editor.config.contentsCss );
					...
					if ( editor.title ) {
						data = data.replace( /<body/, '<body aria-label="' +
							kceditor.tools.htmlEncodeAttr( editor.title ) + '"' );
					}
					...
					// is fully editable even before the editing iframe is fully loaded (https://dev.kceditor.com/ticket/4455).
					var bootstrapCode =
						'<script id="cke_actscrpt" type="text/javascript"' + ( kceditor.env.ie ? ' defer="defer" ' : '' ) + '>' +
							'var wasLoaded=0;' +	// It must be always set to 0 as it remains as a window property.
							'function onload(){' +
								'if(!wasLoaded)' +	// FF3.6 calls onload twice when editor.setData. Stop that.
									'window.parent.kceditor && window.parent.kceditor.tools.callFunction(' + this._.frameLoadedHandler + ',window);' +
								'wasLoaded=1;' +
							'}' +
							( kceditor.env.ie ? 'onload();' : 'document.addEventListener("DOMContentLoaded", onload, false );' ) +
						'</script>';
					...
					if ( kceditor.env.ie && kceditor.env.version < 9 ) {
						bootstrapCode +=
							'<script id="cke_shimscrpt">' +
								'window.parent.kceditor.tools.enableHtml5Elements(document)' +
							'</script>';
					}

					// IE<10 needs this hack to properly enable <base href="...">.
					// See: http://stackoverflow.com/a/13373180/1485219 (https://dev.kceditor.com/ticket/11910).
					if ( baseTag && kceditor.env.ie && kceditor.env.version < 10 ) {
						bootstrapCode +=
					...
					// Work around Firefox bug - error prune when called from XUL (https://dev.kceditor.com/ticket/320),
			...
			getData: function( isSnapshot ) {
				if ( isSnapshot )
					...
					// BR at the end of document is bogus node for Mozilla. (https://dev.kceditor.com/ticket/5293).
					// Prevent BRs from disappearing from the end of the content
					// while enterMode is ENTER_BR (https://dev.kceditor.com/ticket/10146).
					if ( kceditor.env.gecko && config.enterMode != kceditor.ENTER_BR )
			...
			detach: function() {
				...
				kceditor.tools.removeFunction( this._.frameLoadedHandler );

				// On IE, iframe is returned even after remove() method is called on it.
				// Checking if parent is present fixes this issue. (https://dev.kceditor.com/ticket/13850)
				if ( iframe ) {
					...
					// iframe offline first. (https://dev.kceditor.com/ticket/3812 and https://dev.kceditor.com/ticket/5441)
	...
	function objectResizeDisabler( editor ) {
		...
		} else if ( kceditor.env.ie && kceditor.env.version < 11 && editor.config.disableObjectResizing ) {
	...
	function iframeCssFixes() {
		...
		// on element unless it has layout. (https://dev.kceditor.com/ticket/5562)
		if ( kceditor.document.$.documentMode >= 8 ) {
			css.push( 'html.CSS1Compat [contenteditable=false]{min-height:0 !important}' );

			var selectors = [];

			for ( var tag in kceditor.dtd.$removeEmpty )
				selectors.push( 'html.CSS1Compat ' + tag + '[contenteditable=false]' );

			css.push( selectors.join( ',' ) + '{display:inline-block}' );
		}
		// Set the HTML style to 100% to have the text cursor in affect (https://dev.kceditor.com/ticket/6341)
		else if ( kceditor.env.gecko ) {
			css.push( 'html{height:100% !important}' );
			css.push( 'img:-moz-broken{-moz-force-broken-image-icon:1;min-width:24px;min-height:24px}' );
		}

		// https://dev.kceditor.com/ticket/6341: The text cursor must be set on the editor area.
		// https://dev.kceditor.com/ticket/6632: Avoid having "text" shape of cursor in IE7 scrollbars.
 ...
 * this option does not work for inline editors (see ticket [#10197](https://dev.kceditor.com/ticket/10197)),
 * does not work in Internet Explorer 11+ (see [#9317](https://dev.kceditor.com/ticket/9317#comment:16) and
 ...
 */
kceditor.config.disableObjectResizing = false;
 ...
 * @member kceditor.config
 */
kceditor.config.disableNativeTableHandles = true;
 ...
 * not appear in KCEditor's default context menu,
 ...
 * [context menu](https://github.com/sallecta/kceditor/cke4/addon/contextmenu) plugin.
 *
 ...
 * @member kceditor.config
 */
kceditor.config.disableNativeSpellChecker = true;
 ...
 * editor content.
 *
 *		config.baseHref = 'http://www.kceditor.com/path/';
 *
 * @cfg {String} [baseHref='']
 * @member kceditor.config
 */
 ...
 * @param {kceditor.dom.element} data The element being added.
 */
 ...
 *
 * @since 4.17.0
 * @cfg {HTMLElement} [observableParent=kceditor.document.$]
 * @member kceditor.config
 */
kceditor.config.observableParent = kceditor.document.$;
