...
( function() {
	var keystrokes = [
			kceditor.CTRL + 90 /*Z*/,
			kceditor.CTRL + 89 /*Y*/,
			kceditor.CTRL + kceditor.SHIFT + 90 /*Z*/
		],
		backspaceOrDelete = { 8: 1, 46: 1 };

	kceditor.plugins.add( 'undo', {
		...
		init: function( editor ) {
			...
			undoManager.onChange = function() {
				undoCommand.setState( undoManager.undoable() ? kceditor.TRISTATE_OFF : kceditor.TRISTATE_DISABLED );
				redoCommand.setState( undoManager.redoable() ? kceditor.TRISTATE_OFF : kceditor.TRISTATE_DISABLED );
			};
			 ...
			 *
			 * See {@link kceditor.plugins.undo.UndoManager#lock} for more details.
			 *
			 ...
			 * with the current content and selection. Read more in the {@link kceditor.plugins.undo.UndoManager#lock} method.
	...
	kceditor.plugins.undo = {};

	/**
	 * Main logic for the Redo/Undo feature.
	 *
	 * @private
	 * @class kceditor.plugins.undo.UndoManager
	 * @constructor Creates an UndoManager class instance.
	 * @param {kceditor.editor} editor
	 */
	var UndoManager = kceditor.plugins.undo.UndoManager = function( editor ) {
		/**
		...
		/**
		 * The maximum number of snapshots in the stack. Configurable via {@link kceditor.config#undoStackSize}.
		 *
		 * @readonly
	...
	UndoManager.prototype = {
		...
		/**
		 * Refreshes the state of the {@link kceditor.plugins.undo.UndoManager undo manager}
		 ...
		 * @param {kceditor.plugins.undo.Image} image An optional image to save. If skipped, current editor will be used.
		 * @param {Boolean} [autoFireChange=true] If set to `false`, will not trigger the {@link kceditor.editor#change} event to editor.
		 */
		 ...
		 *
		 * @param {kceditor.plugins.undo.Image} image
		 */
		restoreImage: function( image ) {
			...
			if ( image.bookmarks ) {
				editor.focus();
				// Retrieve the selection beforehand. (https://dev.kceditor.com/ticket/8324)
			...
			// the original snapshot due to dom change. (https://dev.kceditor.com/ticket/4622)
			this.update();
			this.refreshState();
		 ...
		 * @returns {kceditor.plugins.undo.Image} Next image or `null`.
		 */
		 ...
		 *
		 * @param {kceditor.plugins.undo.Image} [newImage] The image which will replace the current one.
		 ...
		 *
		 * @since 4.4.4
		 * @param {kceditor.plugins.undo.Image} newSnapshot New snapshot with new selection.
			...
			if ( !this.locked ) {
				if ( dontUpdate )
					...
					if ( forceUpdate )
						...
						// https://dev.kceditor.com/ticket/11027#comment:3
	...
	UndoManager.ieFunctionalKeysBug = function( keyCode ) {
		return kceditor.env.ie && UndoManager.getKeyGroup( keyCode ) == UndoManager.keyGroups.FUNCTIONAL;
	};
	 ...
	 *
	 * @private
	 * @class kceditor.plugins.undo.Image
	 * @constructor Creates an Image class instance.
	 * @param {kceditor.editor} editor The editor instance on which the image is created.
	 * @param {Boolean} [contentsOnly] If set to `true`, the image will only contain content without the selection.
	 */
	var Image = kceditor.plugins.undo.Image = function( editor, contentsOnly ) {
			this.editor = editor;
	...
	function applyRules( data, rules ) {
		return kceditor.tools.array.reduce( rules, function( currentData, rule ) {
	...
	Image.prototype = {
		/**
		 * @param {kceditor.plugins.undo.Image} otherImage Image to compare to.
		...
		equalsContent: function( otherImage ) {
			...
			// For IE7 and IE QM: Comparing only the protected attribute values but not the original ones.(https://dev.kceditor.com/ticket/4522)
			...
			if ( bookmarksA || bookmarksB ) {
				...
				for ( var i = 0; i < bookmarksA.length; i++ ) {
					...
					if ( bookmarkA.startOffset != bookmarkB.startOffset || bookmarkA.endOffset != bookmarkB.endOffset ||
						!kceditor.tools.arrayCompare( bookmarkA.start, bookmarkB.start ) ||
						!kceditor.tools.arrayCompare( bookmarkA.end, bookmarkB.end ) ) {
						return false;
					}
				}
			}
		 ...
		 *
		 * @readonly
		 * @property {Object[]} bookmarks Array of bookmark2 objects, see {@link kceditor.dom.range#createBookmark2} for definition.
		 */
	};
	 ...
	 * @since 4.4.4
	 * @private
	 * @class kceditor.plugins.undo.NativeEditingHandler
	 * @member kceditor.plugins.undo Undo manager owning the handler.
	 * @constructor
	 * @param {kceditor.plugins.undo.UndoManager} undoManager
	 */
	var NativeEditingHandler = kceditor.plugins.undo.NativeEditingHandler = function( undoManager ) {
		 ...
		 *
		 * @property {kceditor.plugins.undo.UndoManager} undoManager
		 */
		 ...
		 *
		 * @since 4.4.5
		 * @property {kceditor.plugins.undo.KeyEventsStack} keyEventsStack
		 */
		 ...
		 *
		 * @property {kceditor.plugins.undo.Image} lastKeydownImage
		 */
	...
	NativeEditingHandler.prototype = {
		...
		onKeydown: function( evt ) {
			...
			// The composition is in progress - ignore the key. (https://dev.kceditor.com/ticket/12597)
			if ( keyCode === 229 ) {
				return;
			}

			// Block undo/redo keystrokes when at the bottom/top of the undo stack (https://dev.kceditor.com/ticket/11126 and https://dev.kceditor.com/ticket/11677).
			if ( kceditor.tools.indexOf( keystrokes, evt.data.getKeystroke() ) > -1 ) {
			...
			if ( UndoManager.isNavigationKey( keyCode ) || this.undoManager.keyGroupChanged( keyCode ) ) {
				if ( undoManager.strokesRecorded[ 0 ] || undoManager.strokesRecorded[ 1 ] ) {
					...
					// https://dev.kceditor.com/ticket/12300
		...
		onKeyup: function( evt ) {
			...
			// Also we need to be aware that lastKeydownImage might not be available (https://dev.kceditor.com/ticket/12327).
		...
		attachListeners: function() {
			...
			editable.attachListener( editable, ( kceditor.env.ie ? 'keypress' : 'input' ), that.onInput, that, null, 999 );
			...
			// https://dev.kceditor.com/ticket/12324 comment:4
	 ...
	 * @since 4.4.5
	 * @private
	 * @class kceditor.plugins.undo.KeyEventsStack
	 * @constructor
	 */
	var KeyEventsStack = kceditor.plugins.undo.KeyEventsStack = function() {
		/**
		 * @readonly
		 */
		this.stack = [];
	...
	KeyEventsStack.prototype = {
		 ...
		 *
		 * @param {kceditor.dom.event} event
		 */
 ...
 * @member kceditor.editor
 * @param {kceditor.editor} editor This editor instance.
 * @see kceditor.editor#afterUndoImage
 */
 ...
 * @member kceditor.editor
 * @param {kceditor.editor} editor This editor instance.
 * @see kceditor.editor#beforeUndoImage
 */
