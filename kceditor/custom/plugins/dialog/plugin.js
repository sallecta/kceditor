 ...
 * @member kceditor
 */
kceditor.DIALOG_RESIZE_WIDTH = 1;
 ...
 * @member kceditor
 */
kceditor.DIALOG_RESIZE_HEIGHT = 2;
 ...
 * @member kceditor
 */
kceditor.DIALOG_STATE_BUSY = 2;

( function() {
	'use strict';

	var cssLength = kceditor.tools.cssLength,
	...
	function getPreviousVisibleTab() {
		var tabId = this._.currentTabId,
			...
			tabIndex = kceditor.tools.indexOf( this._.tabIdList, tabId ) + length;
	...
	function getNextVisibleTab() {
		var tabId = this._.currentTabId,
			length = this._.tabIdList.length,
			tabIndex = kceditor.tools.indexOf( this._.tabIdList, tabId );
	...
	function clearOrRecoverTextInputValue( container, isRecover ) {
		...
		for ( var i = 0, length = inputs.length; i < length; i++ ) {
			var item = new kceditor.dom.element( inputs[ i ] );
	...
	var useFlex = !kceditor.env.ie || kceditor.env.edge,
		templateSource = '<div class="cke_reset_all cke_dialog_container {editorId} {editorDialogClass} {hidpi}' +
			...
			'>' +
			'<table class="cke_dialog ' + kceditor.env.cssClass + ' cke_{langDir}"' +
	...
	function buildDialog( editor ) {
		var element = kceditor.dom.element.createFromHtml( kceditor.addTemplate( 'dialog', templateSource ).output( {
			id: kceditor.tools.getNextNumber(),
			editorId: editor.id,
			langDir: editor.lang.dir,
			langCode: editor.langCode,
			editorDialogClass: 'cke_editor_' + editor.name.replace( /\./g, '\\.' ) + '_dialog',
			closeTitle: editor.lang.common.close,
			hidpi: kceditor.env.hidpi ? 'cke_hidpi' : ''
		} ) );
		...
		// Don't allow dragging on dialog (https://dev.kceditor.com/ticket/13184).
		editor.plugins.clipboard && kceditor.plugins.clipboard.preventDefaultDropOnElement( body );

		// IFrame shim for dialog that masks activeX in IE. (https://dev.kceditor.com/ticket/7619)
		if ( kceditor.env.ie && !kceditor.env.quirks && !kceditor.env.edge ) {
			var src = 'javascript:void(function(){' + encodeURIComponent( 'document.open();(' + kceditor.tools.fixDomain + ')();document.close();' ) + '}())', // jshint ignore:line
				iframe = kceditor.dom.element.createFromHtml( '<iframe' +
					' frameBorder="0"' +
	 ...
	 *
	 *		var dialogObj = new kceditor.dialog( editor, 'smiley' );
	 *
	 * @class
	 ...
	 */
	kceditor.dialog = function( editor, dialogName ) {
		// Load the dialog definition.
		var definition = kceditor.dialog._.dialogDefinitions[ dialogName ],
			defaultDefinition = kceditor.tools.clone( defaultDialogDefinition ),
			buttonsOrder = editor.config.dialog_buttonsOrder || 'OS',
			dir = editor.lang.dir,
			tabsToRemove = {},
			i, processed, stopPropagation;

		if ( ( buttonsOrder == 'OS' && kceditor.env.mac ) || // The buttons in MacOS Apps are in reverse order (https://dev.kceditor.com/ticket/4750)
		( buttonsOrder == 'rtl' && dir == 'ltr' ) || ( buttonsOrder == 'ltr' && dir == 'rtl' ) )
			defaultDefinition.buttons.reverse();


		// Completes the definition with the default values.
		definition = kceditor.tools.extend( definition( editor ), defaultDefinition );

		// Clone a functionally independent copy for this dialog.
		definition = kceditor.tools.clone( definition );
		...
		kceditor.tools.setTimeout( function() {
		...
		this.definition = definition = kceditor.fire( 'dialogDefinition', {
			name: dialogName,
		...
		if ( definition.onOk ) {
			this.on( 'ok', function( evt ) {
				// Dialog confirm might probably introduce content changes (https://dev.kceditor.com/ticket/5415).
		...
		this.state = kceditor.DIALOG_STATE_IDLE;
		...
		function changeFocus( offset ) {
			...
			do {
				...
				if ( hasTabs && !me._.tabBarMode && ( currentIndex == focusList.length || currentIndex == -1 ) ) {
					// If the dialog was not in tab mode, then focus the first tab (https://dev.kceditor.com/ticket/13027).
		...
		function keydownHandler( evt ) {
			...
			if ( me != kceditor.dialog._.currentTop )
				return;
			...
			if ( keystroke == 9 || keystroke == kceditor.SHIFT + 9 ) {
				var shiftPressed = ( keystroke == kceditor.SHIFT + 9 );
				changeFocus( shiftPressed ? -1 : 1 );
				processed = 1;
			} else if ( keystroke == kceditor.ALT + 121 && !me._.tabBarMode && me.getPageCount() > 1 ) {
				// Alt-F10 puts focus into the current tab item in the tab bar.
				focusActiveTab( me );
				processed = 1;
			} else if ( kceditor.tools.indexOf( arrowKeys, keystroke ) != -1 && me._.tabBarMode ) {
				...
				var prevKeyCodes = [
					...
					nextId = kceditor.tools.indexOf( prevKeyCodes, keystroke ) != -1 ? getPreviousVisibleTab.call( me ) : getNextVisibleTab.call( me );
			...
			// If user presses enter key in a text box, it implies clicking OK for the dialog.
				...
				if ( !target.is( 'a', 'button', 'select', 'textarea' ) && ( !target.is( 'input' ) || target.$.type != 'button' ) ) {
					...
					button && kceditor.tools.setTimeout( button.click, 0, button );
					processed = 1;
				}
				stopPropagation = 1; // Always block the propagation (https://dev.kceditor.com/ticket/4269)
			} else if ( keystroke == 27 /*ESC*/ ) {
				button = this.getButton( 'cancel' );

				// If there's a Cancel button, click it, else just fire the cancel event and hide the dialog.
				if ( button )
					kceditor.tools.setTimeout( button.click, 0, button );
				else {
					if ( this.fire( 'cancel', { hide: true } ).hide !== false )
						this.hide();
				}
		...
		this.on( 'show', function() {
			...
			// keypress. So we must do a longer trip in those cases. (https://dev.kceditor.com/ticket/4531,https://dev.kceditor.com/ticket/8985)
		...
		this.on( 'iframeAdded', function( evt ) {
			var doc = new kceditor.dom.document( evt.data.iframe.$.contentWindow.document );
		...
		this.on( 'show', function() {
			...
			if ( editor.config.dialog_startupFocusTab && hasTabs ) {
				...
				// https://dev.kceditor.com/ticket/13114#comment:4.
		...
		// IE6 BUG: Text fields and text areas are only half-rendered the first time the dialog appears in IE6 (https://dev.kceditor.com/ticket/2661).
		...
		// Insert the title.
		( new kceditor.dom.text( definition.title, kceditor.document ) ).appendTo( this.parts.title );
		...
		// Insert buttons.
		var buttonsHtml = [],
			buttons = kceditor.dialog._.uiElementBuilders.hbox.build( this, {
				type: 'hbox',
		 ...
		 *
		 * @readonly
		 * @property {Number} [state=kceditor.DIALOG_STATE_IDLE]
		 */
	};
	...
	function resizeWithWindow( dialog ) {
		var win = kceditor.document.getWindow();
	...
	kceditor.dialog.prototype = {
		...
		move: function( x, y, save ) {
			...
			// (https://dev.kceditor.com/ticket/8888) In some cases of a very small viewport, dialog is incorrectly
		...
		getPosition: function() {
			return kceditor.tools.extend( {}, this._.position );
		},
		...
		show: function() {
			...
			var element = this._.element,
				...
				documentBody = kceditor.document.getBody(),
			...
			if ( kceditor.dialog._.currentZIndex === null ) {
				kceditor.dialog._.currentZIndex = baseFloatZIndex;
			}
			this._.element.getFirst().setStyle( 'z-index', kceditor.dialog._.currentZIndex += 10 );
			this.getElement().setStyle( 'z-index', kceditor.dialog._.currentZIndex );

			// Maintain the dialog ordering and dialog cover.
			if ( kceditor.dialog._.currentTop === null ) {
				kceditor.dialog._.currentTop = this;
				this._.parentDialog = null;
				showCover( this._.editor );
			} else if ( kceditor.dialog._.currentTop !== this ) {
				// Reposition the new dialog only if the current dialog is not already on the top (#3638).
				this._.parentDialog = kceditor.dialog._.currentTop;
		...
		layout: function() {
			...
			var dialogSize = this.getSize(),
				win = kceditor.document.getWindow(),
		 ...
		 * @returns {kceditor.dialog} The current dialog object.
		 */
		foreach: function( fn ) {
		...
		/**
		 * Calls the {@link kceditor.dialog.definition.uiElement#setup} method of each
		...
		/**
		 * Calls the {@link kceditor.dialog.definition.uiElement#commit} method of each
		 * of the UI elements, with the arguments passed through it.
		...
		commitContent: function() {
			var args = arguments;
			this.foreach( function( widget ) {
				// Make sure IE triggers "change" event on last focused input before closing the dialog. (https://dev.kceditor.com/ticket/7915)
				if ( kceditor.env.ie && this._.currentFocusIndex == widget.focusIndex )
		...
		hide: function() {
			...
			while ( kceditor.dialog._.currentTop != this ) {
				kceditor.dialog._.currentTop.hide();
			}
			...
			kceditor.dialog._.currentTop = this._.parentDialog;

			// Deduct or clear the z-index.
			if ( !this._.parentDialog ) {
				kceditor.dialog._.currentZIndex = null;
				...
				// Give a while before unlock, waiting for focus to return to the editable. (https://dev.kceditor.com/ticket/172)
				setTimeout( function() {
					editor.focusManager.unlock();

					// Fixed iOS focus issue (https://dev.kceditor.com/ticket/12381).
				...
				kceditor.dialog._.currentZIndex -= 10;
			}
			...
			// Reset dialog state back to IDLE, if busy (https://dev.kceditor.com/ticket/13213).
			this.setState( kceditor.DIALOG_STATE_IDLE );
		},
		...
		addPage: function( contents ) {
			...
			var pageHtml = [],
				titleHtml = contents.label ? ' title="' + kceditor.tools.htmlEncode( contents.label ) + '"' : '',
				vbox = kceditor.dialog._.uiElementBuilders.vbox.build( this, {
					type: 'vbox',
			...
			var page = kceditor.dom.element.createFromHtml( pageHtml.join( '' ) );
			page.setAttribute( 'role', 'tabpanel' );
			page.setStyle( 'min-height', '100%' );

			var env = kceditor.env,
				tabId = 'cke_' + contents.id + '_' + kceditor.tools.getNextNumber(),
				tab = kceditor.dom.element.createFromHtml( [
		...
		selectPage: function( id ) {
			...
			// before it shows and then recover it (https://dev.kceditor.com/ticket/5649)
			if ( kceditor.env.ie6Compat || kceditor.env.ie7Compat ) {
			...
			this._.currentTabIndex = kceditor.tools.indexOf( this._.tabIdList, id );
		},
		 ...
		 *
		 * @returns {kceditor.dom.element} The `<span>` element containing this dialog.
		 */
		getElement: function() {
		 ...
		 * @returns {kceditor.ui.dialog.uiElement} The dialog UI element.
		 */
		 ...
		 *
		 *		@returns {kceditor.ui.dialog.button} The button object.
		 *
		 ...
		 *
		 * @returns {kceditor.editor} Parent editor instances.
		 */
		 ...
		 *
		 * @returns {kceditor.dom.element} The element that was selected, or `null`.
		 */
		 ...
		 *
		 * @param {kceditor.dom.element} element
		 * @param {Number} [index]
		 */
		 ...
		 *
		 * @since 4.5.0
		 * @param {Number} state Either {@link kceditor#DIALOG_STATE_IDLE} or {@link kceditor#DIALOG_STATE_BUSY}.
		 */
		setState: function( state ) {
			...
			if ( state == kceditor.DIALOG_STATE_BUSY ) {
				...
				if ( !this.parts.spinner ) {
					...
					this.parts.spinner = kceditor.document.createElement( 'div', spinnerDef );
			...
			} else if ( state == kceditor.DIALOG_STATE_IDLE ) {
		...
		/**
		 * @inheritdoc kceditor.dialog.definition#getModel
		 */
		 ...
		 * {@link kceditor.dom.element DOM element instance} if there is an element related to the dialog.
		 * For widget plugins (`image2`, `placeholder`) you should provide a {@link kceditor.plugins.widget} instance that
		 * is the subject of this dialog.
		 *
		 * @since 4.13.0
		 * @private
		 * @param {kceditor.dom.element/kceditor.plugins.widget/Object/null} newModel The model to be set.
		 */
		 ...
		 *
		 * In case if the dialog definition did not define the {@link kceditor.dialog.definition#getMode}
		 * function, it will use the {@link #getModel} method to recognize the editor mode:
		 *
		 * The {@link kceditor.dialog#EDITING_MODE editing mode} is used when the method returns:
		 *
		 * * A {@link kceditor.dom.element} attached to the DOM.
		 * * A {@link kceditor.plugins.widget} instance.
		 *
		 * Otherwise the {@link kceditor.dialog#CREATION_MODE creation mode} is used.
		 *
		 * @since 4.13.0
		 * @param {kceditor.editor} editor
		...
		getMode: function( editor ) {
			...
			if ( !model || ( model instanceof kceditor.dom.element && !model.getParent() ) ) {
				return kceditor.dialog.CREATION_MODE;
			}

			return kceditor.dialog.EDITING_MODE;
		}
	};

	kceditor.tools.extend( kceditor.dialog, {
		 ...
		 *		kceditor.plugins.add( 'myplugin', {
		 *			init: function( editor ) {
		 *				editor.addCommand( 'mydialog',new kceditor.dialogCommand( 'mydialog' ) );
		 *
		 ...
		 *						return { 'My Dialog': kceditor.TRISTATE_OFF };
		 *					} );
		 *				}
		 *
		 *				kceditor.dialog.add( 'mydialog', function( api ) {
		 *					// kceditor.dialog.definition
		 ...
		 *							}
		 *						],
		 *						buttons: [ kceditor.dialog.okButton, kceditor.dialog.cancelButton ],
		 *						onOk: function() {
		 *							// "this" is now a kceditor.dialog object.
		 ...
		 *		} );
		 *
		 *		kceditor.replace( 'editor1', { extraPlugins: 'myplugin' } );
		 *
		 * @static
		 ...
		 * return an object conforming to {@link kceditor.dialog.definition}.
		 * @see kceditor.dialog.definition
		 */
		 ...
		 */
		getCurrent: function() {
			return kceditor.dialog._.currentTop;
		},

		/**
		 * Check whether tab wasn't removed by {@link kceditor.config#removeDialogTabs}.
		 *
		 * @since 4.1.0
		 * @static
		 * @param {kceditor.editor} editor
		...
		okButton: ( function() {
			...
			retval.override = function( override ) {
				return kceditor.tools.extend( function( editor ) {
	...
	kceditor.event.implementOn( kceditor.dialog );
	kceditor.event.implementOn( kceditor.dialog.prototype );

	defaultDialogDefinition = {
		resizable: kceditor.DIALOG_RESIZE_BOTH,
		minWidth: 600,
		minHeight: 400,
		buttons: [ kceditor.dialog.okButton, kceditor.dialog.cancelButton ]
	};
	 ...
	 *
	 *		kceditor.on( 'dialogDefinition', function( evt ) {
	 ...
	 *
	 * @private
	 * @class kceditor.dialog.definitionObject
	 * @extends kceditor.dialog.definition
	...
	function definitionObject( dialog, dialogDefinition ) {
		...
		kceditor.tools.extend( this, dialogDefinition );
	}
	...
	definitionObject.prototype = {
		 ...
		 * @returns {kceditor.dialog.definition.content} The content definition matching id.
		 */
		 ...
		 * @returns {kceditor.dialog.definition.button} The button definition matching id.
		 */
		getButton: function( id ) {
		 ...
		 *
		 * @param {kceditor.dialog.definition.content} contentDefinition The
		 * content definition.
		 * @param {String} [nextSiblingId] The id of an existing content
		 * definition which the new content definition will be inserted
		 * before. Omit if the new content definition is to be inserted as
		 * the last item.
		 * @returns {kceditor.dialog.definition.content} The inserted content definition.
		 */
		 ...
		 *
		 * @param {kceditor.dialog.definition.button} buttonDefinition The
		 * button definition.
		 * @param {String} [nextSiblingId] The id of an existing button
		 * definition which the new button definition will be inserted
		 * before. Omit if the new button definition is to be inserted as
		 * the last item.
		 * @returns {kceditor.dialog.definition.button} The inserted button definition.
		 */
		 ...
		 * @returns {kceditor.dialog.definition.content} The removed content definition.
		 */
		 ...
		 * @returns {kceditor.dialog.definition.button} The removed button definition.
		 */
	 ...
	 * kceditor.dialog.definitionObject.
	 *
	 *		kceditor.on( 'dialogDefinition', function( evt ) {
	 *			var definition = evt.data.definition;
	 *			var content = definition.getContents( 'page1' );
	 ...
	 *
	 * @private
	 * @class kceditor.dialog.definition.contentObject
	...
	function contentObject( dialog, contentDefinition ) {
		...
		kceditor.tools.extend( this, contentDefinition );
	}
	...
	contentObject.prototype = {
		 ...
		 * @returns {kceditor.dialog.definition.uiElement}
		 */
		get: function( id ) {
		 ...
		 *
		 * @param {kceditor.dialog.definition.uiElement} elementDefinition The
		 * UI elemnet definition to be added.
		 * @param {String} nextSiblingId The id of an existing UI element
		 * definition which the new UI element definition will be inserted
		 * before. Omit if the new button definition is to be inserted as
		 * the last item.
		 * @returns {kceditor.dialog.definition.uiElement} The element definition inserted.
		 */
		 ...
		 * @returns {kceditor.dialog.definition.uiElement} The element definition removed.
		 */
		remove: function( id ) {
	...
	function initDragAndDrop( dialog ) {
		var lastCoords = null,
			...
			margins = kceditor.skin.margins || [ 0, 0, 0, 0 ];
		...
		function mouseUpHandler() {
			kceditor.document.removeListener( 'mousemove', mouseMoveHandler );
			kceditor.document.removeListener( 'mouseup', mouseUpHandler );
		...
		dialog.parts.title.on( 'mousedown', function( evt ) {
			...
			kceditor.document.on( 'mousemove', mouseMoveHandler );
			kceditor.document.on( 'mouseup', mouseUpHandler );
	...
	function initResizeHandles( dialog ) {
		...
		if ( resizable == kceditor.DIALOG_RESIZE_NONE )
			return;

		var editor = dialog.getParentEditor();
		var wrapperWidth, wrapperHeight, viewSize, origin, startSize, dialogCover;

		var mouseDownFn = kceditor.tools.addFunction( function( $event ) {
			startSize = dialog.getSize();

			var content = dialog.parts.contents,
				iframeDialog = content.$.getElementsByTagName( 'iframe' ).length,
				isBorderBox = !( kceditor.env.gecko || kceditor.env.ie && kceditor.env.quirks );

			// Shim to help capturing "mousemove" over iframe.
			if ( iframeDialog ) {
				dialogCover = kceditor.dom.element.createFromHtml( '<div class="cke_dialog_resize_cover" style="height: 100%; position: absolute; width: 100%; left:0; top:0;"></div>' );
			...
			viewSize = kceditor.document.getWindow().getViewPaneSize();
		...
		dialog.on( 'load', function() {
			var direction = '';
			if ( resizable == kceditor.DIALOG_RESIZE_WIDTH )
				direction = ' cke_resizer_horizontal';
			else if ( resizable == kceditor.DIALOG_RESIZE_HEIGHT )
				direction = ' cke_resizer_vertical';
			var resizer = kceditor.dom.element.createFromHtml(
				'<div' +
				' class="cke_resizer' + direction + ' cke_resizer_' + editor.lang.dir + '"' +
				' title="' + kceditor.tools.htmlEncode( editor.lang.common.resize ) + '"' +
				' onmousedown="kceditor.tools.callFunction(' + mouseDownFn + ', event )">' +
		...
		editor.on( 'destroy', function() {
			kceditor.tools.removeFunction( mouseDownFn );
		} );
		...
		function mouseMoveHandler( evt ) {
			...
			if ( resizable == kceditor.DIALOG_RESIZE_WIDTH || resizable == kceditor.DIALOG_RESIZE_BOTH ) {
				width = Math.max( def.minWidth || 0, internalWidth - wrapperWidth );
			}

			if ( resizable == kceditor.DIALOG_RESIZE_HEIGHT || resizable == kceditor.DIALOG_RESIZE_BOTH ) {
	...
	function showCover( editor ) {
		var config = editor.config,
			skinName = ( kceditor.skinName || editor.config.skin ),
			backgroundColorStyle = config.dialog_backgroundCoverColor || ( skinName == 'moono-lisa' ? 'black' : 'white' ),
			backgroundCoverOpacity = config.dialog_backgroundCoverOpacity,
			baseFloatZIndex = config.baseFloatZIndex,
			coverKey = kceditor.tools.genKey( backgroundColorStyle, backgroundCoverOpacity, baseFloatZIndex ),
			coverElement = covers[ coverKey ];

		kceditor.document.getBody().addClass( 'cke_dialog_open' );

		if ( !coverElement ) {
			var html = [
				'<div tabIndex="-1" style="position: ', ( kceditor.env.ie6Compat ? 'absolute' : 'fixed' ),
				'; z-index: ', baseFloatZIndex,
				'; top: 0px; left: 0px; ',
				'; width: 100%; height: 100%;',
				( !kceditor.env.ie6Compat ? 'background-color: ' + backgroundColorStyle : '' ),
			...
			coverElement = kceditor.dom.element.createFromHtml( html.join( '' ) );
			...
			coverElement.appendTo( kceditor.document.getBody() );
		...
		// Using Safari/Mac, focus must be kept where it is (https://dev.kceditor.com/ticket/7027)
		if ( !( kceditor.env.mac && kceditor.env.webkit ) )
			coverElement.focus();
	}

	function hideCover( editor ) {
		kceditor.document.getBody().removeClass( 'cke_dialog_open' );
		if ( !currentCover )
			return;
	...
	( function() {
		kceditor.ui.dialog = {
			 ...
			 *
			 * @class kceditor.ui.dialog.uiElement
			 * @constructor Creates a uiElement class instance.
			 * @param {kceditor.dialog} dialog Parent dialog object.
			 * @param {kceditor.dialog.definition.uiElement} elementDefinition Element
			 * definition.
			 *
			 * Accepted fields:
			 *
			 * * `id` (Required) The id of the UI element. See {@link kceditor.dialog#getContentElement}.
			 ...
			 *     refer to {@link kceditor.ui.dialog.uiElement#registerEvents} and
			 *     {@link kceditor.ui.dialog.uiElement#eventProcessors} for more information.
			 *
			...
			uiElement: function( dialog, elementDefinition, htmlList, nodeNameArg, stylesArg, attributesArg, contentsArg ) {
				...
				var nodeName = ( nodeNameArg.call ? nodeNameArg( elementDefinition ) : nodeNameArg ) || 'div',
					...
					domId = this.domId = attributes.id || kceditor.tools.getNextId() + '_uiElement',
					i;
				...
				for ( i in attributes )
					html.push( i + '="' + kceditor.tools.htmlEncode( attributes[ i ] ) + '" ' );
				...
				if ( typeof elementDefinition.setValue == 'function' ) {
					this.setValue = kceditor.tools.override( this.setValue, function( org ) {
						return function( val ) {
				...
				if ( typeof elementDefinition.getValue == 'function' ) {
					this.getValue = kceditor.tools.override( this.getValue, function( org ) {
						return function() {
				...
				// Add events.
				kceditor.event.implementOn( this );
				...
				dialog.on( 'load', function() {
					...
					if ( input ) {
						var focusClass = me.type in { 'checkbox': 1, 'ratio': 1 } && kceditor.env.ie && kceditor.env.version < 8 ? 'cke_dialog_ui_focused' : '';
				...
				// definition.
				kceditor.tools.extend( this, elementDefinition );
			 ...
			 *
			 * @class kceditor.ui.dialog.hbox
			 * @extends kceditor.ui.dialog.uiElement
			 * @constructor Creates a hbox class instance.
			 * @param {kceditor.dialog} dialog Parent dialog object.
			 * @param {Array} childObjList
			 * Array of {@link kceditor.ui.dialog.uiElement} objects inside this container.
			 * @param {Array} childHtmlList
			 * Array of HTML code that correspond to the HTML output of all the
			 * objects in childObjList.
			 * @param {Array} htmlList
			 * Array of HTML code that this element will output to.
			 * @param {kceditor.dialog.definition.uiElement} elementDefinition
			...
			hbox: function( dialog, childObjList, childHtmlList, htmlList, elementDefinition ) {
				...
				var innerHTML = function() {
						...
						for ( i = 0; i < childHtmlList.length; i++ ) {
							...
							// In IE Quirks alignment has to be done on table cells. (https://dev.kceditor.com/ticket/7324)
							if ( kceditor.env.ie && kceditor.env.quirks && children[ i ].align ) {
				...
				kceditor.ui.dialog.uiElement.call( this, dialog, elementDefinition || { type: 'hbox' }, htmlList, 'table', styles, attribs, innerHTML );
			},

			/**
			 * Vertical layout box for dialog UI elements.
			 *
			 * @class kceditor.ui.dialog.vbox
			 * @extends kceditor.ui.dialog.hbox
			...
			vbox: function( dialog, childObjList, childHtmlList, htmlList, elementDefinition ) {
				...
				var innerHTML = function() {
						...
						// (https://dev.kceditor.com/ticket/10123) Temp fix for dialog broken layout in latest webkit.
						if ( kceditor.env.webkit )
							html.push( 'float:none;' );

						html.push( '"' );
						html.push( 'align="', kceditor.tools.htmlEncode(
				...
				kceditor.ui.dialog.uiElement.call( this, dialog, elementDefinition || { type: 'vbox' }, htmlList, 'div', null, { role: 'presentation' }, innerHTML );
			}
		};
	} )();

	/** @class kceditor.ui.dialog.uiElement */
	kceditor.ui.dialog.uiElement.prototype = {
		/**
		 * Gets the root DOM element of this dialog UI object.
		 *
		 *		uiElement.getElement().hide();
		 *
		 * @returns {kceditor.dom.element} Root DOM element of UI object.
		 */
		getElement: function() {
			return kceditor.document.getById( this.domId );
		},
		 ...
		 *
		 * @returns {kceditor.dom.element} The element where the user input values.
		 */
		 ...
		 *
		 * @returns {kceditor.dialog} Parent dialog object.
		 */
		getDialog: function() {
		 ...
		 *
		 * @chainable
		 * @param {kceditor.dialog.definition.uiElement} definition The UI element
		 * definition.
		 */
		 ...
		 * {@link kceditor.ui.dialog.uiElement#getInputElement} at UI element
		 ...
		 *
		 *		// This connects the 'click' event in kceditor.ui.dialog.button to onClick
		 *		// handlers in the UI element's definitions.
		 *		kceditor.ui.dialog.button.eventProcessors = kceditor.tools.extend( {},
		 *			kceditor.ui.dialog.uiElement.prototype.eventProcessors,
		 ...
		 *
		 * @param {kceditor.dialog} dialog The parent dialog object.
	...
	/** @class kceditor.ui.dialog.hbox */
	kceditor.ui.dialog.hbox.prototype = kceditor.tools.extend( new kceditor.ui.dialog.uiElement(), {
		/**
		 ...
		 * @returns {Array/kceditor.ui.dialog.uiElement} Array of all UI elements in the container
	...
	kceditor.ui.dialog.vbox.prototype = new kceditor.ui.dialog.hbox();
		...
		var commonBuilder = {
			build: function( dialog, elementDefinition, output ) {
				...
				( i < children.length && ( child = children[ i ] ) ); i++ ) {
					...
					childObjList.push( kceditor.dialog._.uiElementBuilders[ child.type ].build( dialog, child, childHtml ) );
				}
				return new kceditor.ui.dialog[ elementDefinition.type ]( dialog, childObjList, childHtmlList, output, elementDefinition );
			}
		};

		kceditor.dialog.addUIElement( 'hbox', commonBuilder );
		kceditor.dialog.addUIElement( 'vbox', commonBuilder );
	} )();
	 ...
	 *		editor.addCommand( 'link', new kceditor.dialogCommand( 'link' ) );
	 *
	 * @class
	 ...
	 *		editor.addCommand( 'keystroke', new kceditor.dialogCommand( 'a11yHelp', { tabId: 'keystroke' } ) );
	 */
	kceditor.dialogCommand = function( dialogName, ext ) {
		this.dialogName = dialogName;
		kceditor.tools.extend( this, ext, true );
	};

	kceditor.dialogCommand.prototype = {
	...
	( function() {
		...
		/**
		 * {@link kceditor.dialog Dialog} `OR` logical value indicates the
		 ...
		 * @member kceditor
		 */
		kceditor.VALIDATE_OR = 1;

		/**
		 * {@link kceditor.dialog Dialog} `AND` logical value indicates the
		 * relation between validation functions.
		 *
		 * @readonly
		 * @property {Number} [=2]
		 ...
		 * @singleton
		 */
		kceditor.dialog.validate = {
			/**
			 * Performs validation functions composition.
			 *
			 * ```javascript
			 * kceditor.dialog.validate.functions(
			 * 	kceditor.dialog.validate.notEmpty( 'Value is required.' ),
			 * 	kceditor.dialog.validate.number( 'Value is not a number.' ),
			 * 	'error!'
			 * );
			 * ```
			 ...
			 * @param {Number} [relation=kceditor.VALIDATE_OR] Indicates a relation between validation functions.
			 * Use {@link kceditor#VALIDATE_OR} or {@link kceditor#VALIDATE_AND}.
			 *
			...
			functions: function() {
				...
				return function( value ) {
					...
					var msg,
						relation = kceditor.VALIDATE_AND,
						functions = [],
						i;
			 ...
			 *
			 * ```javascript
			 * kceditor.dialog.validate.regex( /^\d*$/, 'error!' )( '123' ) // true
			 * kceditor.dialog.validate.regex( /^\d*$/, 'error!' )( '123.321' ) // error!
			 * ```
			 *
			 ...
			 *
			 * ```javascript
			 * kceditor.dialog.validate.notEmpty( 'error!' )( 'test' ) // true
			 * kceditor.dialog.validate.notEmpty( 'error!' )( '  ' ) // error!
			 * ```
			 *
			 * @param {String} msg Validator error message.
			 * @returns {Function} Validation function.
			 ...
			 *
			 * ```javascript
			 * kceditor.dialog.validate.integer( 'error!' )( '123' ) // true
			 * kceditor.dialog.validate.integer( 'error!' )( '123.321' ) // error!
			 * ```
			 *
			 * @param {String} msg Validator error message.
			 * @returns {Function} Validation function.
			 ...
			 *
			 * ```javascript
			 * kceditor.dialog.validate.number( 'error!' )( '123' ) // true
			 * kceditor.dialog.validate.number( 'error!' )( 'test' ) // error!
			 * ```
			 *
			 * @param {String} msg Validator error message.
			 * @returns {Function} Validation function.
			 ...
			 *
			 * ```javascript
			 * kceditor.dialog.validate.cssLength( 'error!' )( '10pt' ) // true
			 * kceditor.dialog.validate.cssLength( 'error!' )( 'solid' ) // error!
			 * ```
			 *
			 * @param {String} msg Validator error message.
			 * @returns {Function} Validation function.
			...
			'cssLength': function( msg ) {
				return this.functions( function( val ) {
					return cssLengthRegex.test( kceditor.tools.trim( val ) );
				}, msg );
			},
			 ...
			 *
			 * ```javascript
			 * kceditor.dialog.validate.htmlLength( 'error!' )( '10px' ) // true
			 * kceditor.dialog.validate.htmlLength( 'error!' )( 'solid' ) // error!
			 * ```
			 *
			 * @param {String} msg Validator error message.
			 * @returns {Function} Validation function.
			...
			'htmlLength': function( msg ) {
				return this.functions( function( val ) {
					return htmlLengthRegex.test( kceditor.tools.trim( val ) );
				}, msg );
			},
			 ...
			 *
			 * ```javascript
			 * kceditor.dialog.validate.inlineStyle( 'error!' )( '' ) // true
			 * kceditor.dialog.validate.inlineStyle( 'error!' )( 'height: 10px; width: 20px;' ) // true
			 * kceditor.dialog.validate.inlineStyle( 'error!' )( 'test' ) // error!
			 * ```
			 *
			 * @param {String} msg Validator error message.
			 * @returns {Function} Validation function.
			...
			'inlineStyle': function( msg ) {
				return this.functions( function( val ) {
					var properties = kceditor.tools.trim( val ).split( ';' );
					...
					return kceditor.tools.array.every( properties, function( property ) {
						return inlineStylePropertyRegex.test( kceditor.tools.trim( property ) );
					} );
				}, msg );
			},
			 ...
			 *
			 * ```javascript
			 * kceditor.dialog.validate.equals( 'foo', 'error!' )( 'foo' ) // true
			 * kceditor.dialog.validate.equals( 'foo', 'error!' )( 'baz' ) // error!
			 * ```
			 *
			 ...
			 *
			 * ```javascript
			 * kceditor.dialog.validate.notEqual( 'foo', 'error!' )( 'baz' ) // true
			 * kceditor.dialog.validate.notEqual( 'foo', 'error!' )( 'foo' ) // error!
			 * ```
			 *
			 * @param {String} value The value to compare.
			 * @param {String} msg Validator error message.
		...
		kceditor.on( 'instanceDestroyed', function( evt ) {
			// Remove dialog cover on last instance destroy.
			if ( kceditor.tools.isEmpty( kceditor.instances ) ) {
				var currentTopDialog;
				while ( ( currentTopDialog = kceditor.dialog._.currentTop ) )
	...
	// Extend the kceditor.editor class with dialog specific functions.
	kceditor.tools.extend( kceditor.editor.prototype, {
		/**
		 * Loads and opens a registered dialog.
		 *
		 *		kceditor.instances.editor1.openDialog( 'smiley' );
		 *
		 * @member kceditor.editor
		 * @param {String} dialogName The registered name of the dialog.
		 * @param {Function} callback The function to be invoked after a dialog instance is created.
		 * @param {kceditor.dom.element/kceditor.plugins.widget/Object} [forceModel] Forces opening the dialog
		 * using the given model as a subject. The forced model will take precedence before the
		 * {@link kceditor.dialog.definition#getModel} method. Available since 4.13.0.
		 * @returns {kceditor.dialog} The dialog object corresponding to the dialog displayed or
		 * `null` if the dialog name is not registered.
		 * @see kceditor.dialog#add
		 */
		openDialog: function( dialogName, callback, forceModel ) {
			var dialog = null, dialogDefinitions = kceditor.dialog._.dialogDefinitions[ dialogName ];
			...
			if ( typeof dialogDefinitions == 'function' ) {
				...
				dialog = storedDialogs[ dialogName ] || ( storedDialogs[ dialogName ] = new kceditor.dialog( this, dialogName ) );
				...
				hideCover( this );
				throw new Error( '[kceditor.dialog.openDialog] Dialog "' + dialogName + '" failed when loading definition.' );
			} else if ( typeof dialogDefinitions == 'string' ) {

				kceditor.scriptLoader.load( kceditor.getUrl( dialogDefinitions ),
					function() {
						var dialogDefinition = kceditor.dialog._.dialogDefinitions[ dialogName ];
						// In case of plugin error, mark it as loading failed.
						if ( typeof dialogDefinition != 'function' )
							kceditor.dialog._.dialogDefinitions[ dialogName ] = 'failed';

						this.openDialog( dialogName, callback, forceModel );
					}, this, 0, 1 );
			}

			kceditor.skin.loadPart( 'dialog' );
	...
	kceditor.plugins.add( 'dialog', {
		requires: 'dialogui',
		init: function( editor ) {
			if ( !stylesLoaded ) {
				kceditor.document.appendStyleSheet( this.path + 'styles/dialog.css' );
				stylesLoaded = true;
			}
 ...
 * @param {kceditor.dialog.definition} data.definition The dialog definition that
 * is being loaded.
 * @param {kceditor.dialog} data.dialog A dialog instance that the definition is loaded
 * for. Introduced in **KCEditor 4.13.0**.
 * @param {kceditor.editor} editor The editor instance that will use the dialog.
 */
 ...
 * @event dialogShow
 * @member kceditor.editor
 * @param {kceditor.editor} editor This editor instance.
 * @param {kceditor.dialog} data The opened dialog instance.
 */
 ...
 * @member kceditor.editor
 * @param {kceditor.editor} editor This editor instance.
 * @param {kceditor.dialog} data The hidden dialog instance.
 */

/**
 * Event fired when a dialog is being resized. The event is fired on
 * both the {@link kceditor.dialog} object and the dialog instance
 ...
 * @member kceditor.dialog
 * @param data
 * @param {kceditor.dialog} data.dialog The dialog being resized (if
...
/**
 * Event fired when the dialog state changes, usually by {@link kceditor.dialog#setState}.
 *
 * @since 4.5.0
 * @event state
 * @member kceditor.dialog
 * @param data
 * @param {Number} data The new state. Either {@link kceditor#DIALOG_STATE_IDLE} or {@link kceditor#DIALOG_STATE_BUSY}.
 */
